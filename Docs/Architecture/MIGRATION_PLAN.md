...
Next, advance only Group `01` to `approach-owned`: replace `anchorMigrationStage: 1` with `anchorMigrationStage: 2` in Passage documents `4100000013` and `4100000014`, then update only inventory, tests, and architecture documents. Keep both calibrated reciprocal coordinates and every caller/dependency/topology edge unchanged. Prove exact collision-aware movement to each authored approach from multiple far starts at all four aspects and maximum zoom, plus unchanged near traversal and compatibility fallback. End that slice at `approach-owned`; then run a separate no-scene-change complete-pair certification and advance only inventory/documentation to `complete`.

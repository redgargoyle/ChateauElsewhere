...
Exact next safe phase: characterize only Group `02`, Music Room <-> Library, before authoring its canonical data. Begin with a tests-only legacy snapshot and rendered far/near reciprocal traversal across the required aspects; do not add the Library RoomDefinition, Library RoomView, Passage definitions/components, direct dependencies, canonical callers, or authored anchors until that characterization is independently accepted.

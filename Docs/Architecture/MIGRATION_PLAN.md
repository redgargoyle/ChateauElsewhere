...
Group `02` Music Room <-> Library characterization status: complete at the tests-only inventory state `characterized`. Existing trigger components `552135204` (`MusicRoom_Library`) and `2300000079` (`Library_MusicRoom`) retain their exact four-component owners, room-root/Doors ancestry, rectangles, standard Door profile, `145`-pixel proximity, null serialized dependencies, null canonical callers, prompt/cursor/audio lifecycle, and Inspector traversal. Music retains existing RoomView `4100000003`; Library still has no RoomDefinition, RoomView, PassageDefinition, Passage component, direct dependency, canonical caller, or authored anchor. Runtime resolution still finds the one navigation manager, Player, shared door AudioSource, and door catalog without changing those null serialized fields. The Library chair/lamp blocker and both room boundaries remain exact, and Chapter 2 hide anchors `Ch2_Hide_Guest01` at `(-255, -181)` in Library and `Ch2_Hide_Guest02` at `(306, -162)` in Music follow active-room visibility.
...
Exact next safe slice: add only the Library RoomDefinition plus two reciprocal Music/Library PassageDefinitions and register those three assets once in `GameDatabase`. Do not add a Library RoomView, either Passage component, direct trigger dependencies, canonical callers, or authored approach/arrival anchors in that data-only slice.

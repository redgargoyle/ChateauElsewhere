...
Group `01` passive-passage status: complete. Drawing trigger owner GameObject `2300000095` now carries Passage `4100000013`, bound to definition GUID `3167361ca4c671298c0e84f43320619b`, source RoomView `4100000002`, and reverse Passage `4100000014`; its approach/arrival values are `(-7.106010, -1.508934)` / `(-7.737432, -3.180156)`. Music trigger owner GameObject `2300000085` carries Passage `4100000014`, bound to definition GUID `01544de8f55723585d60e5c0915345fd`, source RoomView `4100000003`, and reverse Passage `4100000013`; its approach/arrival values are `(-7.737432, -3.180156)` / `(-7.106010, -1.508934)`. GameRoot registers the complete Passage order `4100000011`, `4100000012`, `4100000013`, `4100000014`. These finite reciprocal anchors are passive certification data only: no trigger or navigation caller consumes them, and they do not become authoritative until the separate `arrival-owned` and `approach-owned` gates.
...
Group `01` canonical-caller status: complete. Trigger `2300000099` now binds its co-located Drawing -> Music Passage `4100000013`, and trigger `2300000089` binds Music -> Drawing Passage `4100000014`. The scene delta is exactly two added YAML lines in those two existing trigger documents; Gameplay remains `6,014` documents / 3 RoomViews / 4 Passages. No Passage field or stage, anchor value, direct dependency, owner/component edge, GameRoot entry/order, document ID/order, runtime script, prefab, asset, or `.meta` changes. Exactly four of 45 triggers now have canonical callers and 41 remain null; direct bindings remain 4/41. Both Drawing/Music Passages stay `LegacySampling = 0`, so the real serialized callers use canonical identity while the existing legacy approach and arrival samplers remain authoritative. The rendered poison proof now exercises those real caller edges and retains the exact far/near bidirectional movement, arrival, event/audio, camera/visibility, prompt/cursor, and story behavior. Focused canonical/manifest gates pass `9/9`, combined safety passes `27/27`, rendered lifecycle passes `5/5`, and the full suite remains `254` discovered / `208` passed / the exact same `46` baseline failures with failure-name SHA-256 `544759729ac446b3814a3f206021a23c64fd46cc9edc1e997b179affaa0f69f9`. Architecture remains 112 runtime files / 48,787 lines / 48 direct `MonoBehaviour` declarations with unchanged smells, serialized references remain 155 rows, and Y-axis remains zero hard errors / 38 tracked findings.

Next, advance only Group `01` to `arrival-owned`: replace `anchorMigrationStage: 0` with `anchorMigrationStage: 1` in Passage documents `4100000013` and `4100000014`, then update only the inventory, tests, and these architecture documents. Keep both canonical callers, direct dependencies, approach/arrival values, scene topology, and runtime code unchanged. Prove legacy approach sampling remains exact while far and near traversal land on each Passage's exact authored arrival before advancing approach ownership.

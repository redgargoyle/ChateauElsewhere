...
Group `18` Upper Sitting Hall <-> Blue Bedroom is now the accepted production checkpoint. Blue Bedroom RoomView `4100000055`, reciprocal Passages `4100000056` / `4100000057`, and PassageDefinition GUIDs `0927098c96b54093aa079279a7dcad2a` / `84148ad6c0674b03bbda8bcb0690325d` complete passive RoomView representation for all nineteen room definitions while preserving the exact Group18 six-line SHA-256 `ceadb389a272ad0e13a63f7da306109a1bc9277c2fcc986bca70ebf221131a9b`. The accepted graph is Gameplay `6,064` documents / nineteen RoomViews / thirty-eight stage-2 Passages / 45 triggers, `GameDatabase 57` (`19` rooms / `38` passages), and GameRoot `66` scene behaviours / `74` references. Canonical callers/dependencies are `38 bound / 7 null`; direct catalogs are `37` door / `1` stair / `7` fully unbound; inventory is `38 complete / 2 queued / 2 blocked-one-way / 3 blocked-parallel`; placement serialization is `16` source regions / `18` destination regions / `10` explicit coordinate-space values. Architecture tools `4/4`, guard, ledger `114/114`, `165` scripts / `1,996` serialized references (`1,140` project / `856` external), `7,381` paired unique metas, and Unity compilation pass. Focused EditMode is honestly accounted as `78/66/12`, then `78/77/1`, then isolated `1/1`; rendered Group18/17 passes exact `2/2`; and Group18/17/16/15 plus cold PlayMode is accounted as `5/4/1` plus isolated `1/1`. Human doorway, camera, audio, Blue Bedroom occlusion/collision/lighting, and Upper Sitting Hall content review remains; no pruning is authorized. The next tests-first route is Group `19` Nursery <-> Blue Bedroom.
...
## Accepted Slice 2.2-15 Group 15 checkpoint

Group `15` transfers Upper Sitting Hall <-> Upper Gallery onto the canonical scene/data graph without changing either standard-door interaction. Upper Gallery RoomView `4100000046`, PassageDefinitions `9968ef27c1a4121f14100399fe85ab32` / `54dc2ad728e40e59f280711e65b3474d`, and Passages `4100000047` / `4100000048` bind existing triggers `2300000209` / `2300000219`. Stable IDs are `passage.upper-gallery.upper-sitting-hall` / `passage.upper-sitting-hall.upper-gallery`; compatibility IDs `UpperGallery_UpperSittingHall` / `UpperSittingHall_UpperGallery` and display names remain exact.

Both directions serialize stage `2`, source-region approach mode `1`, destination-region arrival mode `1`, no point anchors, direct reciprocal topology, and the existing door catalog. Gallery -> Upper owns the Upper trigger region `BL (-174.1340942, -42.82170105) / TL (-174.1340942, 207.995697) / TR (-135.8659058, 207.995697) / BR (-135.8659058, -42.82170105)`. Upper -> Gallery owns the Gallery trigger region `BL (587.2802124, -69.99983978) / TL (587.2802124, 225.1838379) / TR (710.7197876, 225.1838379) / BR (710.7197876, -69.99983978)`. Each source approach derives from the reciprocal Passage-owned region.

The accepted shape is Gameplay `6,055` documents / sixteen RoomViews / thirty-two stage-2 Passages / 45 triggers; `GameDatabase 51` (`19` rooms / `32` passages); GameRoot `57` scene behaviours / `65` references; dependencies/callers `32 bound / 13 null`; and inventory `32 complete / 8 queued / 2 blocked-one-way / 3 blocked-parallel`. Exact hashes are Gameplay `4ba54fac49bb37c9c693c0fee0e7c32e90aae0e29c75a5989b67da8fba64f5f2`, database `db24627071eff29beaa11c072363d4c2abb1ebeb0012504ab39c17fee394fb7f`, inventory `e7e066f477ce3599526d0a9e3fb8404636752508113fe55dc005ba5a8d4c9820`, and unchanged `doors.txt` `8dc956b84e8436054a8835a7fa7f12f0aa2ce14d1d9a90701e8d98c3f001798e`.

Architecture tools, static/GUID controls, Unity compilation, all 75 accounted focused cases, rendered Group15/14 `2/2`, and genuine Group15/representatives/cold PlayMode `5/5` pass. Group15 SHA is `ed8fad6c038ea9d1ab346cbc7e463403a09b6aab562ad5564d9c48519c29fda2`; cold SHA and Butler metrics remain unchanged. No deletion is authorized. Human review remains for both landings, Butler feet, prompt/cursor/audio cleanup, camera/visibility, and Upper Gallery collision/occlusion. The next safe slice is tests-first Group `16` Upper Gallery <-> Master Bedroom Suite characterization.

## Accepted Slice 2.2-16 Group 16 checkpoint

Starting from clean characterization/rollback commit `9fb4a3d9131368818e6c6fb1f8009b844b9b3a5e`, Group `16` transfers Upper Gallery <-> Master Bedroom Suite onto the canonical scene/data graph without changing either standard-door interaction. Master Bedroom Suite RoomView `4100000049`, PassageDefinitions `1dff2bb730264e9c81ecb2e35985f5a6` / `f1fd515e16c44606af75bf593f67bc07`, and Passages `4100000050` / `4100000051` bind existing triggers `2300000204` / `2300000214`. Stable IDs are `passage.upper-gallery.master-bedroom-suite` / `passage.master-bedroom-suite.upper-gallery`; legacy IDs, display names, Door / `Open Door`, and the direct door catalog remain exact.

Both directions serialize stage `2`, source-region approach mode `1`, destination-region arrival mode `1`, no point anchors, and direct reciprocal topology. Gallery -> Master owns the Master destination region `BL (-770.5059814, -210.125) / TL (-770.5059814, 264.125) / TR (-599.4940186, 264.125) / BR (-599.4940186, -210.125)`. Master -> Gallery owns the Gallery destination region `BL (-722.8327637, -70.00001526) / TL (-722.8327637, 195.9999847) / TR (-604.0205078, 195.9999847) / BR (-604.0205078, -70.00001526)`. Each source approach derives from the reciprocal Passage-owned region. The Master return trigger's non-unit transform and all protected room geometry remain unchanged.

The accepted shape is Gameplay `6,058` documents / seventeen RoomViews / thirty-four stage-2 Passages / 45 triggers; `GameDatabase 53` (`19` rooms / `34` passages); GameRoot `60` scene behaviours / `68` references including eight services; dependencies/callers `34 bound / 11 null`; direct catalogs `33` door / `1` stair / `11` unbound; inventory `34 complete / 6 queued / 2 blocked-one-way / 3 blocked-parallel`; and placement serialization `12` source regions / `14` destination regions / `10` explicit coordinate-space values. Exact hashes are Gameplay `5f5b975165b3ed02d529cea132d6943278ef709f11c8913efda769b405617b5b`, database `6df8c80cb0f98c78ac0a5fb651a44bf3096020ecc46bb02acab8116ad78e338d`, inventory `e901dde6232c67612ca877fd2258cf10f90f5eb2921eac7fded2aaa2e75a9477`, and unchanged `doors.txt` `8dc956b84e8436054a8835a7fa7f12f0aa2ce14d1d9a90701e8d98c3f001798e`.

Architecture tools pass `4/4`; architecture guard, exact `114/114` runtime-ledger coverage, script-integrity, serialized-reference, and metadata/GUID controls pass at `114` runtime files / `51,224` lines / `48` direct MonoBehaviours, smells `106/17/67/51/4/6`, `165` scripts / `1,986` serialized references (`1,130` project / `856` external), and `7,377` paired unique meta GUIDs. Unity `6000.4.10f1` compiles after one test-only syntax repair. The focused gate accounts for all `76` cases (`74` passed / `2` stale literals, then isolated repair `2/2`); rendered Group16/15 passes `2/2` with exact SHAs `f97e30d0807e46ee1846bc1421ebcb0f72a694988c8fda915bb7e87b4b3acb0f` / `ed8fad6c038ea9d1ab346cbc7e463403a09b6aab562ad5564d9c48519c29fda2`; genuine Group16/15/14/13 plus cold-start PlayMode passes `5/5`, retaining cold SHA `34ea66772abd7375f965b2277e7342c82dbd853bc1efecc8d82a00e1b403dd96`. The production-data audit finds no defects: all `856` RectTransforms are byte-identical and maximum region float error is `0.00003220`.

No deletion is authorized; eleven compatibility callers remain. Human review must confirm both doorway landings, approach movement, Butler feet, prompt/cursor/audio cleanup, camera/active-room visibility, and Master Bedroom Suite occlusion/collision. The next safe slice is tests-first Group `17` Upper Sitting Hall <-> Nursery characterization on triggers `2300000224` / `2300000234`.

## Accepted Slice 2.2-17 Group 17 production checkpoint

The committed characterization gate freezes Upper Sitting Hall <-> Nursery legacy behavior at exact no-trailing-newline six-line SHA-256 `57c5dccca03303a443cde3756c02047021d934c5230e51da8e087d48a9649bc4`. The accepted production slice adds Nursery RoomView `4100000052`, PassageDefinitions `e40848fe5ab54ec99ff3b664859e7c1b` / `c05a3dd92c9f45cf8231f1679bd06506`, and reciprocal Passages `4100000053` / `4100000054` to existing triggers `2300000224` / `2300000234`. Stable IDs are `passage.upper-sitting-hall.nursery` / `passage.nursery.upper-sitting-hall`; legacy IDs remain `UpperSittingHall_Nursery` / `Nursery_UpperSittingHall`. Both directions remain standard Door / `Open Door`, stage `2`, source-region mode `1`, destination-region mode `1`, point-free, and directly bound to the existing door catalog.

Upper Sitting Hall -> Nursery owns Nursery destination region `BL (157, -88.39423) / TL (157, 209.43507) / TR (277, 209.43507) / BR (277, -88.39423)`. Nursery -> Upper Sitting Hall owns Upper destination region `BL (-78.9274, -32.718) / TL (-78.9274, 178.862) / TR (46.9532, 178.862) / BR (46.9532, -32.718)`. Each source approach derives from the reciprocal Passage-owned region, leaving one serialized geometry owner and no fabricated point anchor.

The accepted checkpoint contains Gameplay `6,061` documents / eighteen RoomViews / thirty-six stage-2 Passages / 45 triggers; `GameDatabase 55` definitions (`19` rooms / `36` passages); GameRoot `63` scene behaviours / `71` references including eight services; dependencies/callers `36 bound / 9 null`; direct catalogs `35` door / `1` stair / `9` unbound; inventory `36 complete / 4 queued / 2 blocked-one-way / 3 blocked-parallel`; and placement serialization `14` source regions / `16` destination regions / `10` explicit coordinate-space values. Nursery is now represented; Blue Bedroom is the one remaining passive/data-only room.

Architecture tools pass `4/4`; architecture guard, exact `114/114` runtime-ledger coverage, script-integrity, serialized-reference, and metadata/GUID controls pass at `165` scripts / `1,991` serialized references (`1,135` project / `856` external) and `7,379` paired unique metas. Unity compiles. The focused EditMode launch accounts for all `77` cases (`76` pass / one stale prior Group15-neighbor expectation), and its isolated test-only repair passes `1/1`. Rendered Group17/16 passes `2/2` at exact SHAs `57c5dccca03303a443cde3756c02047021d934c5230e51da8e087d48a9649bc4` / `f97e30d0807e46ee1846bc1421ebcb0f72a694988c8fda915bb7e87b4b3acb0f`; genuine Group17/16/15/14 plus cold-start PlayMode passes `5/5`, retaining cold SHA `34ea66772abd7375f965b2277e7342c82dbd853bc1efecc8d82a00e1b403dd96`. Human doorway, camera, audio, and Nursery occlusion review remains. No deletion or pruning is authorized. The exact next safe slice is tests-first Group `18` Upper Sitting Hall <-> Blue Bedroom.

## Current Slice 2.2-18 Group 18 accepted production checkpoint

The committed Group18 characterization freezes Upper Sitting Hall <-> Blue Bedroom at exact no-trailing-newline six-line SHA-256 `ceadb389a272ad0e13a63f7da306109a1bc9277c2fcc986bca70ebf221131a9b`. The accepted production slice adds Blue Bedroom RoomView `4100000055` to existing root/content `2300000060` / `2300000062` and existing RoomDefinition GUID `98e71938214a483786829422c9a47278`. Existing triggers `2300000229` / `2300000239` gain Passages `4100000056` / `4100000057`, PassageDefinition GUIDs `0927098c96b54093aa079279a7dcad2a` / `84148ad6c0674b03bbda8bcb0690325d`, reciprocal links, canonical callers, and the existing manager/player/audio/door-catalog dependencies. Stable IDs are `passage.upper-sitting-hall.blue-bedroom` / `passage.blue-bedroom.upper-sitting-hall`; legacy IDs remain `UpperSittingHall_BlueBedroom` / `BlueBedroom_UpperSittingHall`. Both directions remain standard Door / `Open Door`, stage `2`, source-region mode `1`, destination-region mode `1`, point-free, and directly bound to the existing door catalog.

Upper Sitting Hall -> Blue Bedroom owns the Blue destination region `BL (-715, -240.000061) / TL (-715, 259.7826538) / TR (-532.2076416, 259.7826538) / BR (-532.2076416, -240.000061)`. Blue Bedroom -> Upper Sitting Hall owns the Upper destination region `BL (334.5011902, -225.5001984) / TL (334.5011902, 330.0778198) / TR (437.8641663, 330.0778198) / BR (437.8641663, -225.5001984)`. Each source approach derives from the reciprocal Passage-owned region, leaving one serialized geometry owner and no fabricated point anchor; the forward trigger's authored X scale remains exactly `0.82498`.

The accepted checkpoint contains Gameplay `6,064` documents / nineteen RoomViews / thirty-eight stage-2 Passages / 45 triggers; `GameDatabase 57` definitions (`19` rooms / `38` passages); GameRoot `66` scene behaviours / `74` references including eight services; dependencies/callers `38 bound / 7 null`; direct catalogs `37` door / `1` stair / `7` fully unbound; inventory `38 complete / 2 queued / 2 blocked-one-way / 3 blocked-parallel`; and placement serialization `16` source regions / `18` destination regions / `10` explicit coordinate-space values. Exact production SHA-256 values are Gameplay `a7aa7626f0fab98a68ff505c2a3984f767829f2791a24e91976e6575bd555ae1`, GameDatabase `2a0875f508840bb51c76218ebc15be7f8d06f94665228f625af7cb3e9ad7f8eb`, inventory `8ef5f2befcf0a4e6ff4326194b1da710fb186ccbc1a3c6322ee537b71eeb696b`, definition assets `ab6f99421ad998e2b91125913213ec44e04dd8e3fce514be98f3399851789110` / `d0b7122c93257d9a24615b323dedde9b13687cfdfd97f83f27d4e774adb79785`, and unchanged `doors.txt` `8dc956b84e8436054a8835a7fa7f12f0aa2ce14d1d9a90701e8d98c3f001798e`.

Architecture tools pass `4/4`; architecture guard, exact `114/114` runtime-ledger coverage, script-integrity, serialized-reference, CSV-shape, and metadata pairing/uniqueness checks pass at `114` runtime files / `51,224` lines / `48` direct MonoBehaviours, smells `106/17/67/51/4/6`, `165` scripts / `1,996` references (`1,140` project / `856` external), and `7,381` paired globally unique metas. Generated SHA-256 values are runtime inventory `35b78190fc8820883abde1b8db531edbcd13108742fbfb515a7fb187ac3c8f69`, summary `6748d21ca5f2f6f424fc2c62c42fce36cc076e92ef2498958fc717614eef059c`, generator-native script integrity `7cc065d80eba63b3812fc35add80c75f71e12c46f653e404e803648e4b6350a6`, and serialized references `4fc732141e84ed09b25732db05baa60361efaed7098a6784c9abd69eaac0811f`. Unity `6000.4.10f1` compiles with no C# compiler error.

The focused EditMode sequence remains honest: the first run is `78/66/12` from stale current-graph literals and two stale canonical evidence strings; the batched repair rerun is `78/77/1`; and the final fixture-only correction passes isolated `1/1`, accounting for all `78` without relabeling either failed XML. Rendered Group18/17 passes exact `2/2` at SHAs `ceadb389a272ad0e13a63f7da306109a1bc9277c2fcc986bca70ebf221131a9b` / `57c5dccca03303a443cde3756c02047021d934c5230e51da8e087d48a9649bc4`. Genuine Group18, representative Groups17/16/15, and cold start record `5/4/1` because the new PlayMode fixture inherited the reversed far-start probe for this unusually tall trigger; its test-only correction passes isolated `1/1`, accounting for all five. Cold SHA remains `34ea66772abd7375f965b2277e7342c82dbd853bc1efecc8d82a00e1b403dd96`; Butler height is `114.417 px`, entrance-door height `141.481 px`, ratio `0.808710`, presentation/world multiplier `0.752865`, and sort `1075`.

All `856` pre-existing RectTransforms, both trigger transforms, the Upper six-point boundary and drawers, and Blue Bedroom's exact child order, twelve-point boundary, bed/chair cutouts, four-point blockers and People sorting, inactive moon-flicker patch, thirty-one-child lighting hierarchy, and Chapter 2 anchor remain protected. Group19 triggers `1084796962` / `1498249206` remain four-component, null, and queued. No runtime source, existing `.meta`/GUID, prefab, catalog row, threshold, hierarchy, boundary, art/audio asset, package, ProjectSettings file, or large asset changed. No pruning or deletion is authorized while seven callers remain legacy. Human review must still confirm doorway feet/approach/landings, prompt/cursor/audio cleanup, camera and active-room visibility, Blue cutout/blocker occlusion/collision and lighting, and Upper Sitting Hall content; no manual visual pass is claimed. The exact next safe slice is tests-first Group `19` Nursery <-> Blue Bedroom characterization on existing triggers `1084796962` / `1498249206`; do not author or prune Group19 before its legacy behavior is locked.

...
The Phase 4 navigation seam now contains thirteen registered definitions, five passive RoomViews, and eight directed Passage instances. Six Passages belong to three fully template-certified reciprocal pairs—Entrance/Drawing, Drawing/Music, and Music/Library—while the two Library/Ballroom Passages are caller-bound compatibility data at `LegacySampling = 0`. `RoomDefinition` and directed reciprocal `PassageDefinition` carry stable data identity; `RoomView` only validates/reports its existing root; and `Passage` validates/reports its definition, source room, reverse link, logical anchors, and one temporary per-pair anchor-migration stage. The existing `RoomNavigationManager` remains the sole room-state owner behind `INavigationService`: canonical current room is derived from its one legacy room string and registered database. Six scene Passages serialize `AuthoredAnchors = 2`, matching six `complete` / `template-certified` inventory rows; the other two serialize stage 0, with no stage-1 Passage. The accepted canonical room-side anchors are Entrance `(-7.75, -2.22)`, Drawing `(5.267176, -2.104616)` for Entrance/Drawing, Drawing `(-7.16, -1.78)`, Music `(-7.94, -3.27)` for Drawing/Music, and Music `(7.714471, -3.121709)`, Library `(-7.744175, -3.059095)` for Music/Library. Their acceptance contract evaluates each exact non-projected point from two far starts at all four rendered aspects and widest-aspect maximum zoom, drives real production movement, then protects near synchronous traversal, null-Passage fallback, and the complete lifecycle side-effect sequence. A prerequisite fix synchronizes destination-room collider transforms before exact arrival validation because Unity's automatic 2D transform synchronization is disabled; this prevents a room stage last seen at zoom `1.22` from leaving a stale collider cache during same-call placement. This temporary 0/1/2 gate keeps caller, arrival, and approach cutovers independently testable and is deleted only after every Passage in the full migration reaches the complete stage. `PointClickPlayerMovement.TryWarpToExact` validates authored destinations without clamping, projection, or an old-room path requirement. Trigger input, proximity, hover/prompt, movement callbacks, and audio ownership remain on the compatibility interaction owner. The Music -> Library trigger retains its standard `145`-pixel envelope. The Library -> Music trigger is calibrated pair-locally from `145` to `149`: exhaustive rendered search found that the structurally safest invariant Library point requires `148.573` pixels at its worst profile, so four pixels is the minimum whole-pixel compatibility increase. Thirty-seven scene triggers retain fully null direct dependencies, and 37 retain null canonical callers. All resolvers remain, and no `RoomViewService` activation writer exists. The exact next safe slice is a tests-only Library/Ballroom arrival calibration/preflight; neither Passage advances from stage 0 until one reciprocal arrival pair is collision-safe across every rendered profile.

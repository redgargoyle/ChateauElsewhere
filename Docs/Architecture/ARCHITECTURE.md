...
The first Phase 4 navigation seam now contains canonical room/passage contracts, four registered definitions, two passive RoomViews on the first route's room roots, two reciprocal passive Passages on the existing trigger owners, and a rendered characterization of both passage directions. `RoomDefinition` and directed reciprocal `PassageDefinition` carry stable data identity; `RoomView` only validates/reports its existing root; and `Passage` only validates/reports its definition, source room, reverse link, and logical anchors. The existing `RoomNavigationManager` now implements `INavigationService` as a no-new-state compatibility façade: canonical current room is derived from its one legacy room string and registered database, while traversal still delegates to its existing Inspector route method. Only the first reciprocal legacy trigger pair has direct compatibility references to the manager, exact Player Transform, shared door-audio source, and approved catalog; all legacy resolvers remain available for the other 43 scene triggers. Both `DoorTriggerNavigation` callers still invoke the legacy method directly, so interaction, movement, audio, activation, and warping ownership has not cut over yet, and no `RoomViewService` activation writer exists.

The characterization deliberately separates authored logical data from viewport projection. At the neutral `1366x768` reference view, representative far traversal selects forward approach `(-7.576081, -1.986423)` and Drawing arrival `(5.267176, -2.104616)`, then reverse approach `(5.280546, -2.015396)` and Entrance arrival `(-7.703568, -2.000136)`. The legacy sampler changes those results with aspect ratio and, for Drawing arrival, with the source position. Canonical Passage anchors will serialize the reference values once; camera/layout code may project them, but must never redefine logical passage identity or placement.

#!/usr/bin/env python3
"""Generate Chateau Guest 5 dialogue with Fish Audio S2-Pro.

Uses the approved Guest 5 arrival example as the fixed reference voice,
generates Guest 5 only, exports 44.1 kHz mono PCM_16 WAVs with a short ending
post-roll, backs up Guest05, and replaces only Guest05 WAVs in Assets.
"""
...
REFERENCE_TEXT = "Good evening. I hope the evening has not started without us."
ASSET_DIR = PROJECT_ROOT / "Assets" / "Audio" / "Voice" / "Guests" / "Guest05"
...
LINES = [
    DialogueLine("CH1_G05_ENTRY", "Good evening. I hope the evening has not started without us.", 75501),
    DialogueLine("CH1_G05_DELAYED", "We were beginning to wonder if anyone was home.", 75502),
    DialogueLine("CH1_G05_COAT_HANDOFF", "Of course. There is nothing in the pockets but travel dust and bad omens.", 75503),
    DialogueLine("CH1_G05_TO_DRAWING_ROOM", "Then let us not keep the Drawing Room from its purpose.", 75504),
    DialogueLine("CH1_G05_AMBIENT_01", "This house is colder than I expected.", 75505),
    DialogueLine("CH1_G05_AMBIENT_02", "The portraits look recently offended.", 75506),
    DialogueLine("CH1_G05_EMPTY_BELL_REACTION", "Let us pretend it was a mistake. Pretending is useful in old houses.", 75507),
    DialogueLine("CH2_G05_PRESPEECH_BARK", "Let him speak. The hour has turned strange.", 75508),
    DialogueLine("CH2_G05_PANIC", "Away from the windows!", 75509),
    DialogueLine("CH2_G05_FOUND_START", "I was not hiding. I was choosing a defensible position.", 75510),
    DialogueLine("CH2_G05_FOUND_REPLY", "Proceed. The more ordinary the ritual, the less power we give the extraordinary.", 75511),
    DialogueLine("CH2_G05_MEAL_PLINK", "Fresh monte genellion de plink. Something substantial. I dislike fleeing on an empty stomach.", 75512),
    DialogueLine("CH2_G05_MEAL_THYME", "Thyme with Lillums. Light enough to run after, should running remain necessary.", 75513),
    DialogueLine("CH2_G05_SMOKE_CIGAR", "A cigar. For victory, or for pretending.", 75514),
...
def require_preflight() -> None:
    ...
    if not REFERENCE_FILE.exists():
        raise FileNotFoundError(f"Missing Guest 5 Fish reference: {REFERENCE_FILE}")
...
def backup_guest05(timestamp: str) -> Path | None:
    if not ASSET_DIR.exists():
        return None
    BACKUP_ROOT.mkdir(parents=True, exist_ok=True)
    backup_dir = BACKUP_ROOT / f"guest05_before_fish_s2_{timestamp}"
...
def write_report(
    ...
    lines = [
        "# Fish Audio S2-Pro Guest 5 Full Dialogue Report",
        "",
        "Generated Guest 5 only. Unity gameplay, subtitles, and playback hooks were not modified.",
        "",
        f"- Started: {started}",
        ...
        f"- Guest05 backup: `{backup_dir}`",
        "",
        "## Counts",
        "",
        f"- Total Guest 5 WAV count: {len(generated)}",
...
def main() -> None:
    ...
    run_dir = GENERATED_ROOT / f"guest05_fish_s2_full_dialogue_{started}"
    ...
    for index, line in enumerate(LINES, start=1):
        print(f"[Guest05 {index:02d}/{len(LINES):02d}] {line.line_id}", flush=True)
        generated.append(
            generate_line(
                line,
                model,

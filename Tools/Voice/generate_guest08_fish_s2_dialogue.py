#!/usr/bin/env python3
"""Generate Chateau Guest 8 dialogue with Fish Audio S2-Pro.

Uses the approved Guest 8 arrival example as the fixed reference voice,
generates Guest 8 only, exports 44.1 kHz mono PCM_16 WAVs with a short ending
post-roll, backs up Guest08, and replaces only Guest08 WAVs in Assets.
"""
...
REFERENCE_TEXT = "Good evening, Butler. I see the house has chosen its most severe face."
ASSET_DIR = PROJECT_ROOT / "Assets" / "Audio" / "Voice" / "Guests" / "Guest08"
...
LINES = [
    DialogueLine("CH1_G08_ENTRY", "Good evening, Butler. I see the house has chosen its most severe face.", 78801),
    DialogueLine("CH1_G08_DELAYED", "At last. A closed door should not feel so pleased with itself.", 78802),
    DialogueLine("CH1_G08_COAT_HANDOFF", "Take it. The night has left fingerprints on the sleeves.", 78803),
    DialogueLine("CH1_G08_TO_DRAWING_ROOM", "Very well. Let us see what sort of welcome the room has rehearsed.", 78804),
    DialogueLine("CH1_G08_AMBIENT_01", "The drawing room should be warmer.", 78805),
    DialogueLine("CH1_G08_AMBIENT_02", "There is a draft here that does not come from any door.", 78806),
    DialogueLine("CH1_G08_EMPTY_BELL_REACTION", "Then we are here. I hope it is satisfied.", 78807),
    DialogueLine("CH2_G08_PRESPEECH_BARK", "Say what you came to say, Butler. The room is holding its breath.", 78808),
    DialogueLine("CH2_G08_PANIC", "No one touch it! No one breathe at it!", 78809),
    DialogueLine("CH2_G08_FOUND_START", "Speak plainly. Is the room safe, or merely occupied?", 78810),
    DialogueLine("CH2_G08_FOUND_REPLY", "You may. I admire a household that continues taking orders after an omen.", 78811),
    DialogueLine("CH2_G08_MEAL_PLINK", "Fresh monte genellion de plink. Boldly named food for a cowardly evening.", 78812),
    DialogueLine("CH2_G08_MEAL_THYME", "Thyme with Lillums. Quiet, green, and unlikely to announce itself on nine legs.", 78813),
...
def require_preflight() -> None:
    ...
    if not REFERENCE_FILE.exists():
        raise FileNotFoundError(f"Missing Guest 8 Fish reference: {REFERENCE_FILE}")
...
def backup_guest08(timestamp: str) -> Path | None:
    if not ASSET_DIR.exists():
        return None
    BACKUP_ROOT.mkdir(parents=True, exist_ok=True)
    backup_dir = BACKUP_ROOT / f"guest08_before_fish_s2_{timestamp}"
...
def write_report(
    ...
    lines = [
        ...
        f"- Guest08 backup: `{backup_dir}`",
        "",
        "## Counts",
        "",
        f"- Total Guest 8 WAV count: {len(generated)}",
...
def main() -> None:
    ...
    run_dir = GENERATED_ROOT / f"guest08_fish_s2_full_dialogue_{started}"
    ...
    for index, line in enumerate(LINES, start=1):
        print(f"[Guest08 {index:02d}/{len(LINES):02d}] {line.line_id}", flush=True)
        generated.append(
            generate_line(
                line,
                model,

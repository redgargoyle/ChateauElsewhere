#!/usr/bin/env python3
"""Generate Chateau Guest 6 dialogue with Fish Audio S2-Pro.

Uses the approved Guest 6 arrival example as the fixed reference voice,
generates Guest 6 only, exports 44.1 kHz mono PCM_16 WAVs with a short ending
post-roll, backs up Guest06, and replaces only Guest06 WAVs in Assets.
"""
...
REFERENCE_FILE = (
    ...
    / "final_48k"
    / "Guest06_Lady_Sabine_Marrow_CH1_G06_ENTRY.wav"
)
REFERENCE_TEXT = "Thank you. I nearly mistook the bell pull for a funeral cord."
ASSET_DIR = PROJECT_ROOT / "Assets" / "Audio" / "Voice" / "Guests" / "Guest06"
...
LINES = [
    DialogueLine("CH1_G06_ENTRY", "Thank you. I nearly mistook the bell pull for a funeral cord.", 76601),
    DialogueLine("CH1_G06_DELAYED", "It is rather cold out there, and colder still when one is expected.", 76602),
    DialogueLine("CH1_G06_COAT_HANDOFF", "Yes, please. I have been wearing half the road since the lower gate.", 76603),
    DialogueLine("CH1_G06_TO_DRAWING_ROOM", "If the fire is real, I may forgive the road.", 76604),
    DialogueLine("CH1_G06_AMBIENT_01", "The host is late, isn't he?", 76605),
    DialogueLine("CH1_G06_AMBIENT_02", "I dislike a clock that seems to be waiting for me personally.", 76606),
    DialogueLine("CH1_G06_EMPTY_BELL_REACTION", "I should very much like that to be the last surprise before dinner.", 76607),
    DialogueLine("CH2_G06_PRESPEECH_BARK", "I dislike a room that listens back.", 76608),
    DialogueLine("CH2_G06_PANIC", "The violin—make it stop!", 76609),
    DialogueLine("CH2_G06_FOUND_START", "Is it gone, or has it merely become quiet?", 76610),
    DialogueLine("CH2_G06_FOUND_REPLY", "Yes. Please. Ask me anything that has only two answers.", 76611),
    DialogueLine("CH2_G06_MEAL_PLINK", "Fresh monte genellion de plink. I refuse to fear a meal with a comic name.", 76612),
    DialogueLine("CH2_G06_MEAL_THYME", "Thyme with Lillums. That sounds almost medicinal. I accept.", 76613),
...
def require_preflight() -> None:
    ...
    if not REFERENCE_FILE.exists():
        raise FileNotFoundError(f"Missing Guest 6 Fish reference: {REFERENCE_FILE}")
...
def backup_guest06(timestamp: str) -> Path | None:
    if not ASSET_DIR.exists():
        return None
    BACKUP_ROOT.mkdir(parents=True, exist_ok=True)
    backup_dir = BACKUP_ROOT / f"guest06_before_fish_s2_{timestamp}"
...
def write_report(
    ...
    lines = [
        "# Fish Audio S2-Pro Guest 6 Full Dialogue Report",
        "",
        "Generated Guest 6 only. Unity gameplay, subtitles, and playback hooks were not modified.",
        "",
        f"- Started: {started}",
        ...
        f"- Guest06 backup: `{backup_dir}`",
        "",
        "## Counts",
        "",
        f"- Total Guest 6 WAV count: {len(generated)}",
...
def main() -> None:
    ...
    run_dir = GENERATED_ROOT / f"guest06_fish_s2_full_dialogue_{started}"
    ...
    for index, line in enumerate(LINES, start=1):
        print(f"[Guest06 {index:02d}/{len(LINES):02d}] {line.line_id}", flush=True)
        generated.append(
            generate_line(
                line,
                model,

#!/usr/bin/env python3
"""Generate Chateau Guest 3 dialogue with Fish Audio S2-Pro.

Uses the approved Guest 3 arrival example as the fixed reference voice,
generates Guest 3 only, exports 44.1 kHz mono PCM_16 WAVs with a short ending
post-roll, backs up Guest03, and replaces only Guest03 WAVs in Assets.
"""
...
REFERENCE_TEXT = "Lovely to see you, dear Butler. Tell me, are we late, early, or merely dramatic?"
ASSET_DIR = PROJECT_ROOT / "Assets" / "Audio" / "Voice" / "Guests" / "Guest03"
...
LINES = [
    DialogueLine("CH1_G03_ENTRY", "Lovely to see you, dear Butler. Tell me, are we late, early, or merely dramatic?", 73301),
    DialogueLine("CH1_G03_DELAYED", "We have been waiting at the door for some time.", 73302),
    DialogueLine("CH1_G03_COAT_HANDOFF", "With gratitude. Do hang it somewhere it can be admired.", 73303),
    DialogueLine("CH1_G03_TO_DRAWING_ROOM", "Prepared? How promising. I adore a room that knows guests are coming.", 73304),
    DialogueLine("CH1_G03_AMBIENT_01", "Did you hear something upstairs?", 73305),
    DialogueLine("CH1_G03_AMBIENT_02", "If the house is settling, it is doing so with theatrical timing.", 73306),
    DialogueLine("CH1_G03_EMPTY_BELL_REACTION", "A phantom caller? How rude to arrive without a coat.", 73307),
    DialogueLine("CH2_G03_PRESPEECH_BARK", "This is deliciously awkward. I approve, with reservations.", 73308),
    DialogueLine("CH2_G03_PANIC", "That is not a dog. Someone tell me that is not a dog.", 73309),
    DialogueLine("CH2_G03_FOUND_START", "If this is a party game, I withdraw my admiration.", 73310),
    DialogueLine("CH2_G03_FOUND_REPLY", "Splendid. Nothing steadies the soul like being menued after a monster.", 73311),
    DialogueLine("CH2_G03_MEAL_PLINK", "Fresh monte genellion de plink. It sounds impossible, and I am in an impossible mood.", 73312),
    DialogueLine("CH2_G03_MEAL_THYME", "Thyme with Lillums. Pretty, mysterious, and likely to stain. I accept.", 73313),
    DialogueLine("CH2_G03_SMOKE_CIGAR", "A cigar. I intend to look magnificent while recovering.", 73314),
...
def require_preflight() -> None:
    ...
    if not REFERENCE_FILE.exists():
        raise FileNotFoundError(f"Missing Guest 3 Fish reference: {REFERENCE_FILE}")
...
def backup_guest03(timestamp: str) -> Path | None:
    if not ASSET_DIR.exists():
        return None
    BACKUP_ROOT.mkdir(parents=True, exist_ok=True)
    backup_dir = BACKUP_ROOT / f"guest03_before_fish_s2_{timestamp}"
...
def write_report(
    ...
    lines = [
        ...
        f"- Guest03 backup: `{backup_dir}`",
        "",
        "## Counts",
        "",
        f"- Total Guest 3 WAV count: {len(generated)}",
...
def main() -> None:
    ...
    run_dir = GENERATED_ROOT / f"guest03_fish_s2_full_dialogue_{started}"
    ...
    for index, line in enumerate(LINES, start=1):
        print(f"[Guest03 {index:02d}/{len(LINES):02d}] {line.line_id}", flush=True)
        generated.append(
            generate_line(
                line,
                model,

#!/usr/bin/env python3
"""Generate Chateau Guest 2 dialogue with Fish Audio S2-Pro.

Uses the approved Guest 2 arrival example as the fixed reference voice,
generates Guest 2 only, exports 44.1 kHz mono PCM_16 WAVs with a short ending
post-roll, backs up Guest02, and replaces only Guest02 WAVs in Assets.
"""
...
REFERENCE_TEXT = "Thank you. The drive was longer in the dark than I care to admit."
ASSET_DIR = PROJECT_ROOT / "Assets" / "Audio" / "Voice" / "Guests" / "Guest02"
...
LINES = [
    DialogueLine("CH1_G02_ENTRY", "Thank you. The drive was longer in the dark than I care to admit.", 72201),
    DialogueLine("CH1_G02_DELAYED", "It is rather cold out there.", 72202),
    DialogueLine("CH1_G02_COAT_HANDOFF", "Thank you. The damp seems to cling to everything tonight.", 72203),
    DialogueLine("CH1_G02_TO_DRAWING_ROOM", "The Drawing Room sounds heavenly. I would settle for any room with a pulse of warmth.", 72204),
    DialogueLine("CH1_G02_AMBIENT_01", "The host is late, isn't he?", 72205),
    DialogueLine("CH1_G02_AMBIENT_02", "I keep thinking someone is standing just behind the curtains.", 72206),
    DialogueLine("CH1_G02_EMPTY_BELL_REACTION", "Please do not say the wind. The wind has better manners.", 72207),
    DialogueLine("CH2_G02_PRESPEECH_BARK", "Are we all meant to be waiting like this?", 72208),
    DialogueLine("CH2_G02_PANIC", "It has too many legs!", 72209),
    DialogueLine("CH2_G02_FOUND_START", "Please tell me you are real before you come any closer.", 72210),
    DialogueLine("CH2_G02_FOUND_REPLY", "At seven? After that thing? Yes. Yes, ordinary questions may save us.", 72211),
    DialogueLine("CH2_G02_MEAL_PLINK", "The fresh monte genellion de plink. I cannot explain why, but the longer name feels safer.", 72212),
    DialogueLine("CH2_G02_MEAL_THYME", "Thyme with Lillums, please. Something gentle. Something with leaves.", 72213),
...
def require_preflight() -> None:
    ...
    if not REFERENCE_FILE.exists():
        raise FileNotFoundError(f"Missing Guest 2 Fish reference: {REFERENCE_FILE}")
...
def backup_guest02(timestamp: str) -> Path | None:
    if not ASSET_DIR.exists():
        return None
    BACKUP_ROOT.mkdir(parents=True, exist_ok=True)
    backup_dir = BACKUP_ROOT / f"guest02_before_fish_s2_{timestamp}"
...
def write_report(
    ...
    lines = [
        "# Fish Audio S2-Pro Guest 2 Full Dialogue Report",
        "",
        "Generated Guest 2 only. Unity gameplay, subtitles, and playback hooks were not modified.",
        "",
        f"- Started: {started}",
        ...
        f"- Guest02 backup: `{backup_dir}`",
        "",
        "## Counts",
        "",
        f"- Total Guest 2 WAV count: {len(generated)}",
...
def main() -> None:
    ...
    run_dir = GENERATED_ROOT / f"guest02_fish_s2_full_dialogue_{started}"
    ...
    for index, line in enumerate(LINES, start=1):
        print(f"[Guest02 {index:02d}/{len(LINES):02d}] {line.line_id}", flush=True)
        generated.append(
            generate_line(
                line,
                model,

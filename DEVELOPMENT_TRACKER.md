...
- Added five selectable ButlerClassic idle variant controllers for aesthetic testing: two mostly-still natural idles and three action idles for pocket watch, smoke, and lapel adjust. Each variant has four directional idle clips and reuses the ButlerClassic walk cycles.
- Replaced the first idle-variant placeholders with an eight-frame generated pipeline. `tools/character_animation/build_butler_classic_idle_variants.py` now regenerates planted full-body frames with arm/hand/prop motion, and Unity has `Dreadforge > Characters > Rebuild ButlerClassic Idle Variant Assets` to rebuild editable clips/controllers from those PNGs.
...
- With each ButlerClassic idle variant selected, stop after walking in all four directions and confirm he holds the last facing direction while playing that variant's eight-frame idle motion. For action idles, confirm the arm/hand/prop reads as part of the pose rather than a floating object.
...
- Pick the strongest ButlerClassic idle variant in Play mode, then tune the matching pose recipe in `tools/character_animation/build_butler_classic_idle_variants.py` instead of manually repainting every frame.

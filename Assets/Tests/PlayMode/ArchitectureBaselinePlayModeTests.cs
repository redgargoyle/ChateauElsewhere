...
public sealed class ArchitectureBaselinePlayModeTests
{
    ...
    public IEnumerator GrandEntranceRearViewRoundTripUsesCanonicalDestinationRegions()
    {
        ...
        Assert.That(FindSceneComponents("Chateau.World.Rooms.RoomView"), Has.Count.EqualTo(19));
        Assert.That(FindSceneComponents("Chateau.World.Rooms.Passages.Passage"), Has.Count.EqualTo(38));
        ...
        object[] registeredDefinitions = ((IEnumerable)GetProperty<object>(database, "Definitions"))
            .Cast<object>()
            .ToArray();
        Assert.That(registeredDefinitions, Has.Length.EqualTo(57));
        ...
        Debug.Log(
            ...
            "roomViews=19 passages=38 callers=38/7 stages=authored-anchors " +
    ...
    public IEnumerator ServiceCorridorSideStairMudroomFarRoundTripUsesCanonicalReciprocalRegions()
    {
        ...
        Assert.That(registered, Has.Length.EqualTo(57));
        ...
        Debug.Log($"[Slice22Group13PlayMode] sideLocal={Format(sideLanding)} " +
            ...
            $"reverseStop={Format(approachStops[1])} roomViews=19 passages=38 callers=38/7 " +
    ...
    public IEnumerator UpperSittingHallNurseryFarRoundTripUsesCanonicalReciprocalRegions()
    {
        ...
        foreach (string completedTriggerName in new[]
        {
            "DoorTrigger_UpperSittingHall_BlueBedroom",
            "DoorTrigger_BlueBedroom_UpperSittingHall"
        })
        {
            MonoBehaviour completedTrigger = RequireComponentOnGameObject(
                completedTriggerName, "DoorTriggerNavigation");
            Assert.That(GetField<MonoBehaviour>(completedTrigger, "canonicalPassage"), Is.Not.Null);
            Assert.That(GetField<MonoBehaviour>(completedTrigger, "navigationManager"),
                Is.SameAs(navigation));
        }
    ...
    [UnityTest]
    public IEnumerator UpperSittingHallBlueBedroomFarRoundTripUsesCanonicalReciprocalScaledRegions()
    {
        const string UpperRoom = "Upper Sitting Hall";
        const string BlueBedroomRoom = "Blue Bedroom";
        ...
        MonoBehaviour cameraManager = RequireSingleSceneComponent("CameraManager");
        MonoBehaviour forwardTrigger = RequireComponentOnGameObject(
            "DoorTrigger_UpperSittingHall_BlueBedroom", "DoorTriggerNavigation");
        MonoBehaviour reverseTrigger = RequireComponentOnGameObject(
            "DoorTrigger_BlueBedroom_UpperSittingHall", "DoorTriggerNavigation");
        MonoBehaviour forwardPassage = RequireComponentOnGameObject(
            "DoorTrigger_UpperSittingHall_BlueBedroom",
            "Chateau.World.Rooms.Passages.Passage");
        MonoBehaviour reversePassage = RequireComponentOnGameObject(
            ...
            "Chateau.World.Rooms.Passages.Passage");
        MonoBehaviour upperView = RequireRoomView("room.upper-sitting-hall");
        MonoBehaviour blueView = RequireRoomView("room.blue-bedroom");
        ...
        Assert.That(GetField<MonoBehaviour>(reverseTrigger, "canonicalPassage"),
            Is.SameAs(reversePassage));
        Assert.That(forwardTrigger.transform.localScale.x, Is.EqualTo(0.82498f).Within(0.0001f));
        Assert.That(forwardTrigger.transform.localScale.y, Is.EqualTo(1f).Within(0.0001f));
        Assert.That(reverseTrigger.transform.localScale, Is.EqualTo(Vector3.one));
        foreach (MonoBehaviour trigger in new[] { forwardTrigger, reverseTrigger })
        {
            Assert.That(GetField<MonoBehaviour>(trigger, "navigationManager"), Is.SameAs(navigation));
            Assert.That(GetField<Transform>(trigger, "player"), Is.SameAs(player.transform));
        ...
        Assert.That(GetProperty<string>(forwardTrigger, "DestinationRoom"), Is.EqualTo(BlueBedroomRoom));
        Assert.That(GetProperty<string>(reverseTrigger, "SourceRoom"), Is.EqualTo(BlueBedroomRoom));
        Assert.That(GetProperty<string>(reverseTrigger, "DoorName"),
        ...
        object upperDefinition = GetProperty<object>(upperView, "Definition");
        object blueDefinition = GetProperty<object>(blueView, "Definition");
        object forwardDefinition = GetProperty<object>(forwardPassage, "Definition");
        object reverseDefinition = GetProperty<object>(reversePassage, "Definition");
        Assert.That(GetProperty<string>(upperDefinition, "DisplayName"), Is.EqualTo(UpperRoom));
        Assert.That(GetProperty<string>(blueDefinition, "DisplayName"), Is.EqualTo(BlueBedroomRoom));
        Assert.That(GetProperty<object>(forwardDefinition, "Kind").ToString(), Is.EqualTo("Door"));
        Assert.That(GetProperty<object>(reverseDefinition, "Kind").ToString(), Is.EqualTo("Door"));
        Assert.That(GetProperty<string>(forwardDefinition, "PromptText"), Is.EqualTo("Open Door"));
        Assert.That(GetProperty<string>(reverseDefinition, "PromptText"), Is.EqualTo("Open Door"));
        ...
        AssertCanonicalSourceAndDestinationRegionPassage(
            ...
            "UpperSittingHall_BlueBedroom", BlueBedroomRoom, false,
            new Vector2(-715f, -240.000061f),
            new Vector2(-715f, 259.7826538f),
            new Vector2(-532.2076416f, 259.7826538f),
            new Vector2(-532.2076416f, -240.000061f));
        AssertCanonicalSourceAndDestinationRegionPassage(
            reversePassage, blueView, forwardPassage,
            "passage.blue-bedroom.upper-sitting-hall",
            "BlueBedroom_UpperSittingHall", UpperRoom, false,
            new Vector2(334.5011902f, -225.5001984f),
            new Vector2(334.5011902f, 330.0778198f),
            new Vector2(437.8641663f, 330.0778198f),
            new Vector2(437.8641663f, -225.5001984f));
        ...
        foreach (string protectedName in new[]
        {
            "Room_Blue_Bedroom", "blue_room_bed_0", "bluebedroomchair_0",
            "Ch2_Hide_Guest08", "PlayerBlocker_blue_room_bed_0",
            "PlayerBlocker_bluebedroomchair_0", "PatchCandidate_Blue_Bedroom_Curtain_Moon_Flicker"
        })
            Assert.That(RequireSceneGameObject(protectedName), Is.Not.Null);
        ...
        arrivalEvent.AddEventHandler(player, arrived);
        stoppedEvent.AddEventHandler(player, stopped);
        Vector2 blueLanding = Vector2.zero;
        Vector2 upperLanding = Vector2.zero;
        try
        {
            string[] setupNames =
            ...
            InvokeMethod(forwardTrigger, "ActivateDoor");
            Assert.That(GetProperty<bool>(player, "HasDestination"), Is.True);
            yield return WaitForCurrentRoom(navigation, BlueBedroomRoom, 240);
            FreezeRoomLookForEvidence();
            yield return null;
            Assert.That(GetProperty<object>(navigation, "CurrentRoomDefinition"),
                Is.SameAs(blueDefinition));
            Assert.That(GetProperty<bool>(blueView, "IsVisible"), Is.True);
            blueLanding = GetRoomViewLocalPlayerPosition(player, cameraManager,
                "Blue Bedroom landing");
            ...
            upperLanding = GetRoomViewLocalPlayerPosition(player, cameraManager,
                "Upper Sitting Hall landing");
            Assert.That(approachStops, Has.Count.EqualTo(2));
            Assert.That(approachRooms, Is.EqualTo(new[] { UpperRoom, BlueBedroomRoom }));
            Assert.That(movementStops, Is.EqualTo(2));
            AssertFinite(approachStops[0], "Upper Sitting Hall source-region approach stop");
            AssertFinite(approachStops[1], "Blue Bedroom source-region approach stop");
            AssertFinite(blueLanding, "Blue Bedroom destination-region landing");
            AssertFinite(upperLanding, "Upper Sitting Hall destination-region landing");
            AssertFixedRenderingResolution();
        }
        finally
        ...
        Debug.Log($"[Slice22Group18PlayMode] blueLocal={Format(blueLanding)} " +
            $"upperLocal={Format(upperLanding)} forwardStop={Format(approachStops[0])} " +
            $"reverseStop={Format(approachStops[1])} roomViews=19 passages=38 callers=38/7 " +
            "profile=standard-door/standard-door scaled-upper-source protected=upper/blue-content");
    }
}

...
public class NavigationRegressionTests
{
    ...
    public void ServiceCorridorKitchenRoomLocalPassagePairIsExact()
    {
        ...
        Assert.That(Regex.Matches(sceneText, $"guid: {roomViewGuid}").Count, Is.EqualTo(19));
        Assert.That(Regex.Matches(sceneText, $"guid: {passageGuid}").Count, Is.EqualTo(38));
        ...
        Assert.That(Regex.Matches(sceneBehaviours, @"(?m)^  - \{fileID:").Count, Is.EqualTo(66));
    ...
    public void GrandEntranceRearViewRegionPassagePairIsExact()
    {
        ...
        Assert.That(ReadGuid(forwardDefinitionPath + ".meta"), Is.EqualTo(forwardDefinitionGuid));
        Assert.That(ReadGuid(reverseDefinitionPath + ".meta"), Is.EqualTo(reverseDefinitionGuid));
        Assert.That(Regex.Matches(sceneText, @"(?m)^--- !u!").Count, Is.EqualTo(6064));
        Assert.That(Regex.Matches(sceneText, $"guid: {roomViewGuid}").Count, Is.EqualTo(19));
        Assert.That(Regex.Matches(sceneText, $"guid: {passageGuid}").Count, Is.EqualTo(38));
        Assert.That(Regex.Matches(sceneText, $"guid: {doorTriggerGuid}").Count, Is.EqualTo(45));
        Assert.That(Regex.Matches(databaseText,
            @"(?m)^  - \{fileID: 11400000, guid: [0-9a-f]{32}, type: 2\}$").Count, Is.EqualTo(57));
        Assert.That(Regex.Matches(databaseText, $"guid: {forwardDefinitionGuid}").Count, Is.EqualTo(1));
        Assert.That(Regex.Matches(databaseText, $"guid: {reverseDefinitionGuid}").Count, Is.EqualTo(1));
        ...
        Assert.That(Regex.Matches(sceneText, @"(?m)^  arrivalPlacementMode: 1$").Count, Is.EqualTo(18));
        Assert.That(Regex.Matches(sceneText, @"(?m)^  arrivalRegion:$").Count, Is.EqualTo(18));
    }
    ...
    public void GrandEntranceRearBilliardSourceAndDestinationRegionPassagePairIsExact()
    {
        ...
        Assert.That(CountUnityObjectBlocksContaining(
            ...
            "navigationManager: {fileID: 1878886997}"), Is.EqualTo(38));
        Assert.That(CountUnityObjectBlocksContaining(
            sceneText,
            $"guid: {doorTriggerGuid}",
            "navigationManager: {fileID: 0}"), Is.EqualTo(7));
        Assert.That(CountUnityObjectBlocksContaining(
            sceneText,
            $"guid: {doorTriggerGuid}",
            "canonicalPassage: {fileID:"), Is.EqualTo(38));
    ...
    public void ServiceCorridorSideStairMudroomSourceAndDestinationRegionPassagePairIsExact()
    {
        ...
        string group14ReverseOwner = ExtractUnityObjectBlock(scene, "--- !u!1 &1915714842");
        string group14ReverseTrigger = ExtractUnityObjectBlock(scene, "--- !u!114 &1915714844");

        Assert.That(ReadGuid(forwardPath + ".meta"), Is.EqualTo(forwardGuid));
        Assert.That(ReadGuid(reversePath + ".meta"), Is.EqualTo(reverseGuid));
        Assert.That(Regex.Matches(scene, @"(?m)^--- !u!").Count, Is.EqualTo(6064));
        ...
        Assert.That(Regex.Matches(database,
            @"(?m)^  - \{fileID: 11400000, guid: [0-9a-f]{32}, type: 2\}$").Count,
            Is.EqualTo(57));
        Assert.That(Regex.Matches(gameRoot, @"(?m)^  - \{fileID:").Count, Is.EqualTo(74));
        ...
        Assert.That(Regex.Matches(group14Owner, @"(?m)^  - component:").Count, Is.EqualTo(5));
        Assert.That(Regex.Matches(group14ReverseOwner, @"(?m)^  - component:").Count, Is.EqualTo(5));
        Assert.That(group14Owner, Does.Contain("- component: {fileID: 4100000045}"));
        Assert.That(group14ReverseOwner, Does.Contain("- component: {fileID: 4100000044}"));
        Assert.That(group14Trigger, Does.Contain("canonicalPassage: {fileID: 4100000045}"));
        Assert.That(group14ReverseTrigger, Does.Contain("canonicalPassage: {fileID: 4100000044}"));
        Assert.That(group14Trigger, Does.Contain("navigationManager: {fileID: 1878886997}"));
        Assert.That(group14ReverseTrigger, Does.Contain("navigationManager: {fileID: 1878886997}"));
    ...
    [Test]
    public void SideStairMudroomUpperSittingHallMixedSourceAndDestinationRegionPassagePairIsExact()
    {
        const string passageGuid = "518dad8adf634786a103bf4e76aa0881";
        const string roomViewGuid = "ccd2f3bd803e45aa8a1174cc881d6dc0";
        const string doorTriggerGuid = "7e419b0f8f26d4f2d8d03e567fef4c52";
        const string forwardDefinitionGuid = "77a3efbd4a250afa71e5998bb7118532";
        const string reverseDefinitionGuid = "d5c370a383ecaf7257b8f796d0164e72";
        const string sideRoomGuid = "c5153d08442348c49bf2c92c935d8035";
        const string upperRoomGuid = "2ce92b6f43ff440e95570359e18cce0f";
        const string forwardPath =
        ...
        string database = File.ReadAllText("Assets/_Chateau/Data/GameDatabase.asset");
        string gameRoot = ExtractUnityObjectBlock(scene, "--- !u!114 &1878886998");
        string sideRoot = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000035");
        string sideView = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000040");
        string upperRoot = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000050");
        string upperView = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000043");
        string forwardOwner = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000185");
        string forwardTransform = ExtractUnityObjectBlock(scene, "--- !u!224 &2300000186");
        string forwardTrigger = ExtractUnityObjectBlock(scene, "--- !u!114 &2300000189");
        string forwardPassage = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000045");
        string reverseOwner = ExtractUnityObjectBlock(scene, "--- !u!1 &1915714842");
        string reverseTransform = ExtractUnityObjectBlock(scene, "--- !u!224 &1915714843");
        string reverseTrigger = ExtractUnityObjectBlock(scene, "--- !u!114 &1915714844");
        string reversePassage = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000044");
        string forwardDefinition = File.ReadAllText(forwardPath);
        string reverseDefinition = File.ReadAllText(reversePath);
        string sideRoomDefinition = File.ReadAllText(
            "Assets/_Chateau/Data/World/Rooms/Room_SideStairMudroom.asset");
        string upperRoomDefinition = File.ReadAllText(
            "Assets/_Chateau/Data/World/Rooms/Room_UpperSittingHall.asset");

        Assert.That(ReadGuid(forwardPath + ".meta"), Is.EqualTo(forwardDefinitionGuid));
        Assert.That(ReadGuid(reversePath + ".meta"), Is.EqualTo(reverseDefinitionGuid));
        Assert.That(Regex.Matches(scene, @"(?m)^--- !u!").Count, Is.EqualTo(6064));
        Assert.That(Regex.Matches(scene, $"guid: {roomViewGuid}").Count, Is.EqualTo(19));
        Assert.That(Regex.Matches(scene, $"guid: {passageGuid}").Count, Is.EqualTo(38));
        Assert.That(Regex.Matches(scene, $"guid: {doorTriggerGuid}").Count, Is.EqualTo(45));
        Assert.That(Regex.Matches(database,
            @"(?m)^  - \{fileID: 11400000, guid: [0-9a-f]{32}, type: 2\}$").Count,
            Is.EqualTo(57));
        Assert.That(Regex.Matches(gameRoot, @"(?m)^  - \{fileID:").Count, Is.EqualTo(74));
        foreach (string id in new[] { "4100000043", "4100000044", "4100000045" })
        {
            Assert.That(Regex.Matches(gameRoot, $@"(?m)^  - \{{fileID: {id}\}}$").Count,
                Is.EqualTo(1));
        }
        Assert.That(Regex.Matches(scene,
            "doorOpenSoundCatalog: \\{fileID: 11400000, guid: " +
            "9a77542e25184fbc945d6a79f77007e7, type: 2\\}").Count, Is.EqualTo(37));
        Assert.That(Regex.Matches(scene,
            "stairwaySoundCatalog: \\{fileID: 11400000, guid: " +
            "ca795d9a1ee74c14aa22f9f38a14f8ea, type: 2\\}").Count, Is.EqualTo(1));

        Assert.That(Regex.Matches(sideRoot, @"(?m)^  - component:").Count, Is.EqualTo(3));
        Assert.That(sideRoot, Does.Contain("- component: {fileID: 4100000040}"));
        Assert.That(sideView, Does.Contain(
            $"definition: {{fileID: 11400000, guid: {sideRoomGuid}, type: 2}}"));
        Assert.That(sideView, Does.Contain("legacyContentGroup: {fileID: 2300000037}"));
        Assert.That(Regex.Matches(upperRoot, @"(?m)^  - component:").Count, Is.EqualTo(3));
        Assert.That(upperRoot, Does.Contain("- component: {fileID: 4100000043}"));
        Assert.That(upperView, Does.Contain(
            $"definition: {{fileID: 11400000, guid: {upperRoomGuid}, type: 2}}"));
        Assert.That(upperView, Does.Contain("legacyContentGroup: {fileID: 2300000052}"));

        Assert.That(Regex.Matches(forwardOwner, @"(?m)^  - component:").Count, Is.EqualTo(5));
        Assert.That(Regex.Matches(reverseOwner, @"(?m)^  - component:").Count, Is.EqualTo(5));
        Assert.That(forwardOwner, Does.Contain(
            "m_Name: StairwayTrigger_SideStairMudroom_UpperSittingHall"));
        Assert.That(reverseOwner, Does.Contain(
            "m_Name: DoorTrigger_UpperSittingHall_SideStairMudroom"));
        Assert.That(forwardOwner, Does.Contain("- component: {fileID: 4100000045}"));
        Assert.That(reverseOwner, Does.Contain("- component: {fileID: 4100000044}"));
        Assert.That(forwardTransform, Does.Contain(
            "m_AnchoredPosition: {x: 362.54022, y: 17.147995}"));
        Assert.That(forwardTransform, Does.Contain("m_SizeDelta: {x: 218.7387, y: 438.8239}"));
        Assert.That(reverseTransform, Does.Contain("m_AnchoredPosition: {x: 99.9335, y: 48.6412}"));
        Assert.That(reverseTransform, Does.Contain("m_SizeDelta: {x: 58.133, y: 162.7177}"));

        foreach (string trigger in new[] { forwardTrigger, reverseTrigger })
        {
            Assert.That(trigger, Does.Contain("triggerKind: 0"));
            Assert.That(trigger, Does.Contain("stairwayDirection: 0"));
            Assert.That(trigger, Does.Contain("navigationManager: {fileID: 1878886997}"));
            Assert.That(trigger, Does.Contain("doorOpenAudioSource: {fileID: 2201000013}"));
            Assert.That(trigger, Does.Contain("player: {fileID: 81962843}"));
            Assert.That(trigger, Does.Contain("requirePlayerProximity: 1"));
        ...
        Assert.That(forwardTrigger, Does.Contain("sourceRoom: Side Stair Mudroom"));
        Assert.That(forwardTrigger, Does.Contain(
            "doorName: SideStairMudroom_Stairway_UpperSittingHall"));
        Assert.That(forwardTrigger, Does.Contain("destinationRoom: Upper Sitting Hall"));
        Assert.That(forwardTrigger, Does.Contain("canonicalPassage: {fileID: 4100000045}"));
        Assert.That(forwardTrigger, Does.Contain("doorOpenSoundCatalog: {fileID: 0}"));
        Assert.That(forwardTrigger, Does.Contain(
            "stairwaySoundCatalog: {fileID: 11400000, guid: " +
            "ca795d9a1ee74c14aa22f9f38a14f8ea, type: 2}"));
        Assert.That(reverseTrigger, Does.Contain("sourceRoom: Upper Sitting Hall"));
        Assert.That(reverseTrigger, Does.Contain("doorName: UpperSittingHall_SideStairMudroom"));
        Assert.That(reverseTrigger, Does.Contain("destinationRoom: Side Stair Mudroom"));
        Assert.That(reverseTrigger, Does.Contain("canonicalPassage: {fileID: 4100000044}"));
        Assert.That(reverseTrigger, Does.Contain(
            "doorOpenSoundCatalog: {fileID: 11400000, guid: " +
            "9a77542e25184fbc945d6a79f77007e7, type: 2}"));
        Assert.That(reverseTrigger, Does.Contain("stairwaySoundCatalog: {fileID: 0}"));

        AssertSourceAndDestinationRegionPassageDocument(
            forwardPassage, forwardDefinitionGuid, "4100000040", "4100000044",
            "{x: 70.8670044, y: -32.7176476}", "{x: 70.8670044, y: 130.0000458}",
            "{x: 129, y: 130.0000458}", "{x: 129, y: -32.7176476}");
        AssertSourceAndDestinationRegionPassageDocument(
            reversePassage, reverseDefinitionGuid, "4100000043", "4100000045",
            "{x: 253.170868, y: -202.263962}", "{x: 253.170868, y: 236.559952}",
            "{x: 471.909576, y: 236.559952}", "{x: 471.909576, y: -202.263962}");

        Assert.That(forwardDefinition, Does.Contain(
            "stableId: passage.side-stair-mudroom.upper-sitting-hall"));
        Assert.That(forwardDefinition, Does.Contain(
            $"sourceRoom: {{fileID: 11400000, guid: {sideRoomGuid}, type: 2}}"));
        Assert.That(forwardDefinition, Does.Contain(
            $"destinationRoom: {{fileID: 11400000, guid: {upperRoomGuid}, type: 2}}"));
        Assert.That(forwardDefinition, Does.Contain(
            $"reverse: {{fileID: 11400000, guid: {reverseDefinitionGuid}, type: 2}}"));
        Assert.That(forwardDefinition, Does.Contain("kind: 1"));
        Assert.That(forwardDefinition, Does.Contain("promptText: Use Stairway"));
        Assert.That(forwardDefinition, Does.Contain(
            "legacyDoorId: SideStairMudroom_Stairway_UpperSittingHall"));
        Assert.That(forwardDefinition, Does.Not.Contain("compatibilityDestinationRoomName:"));
        Assert.That(reverseDefinition, Does.Contain(
            "stableId: passage.upper-sitting-hall.side-stair-mudroom"));
        Assert.That(reverseDefinition, Does.Contain(
            $"sourceRoom: {{fileID: 11400000, guid: {upperRoomGuid}, type: 2}}"));
        Assert.That(reverseDefinition, Does.Contain(
            $"destinationRoom: {{fileID: 11400000, guid: {sideRoomGuid}, type: 2}}"));
        Assert.That(reverseDefinition, Does.Contain(
            $"reverse: {{fileID: 11400000, guid: {forwardDefinitionGuid}, type: 2}}"));
        Assert.That(reverseDefinition, Does.Contain("kind: 0"));
        Assert.That(reverseDefinition, Does.Contain("promptText: Open Door"));
        Assert.That(reverseDefinition, Does.Contain(
            "legacyDoorId: UpperSittingHall_SideStairMudroom"));
        Assert.That(reverseDefinition, Does.Contain(
            "compatibilityDestinationRoomName: Side Stair Mudroom"));

        Assert.That(sideRoomDefinition, Does.Contain("displayName: Side Stair & Mudroom"));
        Assert.That(sideRoomDefinition, Does.Contain(
            "legacyNames:\n  - Side Stair & Mudroom\n  - Side Stair Mudroom"));
        Assert.That(upperRoomDefinition, Does.Contain("displayName: Upper Sitting Hall"));
        Assert.That(upperRoomDefinition, Does.Contain("legacyNames:\n  - Upper Sitting Hall"));
        string doors = File.ReadAllText("Assets/Resources/Navigation/doors.txt");
        Assert.That(doors, Does.Not.Contain("SideStairMudroom_Stairway_UpperSittingHall"));
        Assert.That(doors, Does.Not.Contain("UpperSittingHall_SideStairMudroom"));
        Assert.That(doors, Does.Contain("SideStairMudroom_UpperGallery: Upper Gallery"));
        Assert.That(doors, Does.Contain("UpperGallery_SideStairMudroom: Side Stair & Mudroom"));
    }

    [Test]
    public void UpperSittingHallUpperGallerySourceAndDestinationRegionPassagePairIsExact()
    {
        const string passageGuid = "518dad8adf634786a103bf4e76aa0881";
        const string roomViewGuid = "ccd2f3bd803e45aa8a1174cc881d6dc0";
        const string doorTriggerGuid = "7e419b0f8f26d4f2d8d03e567fef4c52";
        const string forwardDefinitionGuid = "54dc2ad728e40e59f280711e65b3474d";
        const string reverseDefinitionGuid = "9968ef27c1a4121f14100399fe85ab32";
        const string upperRoomGuid = "2ce92b6f43ff440e95570359e18cce0f";
        const string galleryRoomGuid = "d89c065ce9d049d4b6f5ba012b16761d";
        const string forwardPath =
        ...
        string scene = File.ReadAllText(GameplayScenePath);
        string database = File.ReadAllText("Assets/_Chateau/Data/GameDatabase.asset");
        string inventory = File.ReadAllText("Docs/Architecture/RemainingRouteInventory.csv");
        string gameRoot = ExtractUnityObjectBlock(scene, "--- !u!114 &1878886998");
        string upperRoot = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000050");
        string upperView = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000043");
        string galleryRoot = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000040");
        string galleryView = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000046");
        string forwardOwner = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000215");
        string forwardTrigger = ExtractUnityObjectBlock(scene, "--- !u!114 &2300000219");
        string forwardPassage = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000048");
        string reverseOwner = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000205");
        string reverseTrigger = ExtractUnityObjectBlock(scene, "--- !u!114 &2300000209");
        string reversePassage = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000047");
        string forwardDefinition = File.ReadAllText(forwardPath);
        string reverseDefinition = File.ReadAllText(reversePath);
        ...
        Assert.That(Regex.Matches(database,
            ...
            Is.EqualTo(57));
        Assert.That(Regex.Matches(gameRoot, @"(?m)^  - \{fileID:").Count, Is.EqualTo(74));
        foreach (string id in new[] { "4100000046", "4100000047", "4100000048" })
        {
            Assert.That(Regex.Matches(gameRoot, $@"(?m)^  - \{{fileID: {id}\}}$").Count,
                Is.EqualTo(1));
        }
        ...
        Assert.That(upperView, Does.Contain(
            $"definition: {{fileID: 11400000, guid: {upperRoomGuid}, type: 2}}"));
        Assert.That(Regex.Matches(galleryRoot, @"(?m)^  - component:").Count, Is.EqualTo(3));
        Assert.That(galleryRoot, Does.Contain("- component: {fileID: 4100000046}"));
        Assert.That(galleryView, Does.Contain(
            $"definition: {{fileID: 11400000, guid: {galleryRoomGuid}, type: 2}}"));
        Assert.That(galleryView, Does.Contain("legacyContentGroup: {fileID: 2300000042}"));

        foreach (string owner in new[] { forwardOwner, reverseOwner })
            Assert.That(Regex.Matches(owner, @"(?m)^  - component:").Count, Is.EqualTo(5));
        Assert.That(forwardOwner, Does.Contain("- component: {fileID: 4100000048}"));
        Assert.That(reverseOwner, Does.Contain("- component: {fileID: 4100000047}"));
        foreach (string trigger in new[] { forwardTrigger, reverseTrigger })
        {
            Assert.That(trigger, Does.Contain("navigationManager: {fileID: 1878886997}"));
            Assert.That(trigger, Does.Contain("doorOpenAudioSource: {fileID: 2201000013}"));
        ...
        Assert.That(forwardTrigger, Does.Contain("canonicalPassage: {fileID: 4100000048}"));
        Assert.That(reverseTrigger, Does.Contain("canonicalPassage: {fileID: 4100000047}"));

        AssertSourceAndDestinationRegionPassageDocument(
            forwardPassage, forwardDefinitionGuid, "4100000043", "4100000047",
            "{x: 587.2802124, y: -69.99983978}", "{x: 587.2802124, y: 225.1838379}",
            "{x: 710.7197876, y: 225.1838379}", "{x: 710.7197876, y: -69.99983978}");
        AssertSourceAndDestinationRegionPassageDocument(
            reversePassage, reverseDefinitionGuid, "4100000046", "4100000048",
            "{x: -174.1340942, y: -42.82170105}", "{x: -174.1340942, y: 207.995697}",
            "{x: -135.8659058, y: 207.995697}", "{x: -135.8659058, y: -42.82170105}");

        Assert.That(forwardDefinition, Does.Contain(
            "stableId: passage.upper-sitting-hall.upper-gallery"));
        Assert.That(forwardDefinition, Does.Contain(
            $"sourceRoom: {{fileID: 11400000, guid: {upperRoomGuid}, type: 2}}"));
        Assert.That(forwardDefinition, Does.Contain(
            $"destinationRoom: {{fileID: 11400000, guid: {galleryRoomGuid}, type: 2}}"));
        Assert.That(forwardDefinition, Does.Contain(
            $"reverse: {{fileID: 11400000, guid: {reverseDefinitionGuid}, type: 2}}"));
        Assert.That(forwardDefinition, Does.Contain("legacyDoorId: UpperSittingHall_UpperGallery"));
        Assert.That(reverseDefinition, Does.Contain(
            "stableId: passage.upper-gallery.upper-sitting-hall"));
        Assert.That(reverseDefinition, Does.Contain(
            $"sourceRoom: {{fileID: 11400000, guid: {galleryRoomGuid}, type: 2}}"));
        Assert.That(reverseDefinition, Does.Contain(
            $"destinationRoom: {{fileID: 11400000, guid: {upperRoomGuid}, type: 2}}"));
        Assert.That(reverseDefinition, Does.Contain(
            $"reverse: {{fileID: 11400000, guid: {forwardDefinitionGuid}, type: 2}}"));
        Assert.That(reverseDefinition, Does.Contain("legacyDoorId: UpperGallery_UpperSittingHall"));
        foreach (string definition in new[] { forwardDefinition, reverseDefinition })
        {
            Assert.That(definition, Does.Contain("kind: 0"));
            Assert.That(definition, Does.Contain("promptText: Open Door"));
            Assert.That(definition, Does.Not.Contain("compatibilityDestinationRoomName:"));
        }

        Assert.That(inventory, Does.Contain(
            "15,complete,UpperSitting-UpperGallery,standard-door,2300000209," +
            "DoorTrigger_UpperGallery_UpperSittingHall"));
        Assert.That(inventory, Does.Contain(
            "15,complete,UpperSitting-UpperGallery,standard-door,2300000219," +
            "DoorTrigger_UpperSittingHall_UpperGallery"));
        Assert.That(inventory, Does.Not.Contain("15,queued,UpperSitting-UpperGallery"));

        foreach (string neighborId in new[]
        {
            "1084796962", "1498249206", "106972347",
            "2300000069", "2300000194"
        })
        {
            string neighbor = ExtractUnityObjectBlock(scene, $"--- !u!114 &{neighborId}");
            Assert.That(neighbor, Does.Contain("navigationManager: {fileID: 0}"));
            Assert.That(neighbor, Does.Contain("doorOpenAudioSource: {fileID: 0}"));
            Assert.That(neighbor, Does.Contain("player: {fileID: 0}"));
            Assert.That(neighbor, Does.Not.Contain("canonicalPassage:"));
        }
        Assert.That(Regex.Matches(
            ExtractUnityObjectBlock(scene, "--- !u!60 &580370978"),
            @"(?m)^      - \{|^    - - \{").Count, Is.EqualTo(12));
        Assert.That(Regex.Matches(
            ExtractUnityObjectBlock(scene, "--- !u!60 &580370981"),
            @"(?m)^      - \{|^    - - \{").Count, Is.EqualTo(16));
        foreach (string protectedName in new[]
        {
            "upper_gallery_railing", "upper_gallery_left_plant_0",
            "upper_gallery_right_plant_0", "Ch2_Hide_Guest07",
            "upperhalldrawer_0", "upperhalldrawerright_0"
        })
            Assert.That(scene, Does.Contain($"m_Name: {protectedName}"));
    }

    [Test]
    public void UpperGalleryMasterBedroomSuiteScaledSourceAndDestinationRegionPassagePairIsExact()
    {
        const string passageGuid = "518dad8adf634786a103bf4e76aa0881";
        const string roomViewGuid = "ccd2f3bd803e45aa8a1174cc881d6dc0";
        const string doorTriggerGuid = "7e419b0f8f26d4f2d8d03e567fef4c52";
        const string forwardDefinitionGuid = "1dff2bb730264e9c81ecb2e35985f5a6";
        const string reverseDefinitionGuid = "f1fd515e16c44606af75bf593f67bc07";
        const string galleryRoomGuid = "d89c065ce9d049d4b6f5ba012b16761d";
        const string masterRoomGuid = "5acd179b4f7543438b4e55e5c0e5f6ca";
        const string forwardPath =
        ...
        string gameRoot = ExtractUnityObjectBlock(scene, "--- !u!114 &1878886998");
        string galleryRoot = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000040");
        string masterRoot = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000045");
        string masterView = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000049");
        string forwardOwner = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000200");
        string forwardTrigger = ExtractUnityObjectBlock(scene, "--- !u!114 &2300000204");
        string forwardPassage = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000050");
        string reverseOwner = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000210");
        string reverseTrigger = ExtractUnityObjectBlock(scene, "--- !u!114 &2300000214");
        string reversePassage = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000051");
        string forwardDefinition = File.ReadAllText(forwardPath);
        string reverseDefinition = File.ReadAllText(reversePath);
        ...
        Assert.That(Regex.Matches(database,
            ...
            Is.EqualTo(57));
        Assert.That(Regex.Matches(gameRoot, @"(?m)^  - \{fileID:").Count, Is.EqualTo(74));
        Assert.That(Regex.Matches(scene, @"(?m)^  approachPlacementMode: 1$").Count,
            Is.EqualTo(16));
        Assert.That(Regex.Matches(scene, @"(?m)^  arrivalPlacementMode: 1$").Count,
            Is.EqualTo(18));
        Assert.That(Regex.Matches(scene, @"(?m)^    coordinateSpace: 1$").Count,
            Is.EqualTo(10));
        Assert.That(CountUnityObjectBlocksContaining(
            scene, $"guid: {doorTriggerGuid}", "navigationManager: {fileID: 1878886997}"),
            Is.EqualTo(38));
        Assert.That(CountUnityObjectBlocksContaining(
            scene, $"guid: {doorTriggerGuid}", "navigationManager: {fileID: 0}"),
            Is.EqualTo(7));
        Assert.That(Regex.Matches(inventory, @"(?m)^\d+,complete,").Count, Is.EqualTo(38));
        Assert.That(Regex.Matches(inventory, @"(?m)^\d+,queued,").Count, Is.EqualTo(2));
        Assert.That(Regex.Matches(inventory, @"(?m)^\d+,blocked-one-way,").Count, Is.EqualTo(2));
        Assert.That(Regex.Matches(inventory, @"(?m)^\d+,blocked-parallel,").Count, Is.EqualTo(3));
        foreach (string id in new[] { "4100000049", "4100000050", "4100000051" })
            Assert.That(Regex.Matches(gameRoot, $@"(?m)^  - \{{fileID: {id}\}}$").Count, Is.EqualTo(1));

        Assert.That(Regex.Matches(galleryRoot, @"(?m)^  - component:").Count, Is.EqualTo(3));
        Assert.That(galleryRoot, Does.Contain("- component: {fileID: 4100000046}"));
        Assert.That(Regex.Matches(masterRoot, @"(?m)^  - component:").Count, Is.EqualTo(3));
        Assert.That(masterRoot, Does.Contain("- component: {fileID: 4100000049}"));
        Assert.That(masterView, Does.Contain(
            $"definition: {{fileID: 11400000, guid: {masterRoomGuid}, type: 2}}"));
        Assert.That(masterView, Does.Contain("legacyContentGroup: {fileID: 2300000047}"));
        foreach (string owner in new[] { forwardOwner, reverseOwner })
            Assert.That(Regex.Matches(owner, @"(?m)^  - component:").Count, Is.EqualTo(5));
        Assert.That(forwardOwner, Does.Contain("- component: {fileID: 4100000050}"));
        Assert.That(reverseOwner, Does.Contain("- component: {fileID: 4100000051}"));
        foreach (string trigger in new[] { forwardTrigger, reverseTrigger })
        {
            Assert.That(trigger, Does.Contain("navigationManager: {fileID: 1878886997}"));
            Assert.That(trigger, Does.Contain("doorOpenAudioSource: {fileID: 2201000013}"));
        ...
        Assert.That(forwardTrigger, Does.Contain("canonicalPassage: {fileID: 4100000050}"));
        Assert.That(reverseTrigger, Does.Contain("canonicalPassage: {fileID: 4100000051}"));
        AssertSourceAndDestinationRegionPassageDocument(
            forwardPassage, forwardDefinitionGuid, "4100000046", "4100000051",
            "{x: -770.5059814, y: -210.125}", "{x: -770.5059814, y: 264.125}",
            "{x: -599.4940186, y: 264.125}", "{x: -599.4940186, y: -210.125}");
        AssertSourceAndDestinationRegionPassageDocument(
            reversePassage, reverseDefinitionGuid, "4100000049", "4100000050",
            "{x: -722.8327637, y: -70.00001526}", "{x: -722.8327637, y: 195.9999847}",
            "{x: -604.0205078, y: 195.9999847}", "{x: -604.0205078, y: -70.00001526}");

        Assert.That(forwardDefinition, Does.Contain("stableId: passage.upper-gallery.master-bedroom-suite"));
        Assert.That(forwardDefinition, Does.Contain(
            $"sourceRoom: {{fileID: 11400000, guid: {galleryRoomGuid}, type: 2}}"));
        Assert.That(forwardDefinition, Does.Contain(
            $"destinationRoom: {{fileID: 11400000, guid: {masterRoomGuid}, type: 2}}"));
        Assert.That(forwardDefinition, Does.Contain(
            $"reverse: {{fileID: 11400000, guid: {reverseDefinitionGuid}, type: 2}}"));
        Assert.That(reverseDefinition, Does.Contain("stableId: passage.master-bedroom-suite.upper-gallery"));
        Assert.That(reverseDefinition, Does.Contain(
            $"sourceRoom: {{fileID: 11400000, guid: {masterRoomGuid}, type: 2}}"));
        Assert.That(reverseDefinition, Does.Contain(
            $"destinationRoom: {{fileID: 11400000, guid: {galleryRoomGuid}, type: 2}}"));
        foreach (string definition in new[] { forwardDefinition, reverseDefinition })
        {
        ...
        Assert.That(inventory, Does.Contain(
            "16,complete,UpperGallery-Master,standard-door,2300000204,"));
        Assert.That(inventory, Does.Contain(
            "16,complete,UpperGallery-Master,standard-door,2300000214,"));
        Assert.That(inventory, Does.Not.Contain("16,queued,UpperGallery-Master"));

        foreach (string group15 in new[] { "2300000209", "2300000219" })
            Assert.That(ExtractUnityObjectBlock(scene, $"--- !u!114 &{group15}"),
                Does.Contain("canonicalPassage: {fileID: 41000000"));
        string group22 = ExtractUnityObjectBlock(scene, "--- !u!114 &106972347");
        Assert.That(group22, Does.Contain("navigationManager: {fileID: 0}"));
        Assert.That(group22, Does.Not.Contain("canonicalPassage:"));
        foreach (string protectedName in new[]
        {
            "upper_gallery_railing", "upper_gallery_left_plant_0", "upper_gallery_right_plant_0",
            "Ch2_Hide_Guest07", "bedroomchair_0", "bed_0", "Master_bedroom_loveseat_0",
            "PlayerBlocker_bedroomchair_0", "PlayerBlocker_bed_0",
            "PlayerBlocker_Master_bedroom_loveseat_0"
        })
            Assert.That(scene, Does.Contain($"m_Name: {protectedName}"));
    }

    [Test]
    public void UpperSittingHallNurserySourceAndDestinationRegionPassagePairIsExact()
    {
        const string passageGuid = "518dad8adf634786a103bf4e76aa0881";
        const string roomViewGuid = "ccd2f3bd803e45aa8a1174cc881d6dc0";
        const string doorTriggerGuid = "7e419b0f8f26d4f2d8d03e567fef4c52";
        const string forwardDefinitionGuid = "e40848fe5ab54ec99ff3b664859e7c1b";
        const string reverseDefinitionGuid = "c05a3dd92c9f45cf8231f1679bd06506";
        const string upperRoomGuid = "2ce92b6f43ff440e95570359e18cce0f";
        const string nurseryRoomGuid = "3f5e14fc508c487da959ed9f5c8538a2";
        const string forwardPath =
        ...
        string gameRoot = ExtractUnityObjectBlock(scene, "--- !u!114 &1878886998");
        string upperRoot = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000050");
        string nurseryRoot = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000055");
        string nurseryView = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000052");
        string forwardOwner = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000220");
        string forwardTrigger = ExtractUnityObjectBlock(scene, "--- !u!114 &2300000224");
        string forwardPassage = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000053");
        string reverseOwner = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000230");
        string reverseTrigger = ExtractUnityObjectBlock(scene, "--- !u!114 &2300000234");
        string reversePassage = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000054");
        string forwardDefinition = File.ReadAllText(forwardPath);
        string reverseDefinition = File.ReadAllText(reversePath);
        ...
        Assert.That(Regex.Matches(inventory, @"(?m)^\d+,blocked-one-way,").Count, Is.EqualTo(2));
        Assert.That(Regex.Matches(inventory, @"(?m)^\d+,blocked-parallel,").Count, Is.EqualTo(3));
        foreach (string id in new[] { "4100000052", "4100000053", "4100000054" })
            Assert.That(Regex.Matches(gameRoot, $@"(?m)^  - \{{fileID: {id}\}}$").Count, Is.EqualTo(1));

        Assert.That(Regex.Matches(upperRoot, @"(?m)^  - component:").Count, Is.EqualTo(3));
        Assert.That(upperRoot, Does.Contain("- component: {fileID: 4100000043}"));
        Assert.That(Regex.Matches(nurseryRoot, @"(?m)^  - component:").Count, Is.EqualTo(3));
        Assert.That(nurseryRoot, Does.Contain("- component: {fileID: 4100000052}"));
        ...
        foreach (string owner in new[] { forwardOwner, reverseOwner })
            Assert.That(Regex.Matches(owner, @"(?m)^  - component:").Count, Is.EqualTo(5));
        Assert.That(forwardOwner, Does.Contain("- component: {fileID: 4100000053}"));
        Assert.That(reverseOwner, Does.Contain("- component: {fileID: 4100000054}"));
        foreach (string trigger in new[] { forwardTrigger, reverseTrigger })
        {
            Assert.That(trigger, Does.Contain("navigationManager: {fileID: 1878886997}"));
            Assert.That(trigger, Does.Contain("doorOpenAudioSource: {fileID: 2201000013}"));
        ...
        Assert.That(forwardTrigger, Does.Contain("canonicalPassage: {fileID: 4100000053}"));
        Assert.That(reverseTrigger, Does.Contain("canonicalPassage: {fileID: 4100000054}"));
        AssertSourceAndDestinationRegionPassageDocument(
            forwardPassage, forwardDefinitionGuid, "4100000043", "4100000054",
            "{x: 157, y: -88.39423}", "{x: 157, y: 209.43507}",
            "{x: 277, y: 209.43507}", "{x: 277, y: -88.39423}");
        AssertSourceAndDestinationRegionPassageDocument(
            reversePassage, reverseDefinitionGuid, "4100000052", "4100000053",
            "{x: -78.9274, y: -32.718}", "{x: -78.9274, y: 178.862}",
            "{x: 46.9532, y: 178.862}", "{x: 46.9532, y: -32.718}");

        Assert.That(forwardDefinition, Does.Contain("stableId: passage.upper-sitting-hall.nursery"));
        Assert.That(forwardDefinition, Does.Contain(
            $"sourceRoom: {{fileID: 11400000, guid: {upperRoomGuid}, type: 2}}"));
        Assert.That(forwardDefinition, Does.Contain(
            $"destinationRoom: {{fileID: 11400000, guid: {nurseryRoomGuid}, type: 2}}"));
        Assert.That(forwardDefinition, Does.Contain(
            $"reverse: {{fileID: 11400000, guid: {reverseDefinitionGuid}, type: 2}}"));
        Assert.That(forwardDefinition, Does.Contain("legacyDoorId: UpperSittingHall_Nursery"));
        Assert.That(reverseDefinition, Does.Contain("stableId: passage.nursery.upper-sitting-hall"));
        Assert.That(reverseDefinition, Does.Contain(
            $"sourceRoom: {{fileID: 11400000, guid: {nurseryRoomGuid}, type: 2}}"));
        Assert.That(reverseDefinition, Does.Contain(
            $"destinationRoom: {{fileID: 11400000, guid: {upperRoomGuid}, type: 2}}"));
        Assert.That(reverseDefinition, Does.Contain(
            $"reverse: {{fileID: 11400000, guid: {forwardDefinitionGuid}, type: 2}}"));
        Assert.That(reverseDefinition, Does.Contain("legacyDoorId: Nursery_UpperSittingHall"));
        foreach (string definition in new[] { forwardDefinition, reverseDefinition })
        {
            Assert.That(definition, Does.Contain("kind: 0"));
            Assert.That(definition, Does.Contain("promptText: Open Door"));
        ...
        Assert.That(inventory, Does.Contain(
            "17,complete,UpperSitting-Nursery,standard-door,2300000224,"));
        Assert.That(inventory, Does.Contain(
            "17,complete,UpperSitting-Nursery,standard-door,2300000234,"));
        Assert.That(inventory, Does.Not.Contain("17,queued,UpperSitting-Nursery"));

        foreach (string completedTriggerId in new[]
                 { "1915714844", "2300000189", "2300000209", "2300000219",
                   "2300000229", "2300000239" })
        {
            string trigger = ExtractUnityObjectBlock(scene, $"--- !u!114 &{completedTriggerId}");
            Assert.That(trigger, Does.Contain("navigationManager: {fileID: 1878886997}"));
            Assert.That(trigger, Does.Contain("canonicalPassage: {fileID: 41000000"));
        }
        foreach (string queuedTriggerId in new[]
                 { "1084796962", "1498249206" })
        {
            string trigger = ExtractUnityObjectBlock(scene, $"--- !u!114 &{queuedTriggerId}");
            Assert.That(trigger, Does.Contain("navigationManager: {fileID: 0}"));
            Assert.That(trigger, Does.Not.Contain("canonicalPassage:"));
        }
        foreach (string protectedName in new[]
        {
            "nursery_chair_0", "nursery_table_0", "nursery_chest_0", "dog_toy_nursery_0",
            "PlayerBlocker_nursery_chair_0", "PlayerBlocker_nursery_table_0",
            "PatchCandidate_Nursery_Mobile_Or_Toy_Flicker"
        })
            Assert.That(scene, Does.Contain($"m_Name: {protectedName}"));
    }

    [Test]
    public void UpperSittingHallBlueBedroomScaledSourceAndDestinationRegionPassagePairIsExact()
    {
        const string passageGuid = "518dad8adf634786a103bf4e76aa0881";
        const string roomViewGuid = "ccd2f3bd803e45aa8a1174cc881d6dc0";
        const string doorTriggerGuid = "7e419b0f8f26d4f2d8d03e567fef4c52";
        const string forwardDefinitionGuid = "0927098c96b54093aa079279a7dcad2a";
        const string reverseDefinitionGuid = "84148ad6c0674b03bbda8bcb0690325d";
        const string upperRoomGuid = "2ce92b6f43ff440e95570359e18cce0f";
        const string blueRoomGuid = "98e71938214a483786829422c9a47278";
        const string forwardPath =
        ...
        string gameRoot = ExtractUnityObjectBlock(scene, "--- !u!114 &1878886998");
        string upperRoot = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000050");
        string blueRoot = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000060");
        string blueView = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000055");
        string forwardOwner = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000225");
        string forwardTrigger = ExtractUnityObjectBlock(scene, "--- !u!114 &2300000229");
        string forwardPassage = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000056");
        string reverseOwner = ExtractUnityObjectBlock(scene, "--- !u!1 &2300000235");
        string reverseTrigger = ExtractUnityObjectBlock(scene, "--- !u!114 &2300000239");
        string reversePassage = ExtractUnityObjectBlock(scene, "--- !u!114 &4100000057");
        string forwardDefinition = File.ReadAllText(forwardPath);
        string reverseDefinition = File.ReadAllText(reversePath);
        ...
        Assert.That(Regex.Matches(database,
            ...
            Is.EqualTo(57));
        Assert.That(Regex.Matches(gameRoot, @"(?m)^  - \{fileID:").Count, Is.EqualTo(74));
        Match sceneBehaviourReferences = Regex.Match(gameRoot,
            @"(?ms)^  sceneBehaviours:\r?\n(?<refs>(?:  - \{fileID: \d+\}\r?\n)+)");
        Assert.That(sceneBehaviourReferences.Success, Is.True);
        Assert.That(Regex.Matches(sceneBehaviourReferences.Groups["refs"].Value,
            @"(?m)^  - \{fileID:").Count, Is.EqualTo(66));
        Assert.That(Regex.Matches(scene, @"(?m)^  approachPlacementMode: 1$").Count,
            Is.EqualTo(16));
        Assert.That(Regex.Matches(scene, @"(?m)^  arrivalPlacementMode: 1$").Count,
            Is.EqualTo(18));
        ...
        Assert.That(CountUnityObjectBlocksContaining(
            ...
            Is.EqualTo(1));
        Assert.That(CountUnityObjectBlocksContaining(
            scene, $"guid: {doorTriggerGuid}", "doorOpenSoundCatalog: {fileID: 0}"),
            Is.EqualTo(8));
        int canonicalCallerCount = CountUnityObjectBlocksContaining(
            scene, $"guid: {doorTriggerGuid}", "canonicalPassage: {fileID:");
        Assert.That(canonicalCallerCount, Is.EqualTo(38));
        Assert.That(45 - canonicalCallerCount, Is.EqualTo(7));
        Assert.That(Regex.Matches(inventory, @"(?m)^\d+,complete,").Count, Is.EqualTo(38));
        Assert.That(Regex.Matches(inventory, @"(?m)^\d+,queued,").Count, Is.EqualTo(2));
        Assert.That(Regex.Matches(inventory, @"(?m)^\d+,blocked-one-way,").Count, Is.EqualTo(2));
        Assert.That(Regex.Matches(inventory, @"(?m)^\d+,blocked-parallel,").Count, Is.EqualTo(3));
        foreach (string id in new[] { "4100000055", "4100000056", "4100000057" })
            Assert.That(Regex.Matches(gameRoot, $@"(?m)^  - \{{fileID: {id}\}}$").Count, Is.EqualTo(1));

        Assert.That(Regex.Matches(upperRoot, @"(?m)^  - component:").Count, Is.EqualTo(3));
        Assert.That(upperRoot, Does.Contain("- component: {fileID: 4100000043}"));
        Assert.That(Regex.Matches(blueRoot, @"(?m)^  - component:").Count, Is.EqualTo(3));
        Assert.That(blueRoot, Does.Contain("- component: {fileID: 4100000055}"));
        Assert.That(blueView, Does.Contain(
            $"definition: {{fileID: 11400000, guid: {blueRoomGuid}, type: 2}}"));
        Assert.That(blueView, Does.Contain("legacyContentGroup: {fileID: 2300000062}"));
        foreach (string owner in new[] { forwardOwner, reverseOwner })
            Assert.That(Regex.Matches(owner, @"(?m)^  - component:").Count, Is.EqualTo(5));
        Assert.That(forwardOwner, Does.Contain("- component: {fileID: 4100000056}"));
        Assert.That(reverseOwner, Does.Contain("- component: {fileID: 4100000057}"));
        foreach (string trigger in new[] { forwardTrigger, reverseTrigger })
        {
            Assert.That(trigger, Does.Contain("navigationManager: {fileID: 1878886997}"));
            Assert.That(trigger, Does.Contain("doorOpenAudioSource: {fileID: 2201000013}"));
        ...
        Assert.That(forwardTrigger, Does.Contain("canonicalPassage: {fileID: 4100000056}"));
        Assert.That(reverseTrigger, Does.Contain("canonicalPassage: {fileID: 4100000057}"));
        AssertSourceAndDestinationRegionPassageDocument(
            forwardPassage, forwardDefinitionGuid, "4100000043", "4100000057",
            "{x: -715, y: -240.000061}", "{x: -715, y: 259.7826538}",
            "{x: -532.2076416, y: 259.7826538}", "{x: -532.2076416, y: -240.000061}");
        AssertSourceAndDestinationRegionPassageDocument(
            reversePassage, reverseDefinitionGuid, "4100000055", "4100000056",
            "{x: 334.5011902, y: -225.5001984}", "{x: 334.5011902, y: 330.0778198}",
            "{x: 437.8641663, y: 330.0778198}", "{x: 437.8641663, y: -225.5001984}");

        Assert.That(forwardDefinition, Does.Contain("stableId: passage.upper-sitting-hall.blue-bedroom"));
        Assert.That(forwardDefinition, Does.Contain(
            $"sourceRoom: {{fileID: 11400000, guid: {upperRoomGuid}, type: 2}}"));
        Assert.That(forwardDefinition, Does.Contain(
            $"destinationRoom: {{fileID: 11400000, guid: {blueRoomGuid}, type: 2}}"));
        Assert.That(forwardDefinition, Does.Contain(
            $"reverse: {{fileID: 11400000, guid: {reverseDefinitionGuid}, type: 2}}"));
        Assert.That(forwardDefinition, Does.Contain("legacyDoorId: UpperSittingHall_BlueBedroom"));
        Assert.That(reverseDefinition, Does.Contain("stableId: passage.blue-bedroom.upper-sitting-hall"));
        Assert.That(reverseDefinition, Does.Contain(
            $"sourceRoom: {{fileID: 11400000, guid: {blueRoomGuid}, type: 2}}"));
        Assert.That(reverseDefinition, Does.Contain(
            $"destinationRoom: {{fileID: 11400000, guid: {upperRoomGuid}, type: 2}}"));
        Assert.That(reverseDefinition, Does.Contain(
            $"reverse: {{fileID: 11400000, guid: {forwardDefinitionGuid}, type: 2}}"));
        Assert.That(reverseDefinition, Does.Contain("legacyDoorId: BlueBedroom_UpperSittingHall"));
        foreach (string definition in new[] { forwardDefinition, reverseDefinition })
        {
            Assert.That(definition, Does.Contain("kind: 0"));
            Assert.That(definition, Does.Contain("promptText: Open Door"));
        ...
        Assert.That(inventory, Does.Contain(
            "18,complete,UpperSitting-Blue,standard-door,2300000229,"));
        Assert.That(inventory, Does.Contain(
            "18,complete,UpperSitting-Blue,standard-door,2300000239,"));
        Assert.That(inventory, Does.Not.Contain("18,queued,UpperSitting-Blue"));

        foreach (string completedTriggerId in new[]
                 { "2300000209", "2300000219", "2300000224", "2300000234" })
        {
            string trigger = ExtractUnityObjectBlock(scene, $"--- !u!114 &{completedTriggerId}");
            Assert.That(trigger, Does.Contain("navigationManager: {fileID: 1878886997}"));
            Assert.That(trigger, Does.Contain("canonicalPassage: {fileID: 41000000"));
        ...
        foreach (string protectedName in new[]
        {
            "blue_room_bed_0", "bluebedroomchair_0", "Ch2_Hide_Guest08",
            "PlayerBlocker_blue_room_bed_0", "PlayerBlocker_bluebedroomchair_0",
            "PatchCandidate_Blue_Bedroom_Curtain_Moon_Flicker"
        })
            Assert.That(scene, Does.Contain($"m_Name: {protectedName}"));
    }

...
public class NavigationRegressionTests
{
    ...
    public void PlayerMovementUsesOnlyFloorBoundaryForWalkability()
    {
        ...
        Assert.That(playerText, Does.Contain("currentRoomStageScaleRatio"), "Player visible position should apply the room-stage zoom ratio.");
        ...
        Assert.That(playerText, Does.Not.Match(@"ApplyPerspectiveScale\s*\([^)]*\)[\s\S]*CalculateExistingPerspectiveScale\(\)\s*\*\s*currentRoomStageScaleRatio"), "Mouse-wheel room zoom must not change the Butler's transform.localScale.");

...
public class NavigationRegressionTests
{
    ...
    public void ActorRoomStateBindsWorldActorsToRoomStageLocalPoints()
    {
        ...
        Assert.That(actorRoomStateText, Does.Contain("BindVisibleFeetToRoomStagePoint"), "Foot waypoints need an explicit binding path separate from static actor-root anchors.");
        Assert.That(actorRoomStateText, Does.Contain("RoomStageBindingAnchorMode"), "Room-stage binding should encode whether an anchor means actor root or visible feet.");
        ...
        Assert.That(actorRoomStateText, Does.Contain("CharacterFootPositionUtility.TryGetWorldPoint"), "Only explicit visible-feet bindings may use renderer feet to correct actor-root placement.");
        Assert.That(actorRoomStateText, Does.Contain("roomStageBindingAnchorMode == RoomStageBindingAnchorMode.VisibleFeet"), "Visible-foot correction must be fenced behind the explicit foot-anchor mode.");
        ...
        Assert.That(chapter1ArrivalText, Does.Contain("BindGuestToRoomStagePoint(guestState, target, true)"), "Chapter 1 movement targets should bind world-space guests by visible feet.");
        ...
        Assert.That(chapter1ArrivalText, Does.Contain("guestState.ActorState.BindVisibleFeetToRoomStagePoint(target)"), "Chapter 1 foot waypoints should use ActorRoomState's explicit visible-feet API.");

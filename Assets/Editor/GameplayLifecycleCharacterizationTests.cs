...
public sealed class GameplayLifecycleCharacterizationTests
{
    ...
    public IEnumerator MusicLibraryAuthoredAnchorPassagesUseInvariantApproachesAndArrivals()
    {
        ...
        try
        {
            ...
            Assert.That(
                ...
                Is.EqualTo(20),
                "The first ten reciprocal route pairs must have canonical callers at this gate.");
            Assert.That(
                FindInActiveScene<DoorTriggerNavigation>()
                    .Count(trigger => GetPrivateField<CanonicalPassage>(trigger, "canonicalPassage") == null),
                Is.EqualTo(25),
                "Every trigger outside the first ten reciprocal pairs must retain its null caller.");
            ...
            Assert.That(roomViews, Has.Length.EqualTo(11));
            ...
            Assert.That(
                serializedGameplayTriggers.Count(document =>
                    ...
                    document.Contains(
                        "doorOpenSoundCatalog: {fileID: 11400000, guid: 9a77542e25184fbc945d6a79f77007e7, type: 2}")),
                Is.EqualTo(20),
                "Exactly the ten complete reciprocal pairs may have direct compatibility bindings.");
            Assert.That(
                serializedGameplayTriggers.Count(document =>
            ...
            Assert.That(
                ...
                Is.EqualTo(20),
                "The first ten reciprocal route pairs must have canonical callers.");
            Assert.That(
                serializedGameplayTriggers.Count(document =>
                    !document.Contains("canonicalPassage: {fileID:")),
                Is.EqualTo(25),
    ...
    [UnityTest]
    public IEnumerator DiningButlersPantryAuthoredAnchorPassagesUseInvariantApproachesAndArrivals()
    {
        const string DiningRoom = "Dining Room";
        const string ButlersPantry = "Butlers Pantry";
        const string AuthoredObservationSha256 =
            "78267865829752279aaa796771f8d51c92b0fac44dc2fe93a4dd12885ccf2d7e";
        Vector2 diningAnchorCandidate = new Vector2(3.391918f, -0.36f);
        Vector2 pantryAnchorCandidate = new Vector2(-5.163103f, -3.463186f);
        ...
        try
        {
            ...
            DoorTriggerNavigation setupTrigger =
            ...
            forward = RequireSceneObject<DoorTriggerNavigation>("DoorTrigger_DiningRoom_ButlersPantry");
            reverse = RequireSceneObject<DoorTriggerNavigation>("DoorTrigger_ButlersPantry_DiningRoom");
            Assert.That(forward.GetComponents<Component>(), Has.Length.EqualTo(5));
            Assert.That(reverse.GetComponents<Component>(), Has.Length.EqualTo(5));
            CanonicalPassage forwardPassage = forward.GetComponent<CanonicalPassage>();
            CanonicalPassage reversePassage = reverse.GetComponent<CanonicalPassage>();
            Assert.That(forwardPassage, Is.Not.Null);
            Assert.That(reversePassage, Is.Not.Null);
            Assert.That(forwardPassage.SourceRoomView.Definition.StableId, Is.EqualTo("room.dining-room"));
            Assert.That(reversePassage.SourceRoomView.Definition.StableId, Is.EqualTo("room.butlers-pantry"));
            Assert.That(forwardPassage.ReversePassage, Is.SameAs(reversePassage));
            Assert.That(reversePassage.ReversePassage, Is.SameAs(forwardPassage));
            AssertVector2Within(forwardPassage.ApproachAnchor.LogicalPosition,
                diningAnchorCandidate, 0.0001f, "authored Dining-to-Pantry approach");
            AssertVector2Within(forwardPassage.ArrivalAnchor.LogicalPosition,
                pantryAnchorCandidate, 0.0001f, "authored Dining-to-Pantry arrival");
            AssertVector2Within(reversePassage.ApproachAnchor.LogicalPosition,
                pantryAnchorCandidate, 0.0001f, "authored Pantry-to-Dining approach");
            AssertVector2Within(reversePassage.ArrivalAnchor.LogicalPosition,
                diningAnchorCandidate, 0.0001f, "authored Pantry-to-Dining arrival");
            Assert.That(forwardPassage.AnchorMigrationStage,
                Is.EqualTo(PassageAnchorMigrationStage.AuthoredAnchors));
            Assert.That(reversePassage.AnchorMigrationStage,
                Is.EqualTo(PassageAnchorMigrationStage.AuthoredAnchors));
            ...
            Assert.That(forwardRect, Is.Not.Null);
            Assert.That(reverseRect, Is.Not.Null);
            AssertVector2Within(forwardRect.anchoredPosition, new Vector2(266.91895f, 119.43254f), 0.0001f,
                "Dining-to-Pantry trigger position");
            AssertVector2Within(forwardRect.sizeDelta, new Vector2(89.6393f, 238.8651f), 0.0001f,
                "Dining-to-Pantry trigger size");
            AssertVector2Within(reverseRect.anchoredPosition, new Vector2(-463f, 16f), 0.0001f,
                "Pantry-to-Dining trigger position");
            AssertVector2Within(reverseRect.sizeDelta, new Vector2(209.7474f, 508.6494f), 0.0001f,
                "Pantry-to-Dining trigger size");
            foreach (DoorTriggerNavigation trigger in new[] { forward, reverse })
            {
                Assert.That(GetPrivateValue<float>(trigger, "maxPlayerScreenDistance"), Is.EqualTo(145f));
                InvokePrivateMethod(trigger, "ResolveReferences");
            ...
            Assert.That(RequireOnlyActiveRoom(DiningRoom), Is.SameAs(diningContent));
            Assert.That(diningContent.GetComponent<CanonicalRoomView>(), Is.Not.Null);
            CanonicalRoomView pantryView = pantryContent.GetComponent<CanonicalRoomView>();
            Assert.That(pantryView, Is.Not.Null);
            Assert.That(pantryView.Definition.StableId, Is.EqualTo("room.butlers-pantry"));
            Assert.That(pantryView.Definition.LegacyNames,
                Is.EqualTo(new[] { "Butlers Pantry", "Butler's Pantry" }));
            Assert.That(pantryView.LegacyContentGroup, Is.SameAs(pantryContent));
            Assert.That(diningContent.PerspectiveProfile, Is.Not.Null);
            Assert.That(pantryContent.PerspectiveProfile, Is.Null);
            Assert.That(pantryView.Definition.PerspectiveProfile, Is.Null);
            Assert.That(FindInActiveScene<ObjectMovementBlocker2D>()
                .Count(blocker => blocker.GetComponentInParent<RoomContentGroup>(true) == diningContent),
                Is.EqualTo(11));
            Assert.That(FindInActiveScene<ObjectMovementBlocker2D>()
            ...
            Assert.That(AuthoredObservationSha256, Has.Length.EqualTo(64));
            ...
            AssertVector2Within(primary.ForwardStart, new Vector2(0f, -1f), 0.001f,
                "primary Dining-to-Pantry far start");
            Assert.That(primary.ForwardScreenDistance, Is.EqualTo(203.514f).Within(0.75f));
            AssertVector2Within(primary.ForwardNull, new Vector2(2.891918f, 0f), 0.01f,
                "primary Dining-to-Pantry null approach");
            AssertVector2Within(primary.ForwardLeft, new Vector2(3.066995f, 0f), 0.01f,
                "primary Dining-to-Pantry left approach");
            AssertVector2Within(primary.ForwardCenter, new Vector2(3.475507f, 0f), 0.01f,
                "primary Dining-to-Pantry center approach");
            AssertVector2Within(primary.ForwardRight, new Vector2(3.884019f, 0f), 0.01f,
                "primary Dining-to-Pantry right approach");
            AssertVector2Within(primary.ForwardDispatch, diningAnchorCandidate, 0.0001f,
                "primary Dining-to-Pantry authored dispatch");
            AssertVector2Within(primary.ForwardProductionApproach, diningAnchorCandidate, 0.0001f,
                "primary Dining-to-Pantry production approach");
            AssertVector2Within(primary.ForwardArrival, pantryAnchorCandidate, 0.0001f,
                "primary authored Pantry arrival");
            AssertVector2Within(primary.ReverseStart, new Vector2(0f, -2f), 0.001f,
                "primary Pantry-to-Dining far start");
            Assert.That(primary.ReverseScreenDistance, Is.EqualTo(318.7f).Within(0.75f));
            AssertVector2Within(primary.ReverseNull, new Vector2(-4.663103f, -3.103186f), 0.01f,
                "primary Pantry-to-Dining null approach");
            AssertVector2Within(primary.ReverseLeft, primary.ReverseNull, 0.0001f,
                "primary Pantry-to-Dining left approach");
            AssertVector2Within(primary.ReverseCenter, new Vector2(-6.028646f, -3.103186f), 0.01f,
                "primary Pantry-to-Dining center approach");
            AssertVector2Within(primary.ReverseRight, new Vector2(-5.072765f, -3.103186f), 0.01f,
                "primary Pantry-to-Dining right approach");
            AssertVector2Within(primary.ReverseDispatch, pantryAnchorCandidate, 0.0001f,
                "primary Pantry-to-Dining authored dispatch");
            AssertVector2Within(primary.ReverseProductionApproach, pantryAnchorCandidate, 0.0001f,
                "primary Pantry-to-Dining production approach");
            AssertVector2Within(primary.ReverseArrival, diningAnchorCandidate, 0.0001f,
                "primary authored Dining arrival");
            AssertVector2Within(primary.NearForwardArrival, pantryAnchorCandidate, 0.0001f,
                "primary near authored Pantry arrival");
            AssertVector2Within(primary.NearReverseArrival, diningAnchorCandidate, 0.0001f,
                "primary near authored Dining arrival");
            Debug.Log($"[DiningButlersAuthoredPrimary] events={string.Join("|", primaryEvents)} " +
                FormatDoorRoundTripObservation(primary, includeNear: true));
            ...
            float[] expectedForwardDistances = { 203.514f, 290.954f, 286.059f, 376.283f };
            Vector2[] expectedForwardNulls =
            {
                new Vector2(2.891918f, 0f),
                new Vector2(2.503864f, -0.000001f),
                new Vector2(2.891213f, 0f),
                new Vector2(3.338484f, 0f)
            };
            Vector2[] expectedForwardLeft =
            {
                new Vector2(3.066995f, 0f),
                new Vector2(2.655447f, -0.000001f),
                new Vector2(3.066248f, 0f),
                new Vector2(3.540595f, 0f)
            };
            Vector2[] expectedForwardCenter =
            {
                new Vector2(3.475507f, 0f),
                new Vector2(3.009144f, -0.000001f),
                new Vector2(3.474659f, 0f),
                new Vector2(4.01219f, 0f)
            };
            Vector2[] expectedForwardRight =
            {
                new Vector2(3.884019f, 0f),
                new Vector2(3.362839f, -0.000001f),
                new Vector2(3.883071f, 0f),
                new Vector2(4.483785f, 0f)
            };
            Vector2[] expectedPantryPoints =
            {
                new Vector2(-4.663103f, -3.103186f),
                new Vector2(-4.03738f, -2.686783f),
                new Vector2(-4.661965f, -3.102429f),
                new Vector2(-5.383173f, -3.582376f)
            };
            float[] expectedReverseDistances = { 318.7f, 441.947f, 447.943f, 605.817f };
            Vector2[] expectedReverseCenter =
            {
                new Vector2(-6.028646f, -3.103186f),
                new Vector2(-5.219687f, -2.686783f),
                new Vector2(-6.027174f, -3.102429f),
                new Vector2(-6.959581f, -3.582376f)
            };
            Vector2[] expectedReverseRight =
            {
                new Vector2(-5.072765f, -3.103186f),
                new Vector2(-4.392071f, -2.686783f),
                new Vector2(-5.071528f, -3.102429f),
                new Vector2(-5.856095f, -3.582376f)
            };
            for (int sizeIndex = 0; sizeIndex < renderedSizes.Length; sizeIndex++)
            {
                Vector2Int renderedSize = renderedSizes[sizeIndex];
                ...
                Assert.That(aspect, Is.Not.Null);
                float coordinateTolerance = sizeIndex == renderedSizes.Length - 1 ? 0.2f : 0.05f;
                AssertVector2Within(aspect.ForwardStart, new Vector2(0f, -1f), 0.001f,
                    "aspect Dining-to-Pantry far start");
                Assert.That(aspect.ForwardScreenDistance,
                    Is.EqualTo(expectedForwardDistances[sizeIndex]).Within(0.75f));
                AssertVector2Within(aspect.ForwardNull, expectedForwardNulls[sizeIndex],
                    coordinateTolerance, "aspect Dining-to-Pantry null approach");
                AssertVector2Within(aspect.ForwardLeft, expectedForwardLeft[sizeIndex],
                    coordinateTolerance, "aspect Dining-to-Pantry left approach");
                AssertVector2Within(aspect.ForwardCenter, expectedForwardCenter[sizeIndex],
                    coordinateTolerance, "aspect Dining-to-Pantry center approach");
                AssertVector2Within(aspect.ForwardRight, expectedForwardRight[sizeIndex],
                    coordinateTolerance, "aspect Dining-to-Pantry right approach");
                AssertVector2Within(aspect.ForwardDispatch, diningAnchorCandidate, 0.0001f,
                    "aspect Dining-to-Pantry authored dispatch");
                AssertVector2Within(aspect.ForwardProductionApproach, diningAnchorCandidate, 0.0001f,
                    "aspect Dining-to-Pantry production approach");
                AssertVector2Within(aspect.ForwardArrival, pantryAnchorCandidate,
                    0.0001f, "aspect authored Pantry arrival");
                AssertVector2Within(aspect.ReverseStart, new Vector2(0f, -2f), 0.001f,
                    "aspect Pantry-to-Dining far start");
                Assert.That(aspect.ReverseScreenDistance,
                    Is.EqualTo(expectedReverseDistances[sizeIndex]).Within(0.75f));
                AssertVector2Within(aspect.ReverseNull, expectedPantryPoints[sizeIndex],
                    coordinateTolerance, "aspect Pantry-to-Dining null approach");
                AssertVector2Within(aspect.ReverseLeft, aspect.ReverseNull, 0.0001f,
                    "aspect Pantry-to-Dining left approach");
                AssertVector2Within(aspect.ReverseCenter, expectedReverseCenter[sizeIndex],
                    coordinateTolerance, "aspect Pantry-to-Dining center approach");
                AssertVector2Within(aspect.ReverseRight, expectedReverseRight[sizeIndex],
                    coordinateTolerance, "aspect Pantry-to-Dining right approach");
                AssertVector2Within(aspect.ReverseDispatch, pantryAnchorCandidate, 0.0001f,
                    "aspect Pantry-to-Dining authored dispatch");
                AssertVector2Within(aspect.ReverseProductionApproach, pantryAnchorCandidate, 0.0001f,
                    "aspect Pantry-to-Dining production approach");
                AssertVector2Within(aspect.ReverseArrival, diningAnchorCandidate,
                    0.0001f, "aspect authored Dining arrival");
                Debug.Log($"[DiningButlersAuthoredAspect] viewport={renderedSize.x}x{renderedSize.y} " +
                    FormatDoorRoundTripObservation(aspect, includeNear: false));
            }
            ...
            Assert.That(maximum, Is.Not.Null);
            AssertVector2Within(maximum.ForwardStart, new Vector2(0f, -1f), 0.001f,
                "maximum-zoom Dining-to-Pantry far start");
            Assert.That(maximum.ForwardScreenDistance, Is.EqualTo(433.081f).Within(0.75f));
            AssertVector2Within(maximum.ForwardNull, new Vector2(3.338485f, 0f), 0.2f,
                "maximum-zoom Dining-to-Pantry null approach");
            AssertVector2Within(maximum.ForwardLeft, new Vector2(3.540596f, 0f), 0.2f,
                "maximum-zoom Dining-to-Pantry left approach");
            AssertVector2Within(maximum.ForwardCenter, new Vector2(4.01219f, 0f), 0.2f,
                "maximum-zoom Dining-to-Pantry center approach");
            AssertVector2Within(maximum.ForwardRight, new Vector2(4.483785f, 0f), 0.2f,
                "maximum-zoom Dining-to-Pantry right approach");
            AssertVector2Within(maximum.ForwardDispatch, diningAnchorCandidate, 0.0001f,
                "maximum-zoom Dining-to-Pantry authored dispatch");
            AssertVector2Within(maximum.ForwardProductionApproach, diningAnchorCandidate, 0.0001f,
                "maximum-zoom Dining-to-Pantry production approach");
            AssertVector2Within(maximum.ForwardArrival, pantryAnchorCandidate, 0.0001f,
                "maximum-zoom authored Pantry arrival");
            AssertVector2Within(maximum.ReverseStart, new Vector2(0f, -2f), 0.001f,
                "maximum-zoom Pantry-to-Dining far start");
            Assert.That(maximum.ReverseScreenDistance, Is.EqualTo(697.262f).Within(0.75f));
            AssertVector2Within(maximum.ReverseNull, new Vector2(-5.383173f, -3.582376f), 0.2f,
                "maximum-zoom Pantry-to-Dining null approach");
            AssertVector2Within(maximum.ReverseLeft, maximum.ReverseNull, 0.0001f,
                "maximum-zoom Pantry-to-Dining left approach");
            AssertVector2Within(maximum.ReverseCenter, new Vector2(-6.959581f, -3.582376f), 0.2f,
                "maximum-zoom Pantry-to-Dining center approach");
            AssertVector2Within(maximum.ReverseRight, new Vector2(-5.856096f, -3.582376f), 0.2f,
                "maximum-zoom Pantry-to-Dining right approach");
            AssertVector2Within(maximum.ReverseDispatch, pantryAnchorCandidate, 0.0001f,
                "maximum-zoom Pantry-to-Dining authored dispatch");
            AssertVector2Within(maximum.ReverseProductionApproach, pantryAnchorCandidate, 0.0001f,
                "maximum-zoom Pantry-to-Dining production approach");
            AssertVector2Within(maximum.ReverseArrival, diningAnchorCandidate, 0.0001f,
                "maximum-zoom authored Dining arrival");
            Debug.Log($"[DiningButlersAuthoredMaximumZoom] viewport={Screen.width}x{Screen.height} " +
                $"zoom={cameraManager.maxRoomZoom:0.###} " +
                FormatDoorRoundTripObservation(maximum, includeNear: false));
            ...
            yield return AssertDiningButlersCallerBoundStageZeroPoisonProof(
                navigation,
                player,
                cameraManager,
                forward,
                ...
                forwardPassage,
                reversePassage);
            yield return AssertDiningButlersStageOneArrivalFallbackProof(
                navigation,
                player,
                cameraManager,
                forward,
                ...
                forwardPassage,
                reversePassage);
            yield return AssertDiningButlersPairLocalNullCallerFallback(
                navigation,
                player,
                cameraManager,
                forward,
                ...
                reversePassage);
            Debug.Log(
                $"[DiningButlersAuthoredProfile] forwardGeometry={FormatVector(forwardRect.anchoredPosition)}/" +
                $"{FormatVector(forwardRect.sizeDelta)} reverseGeometry={FormatVector(reverseRect.anchoredPosition)}/" +
                $"{FormatVector(reverseRect.sizeDelta)} profiles=invariant callers=bound serializedDependencies=bound " +
                $"runtimeDependencies=resolved");
        }
    ...
    [UnityTest]
    public IEnumerator ButlersPantryServiceCorridorAuthoredAnchorPassagesUseInvariantApproachesAndArrivals()
    {
        const string ButlersPantry = "Butlers Pantry";
        const string ServiceCorridor = "Service Corridor";
        const string AuthoredObservationSha256 =
            "7cc7c8706a02443b30c91c7a3cdcce40b7c04fec2f54e3f2eb7d5acad7058ea9";
        Vector2 pantryAnchorCandidate = new Vector2(7f, -2.8f);
        Vector2 serviceAnchorCandidate = new Vector2(4.2f, -3.3f);
        ...
        try
        {
            ...
            Assert.That(FindInActiveScene<CanonicalPassage>()
                ...
                Is.Zero);
            Assert.That(FindInActiveScene<CanonicalPassage>()
                .Count(passage => passage.AnchorMigrationStage == PassageAnchorMigrationStage.AuthoredArrival),
                Is.Zero);
            Assert.That(FindInActiveScene<CanonicalPassage>()
            ...
            Assert.That(forward.DestinationRoom, Is.EqualTo(ServiceCorridor));
            Assert.That(reverse.SourceRoom, Is.EqualTo(ServiceCorridor));
            Assert.That(reverse.DoorName, Is.EqualTo("ServiceCorridor_ButlersPantry"));
            ...
            Assert.That(reverse.IsStairway, Is.False);
            Assert.That(forwardPassage.Definition.StableId,
                Is.EqualTo("passage.butlers-pantry.service-corridor"));
            Assert.That(reversePassage.Definition.StableId,
                Is.EqualTo("passage.service-corridor.butlers-pantry"));
            Assert.That(forwardPassage.Definition.LegacyDoorId, Is.EqualTo(forward.DoorName));
            Assert.That(reversePassage.Definition.LegacyDoorId, Is.EqualTo(reverse.DoorName));
            ...
            Assert.That(reversePassage.UsesAuthoredArrival, Is.True);
            AssertVector2Within(forwardPassage.ApproachAnchor.LogicalPosition,
                pantryAnchorCandidate, 0.0001f, "Pantry-to-Service authored approach");
            AssertVector2Within(forwardPassage.ArrivalAnchor.LogicalPosition,
                serviceAnchorCandidate, 0.0001f, "Pantry-to-Service authored arrival");
            AssertVector2Within(reversePassage.ApproachAnchor.LogicalPosition,
                serviceAnchorCandidate, 0.0001f, "Service-to-Pantry authored approach");
            AssertVector2Within(reversePassage.ArrivalAnchor.LogicalPosition,
                pantryAnchorCandidate, 0.0001f, "Service-to-Pantry authored arrival");
            foreach (DoorTriggerNavigation trigger in new[] { forward, reverse })
            {
                Assert.That(GetPrivateValue<float>(trigger, "maxPlayerScreenDistance"), Is.EqualTo(145f));
                Assert.That(GetPrivateValue<bool>(trigger, "requirePlayerInSourceRoom"), Is.True);
            ...
            Assert.That(forwardRect, Is.Not.Null);
            Assert.That(reverseRect, Is.Not.Null);
            AssertVector2Within(forwardRect.anchoredPosition, new Vector2(591.2165f, 33.108276f), 0.0001f,
                "Pantry-to-Service trigger position");
            AssertVector2Within(forwardRect.sizeDelta, new Vector2(188.3424f, 453.9467f), 0.0001f,
                "Pantry-to-Service trigger size");
            AssertVector2Within(reverseRect.anchoredPosition, new Vector2(352f, 28f), 0.0001f,
                "Service-to-Pantry trigger position");
            AssertVector2Within(reverseRect.sizeDelta, new Vector2(124.2894f, 524.2852f), 0.0001f,
                "Service-to-Pantry trigger size");
            ...
            Assert.That(reverse.GetComponentInParent<RoomContentGroup>(true), Is.SameAs(serviceContent));
            Assert.That(forwardRect.parent.name, Is.EqualTo("Doors"));
            Assert.That(reverseRect.parent.name, Is.EqualTo("Doors"));
            Assert.That(pantryContent.GetComponent<CanonicalRoomView>(), Is.Not.Null);
            CanonicalRoomView pantryView = pantryContent.GetComponent<CanonicalRoomView>();
            CanonicalRoomView serviceView = serviceContent.GetComponent<CanonicalRoomView>();
            Assert.That(pantryView, Is.Not.Null);
            Assert.That(serviceView, Is.Not.Null);
            Assert.That(forwardPassage.SourceRoomView, Is.SameAs(pantryView));
            Assert.That(reversePassage.SourceRoomView, Is.SameAs(serviceView));
            Assert.That(forwardPassage.Definition.SourceRoom, Is.SameAs(pantryView.Definition));
            Assert.That(forwardPassage.Definition.DestinationRoom, Is.SameAs(serviceView.Definition));
            Assert.That(reversePassage.Definition.SourceRoom, Is.SameAs(serviceView.Definition));
            Assert.That(reversePassage.Definition.DestinationRoom, Is.SameAs(pantryView.Definition));
            Assert.That(serviceView.Definition.StableId, Is.EqualTo("room.service-corridor"));
            Assert.That(serviceView.Definition.DisplayName, Is.EqualTo(ServiceCorridor));
            Assert.That(serviceView.Definition.LegacyNames, Is.EqualTo(new[] { ServiceCorridor }));
            Assert.That(serviceView.LegacyContentGroup, Is.SameAs(serviceContent));
            Assert.That(serviceView.Root, Is.SameAs(serviceContent.transform));
            Assert.That(pantryView.IsVisible, Is.True);
            Assert.That(serviceView.IsVisible, Is.False);
            Assert.That(serviceView.HasGameContext, Is.True);
            ...
            Assert.That(GetPrivateField<Texture>(serviceContent, "roomBackgroundTexture"), Is.Not.Null);
            Assert.That(GetPrivateField<Texture>(pantryContent, "roomBackgroundTexture"),
                Is.Not.SameAs(GetPrivateField<Texture>(serviceContent, "roomBackgroundTexture")));
            Assert.That(pantryContent.PerspectiveProfile, Is.Null);
            Assert.That(serviceContent.PerspectiveProfile, Is.Null);
            Assert.That(serviceView.Definition.BackgroundTexture,
                Is.SameAs(GetPrivateField<Texture>(serviceContent, "roomBackgroundTexture")));
            Assert.That(serviceView.Definition.PerspectiveProfile, Is.Null);
            Assert.That(FindInActiveScene<ObjectMovementBlocker2D>()
                .Count(blocker => blocker.GetComponentInParent<RoomContentGroup>(true) == pantryContent),
                Is.Zero);
            ObjectMovementBlocker2D[] serviceBlockers = FindInActiveScene<ObjectMovementBlocker2D>()
                .Where(blocker => blocker.GetComponentInParent<RoomContentGroup>(true) == serviceContent)
                .ToArray();
            Assert.That(serviceBlockers, Has.Length.EqualTo(2));
            foreach (string blockerName in new[]
            {
                "PlayerBlocker_service_corridor_left_table_0",
                "PlayerBlocker_service_corridor_right_desk_0"
            })
            {
                ObjectMovementBlocker2D blocker = serviceBlockers.Single(item => item.name == blockerName);
                Assert.That(blocker.GetComponents<Component>(), Has.Length.EqualTo(3));
            }
            SpriteRenderer serviceLeftTable = FindInActiveScene<SpriteRenderer>()
                .Single(item => item.name == "service_corridor_left_table_0");
            SpriteRenderer serviceRightDesk = FindInActiveScene<SpriteRenderer>()
                .Single(item => item.name == "service_corridor_right_desk_0");
            Assert.That(serviceLeftTable.sortingOrder, Is.EqualTo(1000),
                "The inactive Service Corridor cutout must retain its serialized sorting baseline.");
            Assert.That(serviceRightDesk.sortingOrder, Is.EqualTo(1000),
                "The inactive Service Corridor cutout must retain its serialized sorting baseline.");

            Chateau.Architecture.ValidationReport serviceViewReport =
                new Chateau.Architecture.ValidationReport();
            Chateau.Architecture.ValidationReport forwardPassageReport =
                new Chateau.Architecture.ValidationReport();
            Chateau.Architecture.ValidationReport reversePassageReport =
                new Chateau.Architecture.ValidationReport();
            serviceView.ValidateConfiguration(serviceViewReport);
            forwardPassage.ValidateConfiguration(forwardPassageReport);
            reversePassage.ValidateConfiguration(reversePassageReport);
            Assert.That(serviceViewReport.Messages, Is.Empty);
            Assert.That(forwardPassageReport.Messages, Is.Empty);
            Assert.That(reversePassageReport.Messages, Is.Empty);

            Chateau.Architecture.GameRoot gameRoot =
                RequireExactlyOneInActiveScene<Chateau.Architecture.GameRoot>();
            Assert.That(gameRoot.Context.Database.Definitions, Has.Count.EqualTo(39));
            ...
            Assert.That(AssetDatabase.GetAssetPath(serviceView.Definition),
                Is.EqualTo("Assets/_Chateau/Data/World/Rooms/Room_ServiceCorridor.asset"));
            Assert.That(AssetDatabase.GetAssetPath(forwardPassage.Definition),
                Is.EqualTo("Assets/_Chateau/Data/World/Passages/Passage_ButlersPantry_ServiceCorridor.asset"));
            Assert.That(AssetDatabase.GetAssetPath(reversePassage.Definition),
                Is.EqualTo("Assets/_Chateau/Data/World/Passages/Passage_ServiceCorridor_ButlersPantry.asset"));
            List<Chateau.Architecture.ChateauBehaviour> sceneBehaviours =
                GetPrivateField<List<Chateau.Architecture.ChateauBehaviour>>(gameRoot, "sceneBehaviours");
            Assert.That(sceneBehaviours.Count(item => item == serviceView), Is.EqualTo(1));
            ...
            string projectRoot = System.IO.Directory.GetParent(Application.dataPath).FullName;
            string sceneText = System.IO.File.ReadAllText(System.IO.Path.Combine(projectRoot, GameplayScenePath));
            string databaseText = System.IO.File.ReadAllText(System.IO.Path.Combine(
                projectRoot,
                AssetDatabase.GetAssetPath(gameRoot.Context.Database)));
            ...
            {
                Assert.That(databaseText, Does.Contain(
                    $"{{fileID: 11400000, guid: {definitionGuid}, type: 2}}"));
            }
            string serializedForward = RequireSerializedUnityDocument(sceneText, "2300000149");
            string serializedReverse = RequireSerializedUnityDocument(sceneText, "2300000154");
            string serializedServiceRoot = RequireSerializedUnityDocument(sceneText, "2300000025", "1");
            string serializedServiceView = RequireSerializedUnityDocument(sceneText, "4100000009");
            string serializedForwardPassage = RequireSerializedUnityDocument(sceneText, "4100000025");
            string serializedReversePassage = RequireSerializedUnityDocument(sceneText, "4100000026");
            string serializedGameRoot = RequireSerializedUnityDocument(sceneText, "1878886998");
            Assert.That(serializedServiceRoot, Does.Contain("- component: {fileID: 4100000009}"));
            Assert.That(serializedServiceView, Does.Contain(
                "definition: {fileID: 11400000, guid: 85d51b6fcb4840458d45f66bbf6c233b, type: 2}"));
            Assert.That(serializedServiceView, Does.Contain("legacyContentGroup: {fileID: 2300000027}"));
            Assert.That(serializedForward, Does.Contain("canonicalPassage: {fileID: 4100000025}"));
            Assert.That(serializedReverse, Does.Contain("canonicalPassage: {fileID: 4100000026}"));
            foreach (string document in new[] { serializedForward, serializedReverse })
            {
                Assert.That(document, Does.Contain("navigationManager: {fileID: 1878886997}"));
                Assert.That(document, Does.Contain("doorOpenAudioSource: {fileID: 2201000013}"));
            ...
            Assert.That(serializedForwardPassage, Does.Contain("sourceRoomView: {fileID: 4100000007}"));
            Assert.That(serializedForwardPassage, Does.Contain("reversePassage: {fileID: 4100000026}"));
            Assert.That(serializedForwardPassage, Does.Contain(
            ...
            Assert.That(serializedReversePassage, Does.Contain("sourceRoomView: {fileID: 4100000009}"));
            Assert.That(serializedReversePassage, Does.Contain("reversePassage: {fileID: 4100000025}"));
            Assert.That(serializedReversePassage, Does.Contain(
                "  approachAnchor:\n    logicalPosition: {x: 4.2, y: -3.3}"));
            Assert.That(serializedReversePassage, Does.Contain(
                "  arrivalAnchor:\n    logicalPosition: {x: 7, y: -2.8}"));
            Assert.That(serializedReversePassage, Does.Contain("anchorMigrationStage: 2"));
            foreach (string fileId in new[] { "4100000009", "4100000025", "4100000026" })
            {
                Assert.That(serializedGameRoot, Does.Contain($"  - {{fileID: {fileId}}}"));
            }
            ...
            for (int sizeIndex = 0; sizeIndex < renderedSizes.Length; sizeIndex++)
            {
                ...
                IEnumerator routine = ObserveDoorRoundTrip(
                    ...
                    includeNearRoundTrip: sizeIndex == 0,
                    maximumZoom: false,
                ...
                AssertButlersBilliardAuthoredObservationStructure(
                    observation,
                    sizeIndex == 0,
                    $"authored {renderedSize.x}x{renderedSize.y}");
                AssertVector2Within(observation.ForwardDispatch, pantryAnchorCandidate, 0.0001f,
                    $"{renderedSize.x}x{renderedSize.y} Pantry authored dispatch");
                AssertVector2Within(observation.ForwardProductionApproach, pantryAnchorCandidate, 0.0001f,
                    $"{renderedSize.x}x{renderedSize.y} Pantry production approach");
                AssertVector2Within(observation.ForwardArrival, serviceAnchorCandidate, 0.0001f,
                    $"{renderedSize.x}x{renderedSize.y} Service authored arrival");
                AssertVector2Within(observation.ReverseDispatch, serviceAnchorCandidate, 0.0001f,
                    $"{renderedSize.x}x{renderedSize.y} Service authored dispatch");
                AssertVector2Within(observation.ReverseProductionApproach, serviceAnchorCandidate, 0.0001f,
                    $"{renderedSize.x}x{renderedSize.y} Service production approach");
                AssertVector2Within(observation.ReverseArrival, pantryAnchorCandidate, 0.0001f,
                    $"{renderedSize.x}x{renderedSize.y} Pantry authored arrival");
                if (sizeIndex == 0)
                {
                    AssertVector2Within(observation.NearForwardArrival, serviceAnchorCandidate, 0.0001f,
                        "primary near Service authored arrival");
                    AssertVector2Within(observation.NearReverseArrival, pantryAnchorCandidate, 0.0001f,
                        "primary near Pantry authored arrival");
                }
                string observationLine =
                    $"[ButlersServiceAuthoredAspect] viewport={renderedSize.x}x{renderedSize.y} " +
                    FormatDoorRoundTripObservation(observation, includeNear: sizeIndex == 0);
                observationProfileLines.Add(observationLine);
                Debug.Log(observationLine);
                IEnumerator candidateRoutine = AssertGroup07AnchorCandidatesAndReturnToPantry(
                    navigation,
                    player,
                    cameraManager,
                    forward,
                    reverse,
                    pantryAnchorCandidate,
                    serviceAnchorCandidate,
                    maximumZoom: false,
                    $"{renderedSize.x}x{renderedSize.y}");
                while (candidateRoutine.MoveNext())
                {
                    yield return candidateRoutine.Current;
                }
            }
            Assert.That(primaryEvents, Is.EqualTo(new[]
            {
            ...
            AssertButlersBilliardAuthoredObservationStructure(
                maximum,
                includeNear: false,
                "authored maximum zoom");
            AssertVector2Within(maximum.ForwardDispatch, pantryAnchorCandidate, 0.0001f,
                "maximum Pantry authored dispatch");
            AssertVector2Within(maximum.ForwardProductionApproach, pantryAnchorCandidate, 0.0001f,
                "maximum Pantry production approach");
            AssertVector2Within(maximum.ForwardArrival, serviceAnchorCandidate, 0.0001f,
                "maximum Service authored arrival");
            AssertVector2Within(maximum.ReverseDispatch, serviceAnchorCandidate, 0.0001f,
                "maximum Service authored dispatch");
            AssertVector2Within(maximum.ReverseProductionApproach, serviceAnchorCandidate, 0.0001f,
                "maximum Service production approach");
            AssertVector2Within(maximum.ReverseArrival, pantryAnchorCandidate, 0.0001f,
                "maximum Pantry authored arrival");
            string maximumObservationLine =
                $"[ButlersServiceAuthoredMaximumZoom] viewport={Screen.width}x{Screen.height} " +
                $"zoom={cameraManager.maxRoomZoom:0.###} " +
                FormatDoorRoundTripObservation(maximum, includeNear: false);
            observationProfileLines.Add(maximumObservationLine);
            Debug.Log(maximumObservationLine);
            IEnumerator maximumCandidateRoutine = AssertGroup07AnchorCandidatesAndReturnToPantry(
                navigation,
                player,
                cameraManager,
                forward,
                ...
                maximumZoom: true,
                "2560x1080-max");
            while (maximumCandidateRoutine.MoveNext())
            {
                yield return maximumCandidateRoutine.Current;
            }

            string profileObservationLine =
                $"[ButlersServiceAuthoredProfile] forwardGeometry={FormatVector(forwardRect.anchoredPosition)}/" +
                $"{FormatVector(forwardRect.sizeDelta)} reverseGeometry={FormatVector(reverseRect.anchoredPosition)}/" +
                $"{FormatVector(reverseRect.sizeDelta)} profiles=invariant callers=bound canonical=authored " +
                $"blockers=0/2";
            observationProfileLines.Add(profileObservationLine);
            Debug.Log(profileObservationLine);
            Assert.That(observationProfileLines, Has.Count.EqualTo(6));
            ...
            Assert.That(observationProfile.EndsWith("\n", System.StringComparison.Ordinal), Is.False,
                "The six-line Group 07 authored fingerprint must not include a trailing newline.");
            Assert.That(ComputeSha256(observationProfile), Is.EqualTo(AuthoredObservationSha256));
            Debug.Log($"[ButlersServiceAuthoredSha256] {ComputeSha256(observationProfile)}");
        }
        finally
        {
            if (forward != null)
    ...
    [UnityTest]
    public IEnumerator ServiceCorridorKitchenPassagesUseRoomViewLocalAnchorsAcrossRenderedAspects()
    {
        const string ServiceCorridor = "Service Corridor";
        ...
        try
        {
            ...
            Assert.That(navigation.CurrentRoom, Is.EqualTo(ServiceCorridor));
            forward = RequireSceneObject<DoorTriggerNavigation>("DoorTrigger_ServiceCorridor_Kitchen");
            reverse = RequireSceneObject<DoorTriggerNavigation>("DoorTrigger_Kitchen_ServiceCorridor");
            CanonicalPassage forwardPassage = forward.GetComponent<CanonicalPassage>();
            CanonicalPassage reversePassage = reverse.GetComponent<CanonicalPassage>();
            Assert.That(forwardPassage, Is.Not.Null);
            Assert.That(reversePassage, Is.Not.Null);
            ...
            Assert.That(forward.DestinationRoom, Is.EqualTo(Kitchen));
            Assert.That(reverse.SourceRoom, Is.EqualTo(Kitchen));
            Assert.That(reverse.DoorName, Is.EqualTo("Kitchen_ServiceCorridor"));
            ...
            Assert.That(reversePassage.ReversePassage, Is.SameAs(forwardPassage));
            Assert.That(forwardPassage.SourceRoomView.Definition.StableId,
                Is.EqualTo("room.service-corridor"));
            Assert.That(reversePassage.SourceRoomView.Definition.StableId, Is.EqualTo("room.kitchen"));
            Assert.That(forwardPassage.ApproachAnchor.CoordinateSpace,
                Is.EqualTo(PassageAnchorCoordinateSpace.RoomViewLocal));
            Assert.That(forwardPassage.ArrivalAnchor.CoordinateSpace,
                Is.EqualTo(PassageAnchorCoordinateSpace.RoomViewLocal));
            Assert.That(reversePassage.ApproachAnchor.CoordinateSpace,
                Is.EqualTo(PassageAnchorCoordinateSpace.RoomViewLocal));
            Assert.That(reversePassage.ArrivalAnchor.CoordinateSpace,
                Is.EqualTo(PassageAnchorCoordinateSpace.RoomViewLocal));
            AssertVector2Within(forwardPassage.ApproachAnchor.RoomViewLocalPosition,
                serviceAnchor, 0.0001f, "Service-to-Kitchen room-local approach");
            AssertVector2Within(forwardPassage.ArrivalAnchor.RoomViewLocalPosition,
                kitchenAnchor, 0.0001f, "Service-to-Kitchen room-local arrival");
            AssertVector2Within(reversePassage.ApproachAnchor.RoomViewLocalPosition,
                kitchenAnchor, 0.0001f, "Kitchen-to-Service room-local approach");
            AssertVector2Within(reversePassage.ArrivalAnchor.RoomViewLocalPosition,
                serviceAnchor, 0.0001f, "Kitchen-to-Service room-local arrival");
            Assert.That(GetPrivateValue<float>(forward, "maxPlayerScreenDistance"), Is.EqualTo(145f));
            Assert.That(GetPrivateValue<float>(reverse, "maxPlayerScreenDistance"), Is.EqualTo(145f));

            RoomContentGroup serviceContent = FindInActiveScene<RoomContentGroup>()
                .Single(item => item.RoomName == ServiceCorridor);
            RoomContentGroup kitchenContent = FindInActiveScene<RoomContentGroup>()
                .Single(item => item.RoomName == Kitchen);
            Assert.That(forwardPassage.SourceRoomView.LegacyContentGroup, Is.SameAs(serviceContent));
            Assert.That(reversePassage.SourceRoomView.LegacyContentGroup, Is.SameAs(kitchenContent));
            Assert.That(RequireOnlyActiveRoom(ServiceCorridor), Is.SameAs(serviceContent));
            Assert.That(FindInActiveScene<ObjectMovementBlocker2D>().Count(blocker =>
                blocker.GetComponentInParent<RoomContentGroup>(true) == serviceContent), Is.EqualTo(2));
            Assert.That(FindInActiveScene<ObjectMovementBlocker2D>().Count(blocker =>
                blocker.GetComponentInParent<RoomContentGroup>(true) == kitchenContent), Is.EqualTo(1));
            ...
            List<Chateau.Architecture.ChateauBehaviour> sceneBehaviours =
                GetPrivateField<List<Chateau.Architecture.ChateauBehaviour>>(gameRoot, "sceneBehaviours");
            Assert.That(sceneBehaviours.Count(item => item == reversePassage.SourceRoomView), Is.EqualTo(1));
            Assert.That(sceneBehaviours.Count(item => item == forwardPassage), Is.EqualTo(1));
            Assert.That(sceneBehaviours.Count(item => item == reversePassage), Is.EqualTo(1));
            ...
            foreach (Vector2Int renderedSize in renderedSizes)
            {
                yield return SetAndWaitForRenderedGameViewResolution(
                    (uint)renderedSize.x,
                    (uint)renderedSize.y);
                cameraManager.ResetRoomLookForPreview();
                yield return WaitForSettledLayout();
                IEnumerator routine = AssertServiceKitchenRoomViewLocalRoundTrip(
                    navigation,
                    player,
                    cameraManager,
                    forward,
            ...
            IEnumerator maximumRoutine = AssertServiceKitchenRoomViewLocalRoundTrip(
                navigation,
                player,
                cameraManager,
                forward,
            ...
            string projectRoot = System.IO.Directory.GetParent(Application.dataPath).FullName;
            string sceneText = System.IO.File.ReadAllText(System.IO.Path.Combine(projectRoot, GameplayScenePath));
            string serializedForward = RequireSerializedUnityDocument(sceneText, "4100000027");
            string serializedReverse = RequireSerializedUnityDocument(sceneText, "4100000028");
            Assert.That(serializedForward, Does.Contain("sourceRoomView: {fileID: 4100000009}"));
            Assert.That(serializedReverse, Does.Contain("sourceRoomView: {fileID: 4100000010}"));
            Assert.That(serializedForward, Does.Contain("reversePassage: {fileID: 4100000028}"));
            Assert.That(serializedReverse, Does.Contain("reversePassage: {fileID: 4100000027}"));
            foreach (string document in new[] { serializedForward, serializedReverse })
            {
                Assert.That(System.Text.RegularExpressions.Regex.Matches(
                    document,
                    "coordinateSpace: 1").Count, Is.EqualTo(2));
                Assert.That(System.Text.RegularExpressions.Regex.Matches(
                    document,
                    "logicalPosition: \\{x: 0, y: 0\\}").Count, Is.EqualTo(2));
                Assert.That(document, Does.Contain("anchorMigrationStage: 2"));
            }
            Assert.That(serializedForward, Does.Contain(
                "roomViewLocalPosition: {x: 589.9897, y: -419.25894}"));
            Assert.That(serializedForward, Does.Contain(
                "roomViewLocalPosition: {x: -478.36285, y: -156.76599}"));
            Assert.That(serializedReverse, Does.Contain(
                "roomViewLocalPosition: {x: -478.36285, y: -156.76599}"));
            Assert.That(serializedReverse, Does.Contain(
                "roomViewLocalPosition: {x: 589.9897, y: -419.25894}"));
        }
    ...
    [UnityTest]
    public IEnumerator ServiceCorridorChapelPassagesUseRoomViewLocalAnchorsAcrossRenderedAspects()
    {
        const string ServiceCorridor = "Service Corridor";
        const string Chapel = "Chapel";
        const string LegacyObservationSha256 =
            "9574b82e5099f96bdaef11dd20bdbab386f5b54baf9ad06278b40af7a58449b2";
        const string AuthoredObservationSha256 =
            "bff3067e14fd0710f8de6dd4f7e4434d9025a2b7593adf3cd54571020a622475";
        Vector2 serviceAnchor = new Vector2(-133.2642f, -171.8258f);
        Vector2 chapelAnchor = new Vector2(461.4019f, -190.7613f);
        Vector2 requestedForwardStart = new Vector2(4.2f, -3.3f);
        Vector2 requestedReverseStart = new Vector2(0f, -2f);
        List<string> observationProfileLines = new List<string>(6);
        ...
        try
        {
            ...
            })
            {
                CanonicalPassage setupPassage =
                    RequireSceneObject<DoorTriggerNavigation>(setupTriggerName).GetComponent<CanonicalPassage>();
                Assert.That(setupPassage, Is.Not.Null);
                Assert.That(setupPassage.AnchorMigrationStage,
                    Is.EqualTo(PassageAnchorMigrationStage.AuthoredAnchors));
                Assert.That(navigationFacade.TryTraverse(setupPassage), Is.True);
                yield return WaitForSettledLayout();
            }
            ...
            Assert.That(navigation.CurrentRoom, Is.EqualTo(ServiceCorridor));
            forward = RequireSceneObject<DoorTriggerNavigation>("DoorTrigger_ServiceCorridor_Chapel");
            reverse = RequireSceneObject<DoorTriggerNavigation>("DoorTrigger_Chapel_ServiceCorridor");
            CanonicalPassage forwardPassage = forward.GetComponent<CanonicalPassage>();
            CanonicalPassage reversePassage = reverse.GetComponent<CanonicalPassage>();
            Assert.That(forward.GetComponents<Component>(), Has.Length.EqualTo(5));
            Assert.That(reverse.GetComponents<Component>(), Has.Length.EqualTo(5));
            ...
            Assert.That(forward.DestinationRoom, Is.EqualTo(Chapel));
            Assert.That(reverse.SourceRoom, Is.EqualTo(Chapel));
            Assert.That(reverse.DoorName, Is.EqualTo("Chapel_ServiceCorridor"));
            Assert.That(reverse.DestinationRoom, Is.EqualTo(ServiceCorridor));
            Assert.That(forwardPassage.Definition.StableId,
            ...
            Assert.That(forwardPassage.SourceRoomView.Definition.StableId,
                Is.EqualTo("room.service-corridor"));
            Assert.That(reversePassage.SourceRoomView.Definition.StableId,
                Is.EqualTo("room.chapel"));
            Assert.That(forwardPassage.ApproachAnchor.CoordinateSpace,
                Is.EqualTo(PassageAnchorCoordinateSpace.RoomViewLocal));
            Assert.That(forwardPassage.ArrivalAnchor.CoordinateSpace,
                Is.EqualTo(PassageAnchorCoordinateSpace.RoomViewLocal));
            ...
            Assert.That(reversePassage.ArrivalAnchor.CoordinateSpace,
                Is.EqualTo(PassageAnchorCoordinateSpace.RoomViewLocal));
            AssertVector2Within(forwardPassage.ApproachAnchor.RoomViewLocalPosition,
                serviceAnchor, 0.0001f, "Service-to-Chapel room-local approach");
            AssertVector2Within(forwardPassage.ArrivalAnchor.RoomViewLocalPosition,
                chapelAnchor, 0.0001f, "Service-to-Chapel room-local arrival");
            AssertVector2Within(reversePassage.ApproachAnchor.RoomViewLocalPosition,
                chapelAnchor, 0.0001f, "Chapel-to-Service room-local approach");
            AssertVector2Within(reversePassage.ArrivalAnchor.RoomViewLocalPosition,
                serviceAnchor, 0.0001f, "Chapel-to-Service room-local arrival");
            Chateau.Architecture.ValidationReport forwardReport = new Chateau.Architecture.ValidationReport();
            Chateau.Architecture.ValidationReport reverseReport = new Chateau.Architecture.ValidationReport();
            forwardPassage.ValidateConfiguration(forwardReport);
            reversePassage.ValidateConfiguration(reverseReport);
            ...
            Assert.That(forwardRect, Is.Not.Null);
            Assert.That(reverseRect, Is.Not.Null);
            AssertVector2Within(forwardRect.anchoredPosition,
                new Vector2(-204.79747f, 53.84522f), 0.0001f,
                "Service-to-Chapel trigger position");
            AssertVector2Within(forwardRect.sizeDelta,
                new Vector2(135.9724f, 358.7524f), 0.0001f,
                "Service-to-Chapel trigger size");
            AssertVector2Within(reverseRect.anchoredPosition,
                new Vector2(501.5676f, -28.297081f), 0.0001f,
                "Chapel-to-Service trigger position");
            AssertVector2Within(reverseRect.sizeDelta,
                new Vector2(66.7197f, 293.4059f), 0.0001f,
                "Chapel-to-Service trigger size");
            Assert.That(forwardRect.parent.name, Is.EqualTo("Doors"));
            Assert.That(reverseRect.parent.name, Is.EqualTo("Doors"));

            RoomContentGroup serviceContent = FindInActiveScene<RoomContentGroup>()
                .Single(item => item.RoomName == ServiceCorridor);
            RoomContentGroup chapelContent = FindInActiveScene<RoomContentGroup>()
                .Single(item => item.RoomName == Chapel);
            Assert.That(RequireOnlyActiveRoom(ServiceCorridor), Is.SameAs(serviceContent));
            Assert.That(forward.GetComponentInParent<RoomContentGroup>(true), Is.SameAs(serviceContent));
            Assert.That(reverse.GetComponentInParent<RoomContentGroup>(true), Is.SameAs(chapelContent));
            Assert.That(serviceContent.GetComponent<CanonicalRoomView>(), Is.Not.Null);
            Assert.That(chapelContent.GetComponent<CanonicalRoomView>(), Is.SameAs(reversePassage.SourceRoomView));
            Assert.That(serviceContent.transform.childCount, Is.EqualTo(41));
            Assert.That(chapelContent.transform.childCount, Is.EqualTo(14));
            Assert.That(serviceContent.transform.Cast<Transform>().Single(child => child.name == "Doors").childCount,
                Is.EqualTo(5));
            Assert.That(chapelContent.transform.Cast<Transform>().Single(child => child.name == "Doors").childCount,
                Is.EqualTo(1));
            Assert.That(GetPrivateField<Texture>(serviceContent, "roomBackgroundTexture"), Is.Not.Null);
            Assert.That(GetPrivateField<Texture>(chapelContent, "roomBackgroundTexture"), Is.Not.Null);
            Assert.That(GetPrivateField<Texture>(serviceContent, "roomBackgroundTexture"),
                Is.Not.SameAs(GetPrivateField<Texture>(chapelContent, "roomBackgroundTexture")));
            Assert.That(serviceContent.PerspectiveProfile, Is.Null);
            Assert.That(chapelContent.PerspectiveProfile, Is.Null);

            Dictionary<string, string> expectedServiceBlockers = new Dictionary<string, string>
            {
            ...
            Dictionary<string, string> expectedChapelBlockers = new Dictionary<string, string>
            {
                { "PlayerBlocker_chapel_bench_0", "chapel_bench_0" },
                { "PlayerBlocker_chapel_bench_right1_0", "chapel_bench_right1_0" },
                { "PlayerBlocker_chapel_bench_right_2_0", "chapel_bench_right_2_0" }
            };
            ObjectMovementBlocker2D[] serviceBlockers = FindInActiveScene<ObjectMovementBlocker2D>()
                .Where(blocker => blocker.GetComponentInParent<RoomContentGroup>(true) == serviceContent)
                .ToArray();
            ObjectMovementBlocker2D[] chapelBlockers = FindInActiveScene<ObjectMovementBlocker2D>()
                .Where(blocker => blocker.GetComponentInParent<RoomContentGroup>(true) == chapelContent)
                .ToArray();
            CollectionAssert.AreEquivalent(expectedServiceBlockers.Keys, serviceBlockers.Select(item => item.name));
            CollectionAssert.AreEquivalent(expectedChapelBlockers.Keys, chapelBlockers.Select(item => item.name));
            foreach (ObjectMovementBlocker2D blocker in serviceBlockers.Concat(chapelBlockers))
            {
                Dictionary<string, string> expected = blocker.SourceRoomName == ServiceCorridor
                    ? expectedServiceBlockers
                    : expectedChapelBlockers;
                Assert.That(expected.ContainsKey(blocker.name), Is.True, blocker.name);
                Assert.That(blocker.SourceObjectName, Is.EqualTo(expected[blocker.name]), blocker.name);
                Assert.That(blocker.SourceObject, Is.Not.Null, blocker.name);
                Assert.That(blocker.SourceObject.name, Is.EqualTo(expected[blocker.name]), blocker.name);
                Assert.That(blocker.BlockingCollider, Is.TypeOf<PolygonCollider2D>(), blocker.name);
                Assert.That(blocker.BlockingCollider.isTrigger, Is.True, blocker.name);
                Assert.That(blocker.GetComponents<Component>(), Has.Length.EqualTo(3), blocker.name);
            }

            foreach (string cutoutName in expectedChapelBlockers.Values.Concat(new[]
            {
                "chapel_char_right2_0",
                "chapel_right_3_0",
                "chapel_seat_left3_0"
            }))
            {
                Transform cutout = FindInActiveScene<Transform>().Single(item => item.name == cutoutName);
                Assert.That(cutout.parent, Is.SameAs(chapelContent.transform), cutoutName);
                Assert.That(cutout.GetComponent<SpriteRenderer>(), Is.Not.Null, cutoutName);
            }

            PolygonCollider2D serviceBoundary = serviceContent.GetComponentsInChildren<PolygonCollider2D>(true)
                .Single(collider => collider.name == "PlayerBoundary" &&
                    collider.transform.parent == serviceContent.transform);
            PolygonCollider2D chapelBoundary = chapelContent.GetComponentsInChildren<PolygonCollider2D>(true)
                .Single(collider => collider.name == "PlayerBoundary" &&
                    collider.transform.parent == chapelContent.transform);
            Assert.That(serviceBoundary.isTrigger, Is.False);
            Assert.That(chapelBoundary.isTrigger, Is.False);
            Assert.That(serviceBoundary.pathCount, Is.EqualTo(1));
            Assert.That(chapelBoundary.pathCount, Is.EqualTo(1));
            Assert.That(serviceBoundary.GetPath(0), Has.Length.EqualTo(14));
            Assert.That(chapelBoundary.GetPath(0), Has.Length.EqualTo(18));

            RectTransform chapelHideTransform = FindInActiveScene<Transform>()
                .Single(item => item.name == "Ch2_Hide_Guest06") as RectTransform;
            Assert.That(chapelHideTransform, Is.Not.Null);
            Assert.That(chapelHideTransform.parent, Is.SameAs(chapelContent.transform));
            AssertVector2Within(chapelHideTransform.anchoredPosition,
                new Vector2(-598f, -271f), 0.0001f, "Chapel Chapter 2 hide anchor");
            RoomAnchor chapelHideAnchor = chapelHideTransform.GetComponent<RoomAnchor>();
            Assert.That(chapelHideAnchor, Is.Not.Null);
            Assert.That(chapelHideAnchor.AnchorId, Is.EqualTo("Ch2_Hide_Guest06"));
            Assert.That(chapelHideAnchor.RoomId, Is.EqualTo(Chapel));
            Assert.That(chapelHideTransform.gameObject.activeInHierarchy, Is.False);
            ...
            string serializedForward = RequireSerializedUnityDocument(sceneText, "2300000169");
            string serializedReverse = RequireSerializedUnityDocument(sceneText, "2300000179");
            string serializedForwardPassage = RequireSerializedUnityDocument(sceneText, "4100000030");
            string serializedReversePassage = RequireSerializedUnityDocument(sceneText, "4100000031");
            string serializedForwardRect = RequireSerializedUnityDocument(sceneText, "2300000166", "224");
            string serializedReverseRect = RequireSerializedUnityDocument(sceneText, "2300000176", "224");
            string serializedServiceRoot = RequireSerializedUnityDocument(sceneText, "2300000025", "1");
            string serializedChapelRoot = RequireSerializedUnityDocument(sceneText, "2300000030", "1");
            string serializedChapelContent = RequireSerializedUnityDocument(sceneText, "2300000032");
            string serializedGameRoot = RequireSerializedUnityDocument(sceneText, "1878886998");
            Assert.That(System.Text.RegularExpressions.Regex.Matches(
                sceneText,
                "^--- !u!",
                System.Text.RegularExpressions.RegexOptions.Multiline).Count, Is.EqualTo(6038));
            Assert.That(System.Text.RegularExpressions.Regex.Matches(
                databaseText,
                "^  - \\{fileID: 11400000, guid:",
                System.Text.RegularExpressions.RegexOptions.Multiline).Count, Is.EqualTo(39));
            Assert.That(System.Text.RegularExpressions.Regex.Matches(
                serializedGameRoot,
                "(?m)^  - \\{fileID:").Count, Is.EqualTo(48));
            Assert.That(System.Text.RegularExpressions.Regex.Matches(
                serializedServiceRoot,
                "(?m)^  - component:").Count, Is.EqualTo(3));
            Assert.That(System.Text.RegularExpressions.Regex.Matches(
                serializedChapelRoot,
                "(?m)^  - component:").Count, Is.EqualTo(3));
            Assert.That(serializedChapelContent, Does.Contain("roomName: Chapel"));
            Assert.That(serializedChapelContent, Does.Contain(
                "roomBackgroundTexture: {fileID: 2800000, guid: d40ce95937763bcddb24975fe9c6ec20, type: 3}"));
            Assert.That(serializedChapelContent, Does.Contain("perspectiveProfile: {fileID: 0}"));
            Assert.That(serializedForwardRect, Does.Contain("m_Father: {fileID: 2300000029}"));
            Assert.That(serializedForwardRect, Does.Contain(
                "m_AnchoredPosition: {x: -204.79747, y: 53.84522}"));
            Assert.That(serializedForwardRect, Does.Contain(
                "m_SizeDelta: {x: 135.9724, y: 358.7524}"));
            Assert.That(serializedReverseRect, Does.Contain("m_Father: {fileID: 2300000034}"));
            Assert.That(serializedReverseRect, Does.Contain(
                "m_AnchoredPosition: {x: 501.5676, y: -28.297081}"));
            Assert.That(serializedReverseRect, Does.Contain(
                "m_SizeDelta: {x: 66.7197, y: 293.4059}"));
            foreach (string triggerDocument in new[] { serializedForward, serializedReverse })
            {
                Assert.That(triggerDocument, Does.Contain("triggerKind: 0"));
                Assert.That(triggerDocument, Does.Contain("stairwayDirection: 0"));
                Assert.That(triggerDocument, Does.Contain("navigationManager: {fileID: 1878886997}"));
                Assert.That(triggerDocument, Does.Contain("doorOpenAudioSource: {fileID: 2201000013}"));
                Assert.That(triggerDocument, Does.Contain("player: {fileID: 81962843}"));
                Assert.That(triggerDocument, Does.Contain(
                    "doorOpenSoundCatalog: {fileID: 11400000, guid: 9a77542e25184fbc945d6a79f77007e7, type: 2}"));
                Assert.That(triggerDocument, Does.Contain("canonicalPassage: {fileID: 41000000"));
                Assert.That(triggerDocument, Does.Contain("maxPlayerScreenDistance: 145"));
            }
            Assert.That(serializedForward, Does.Contain("sourceRoom: Service Corridor"));
            Assert.That(serializedForward, Does.Contain("doorName: ServiceCorridor_Chapel"));
            Assert.That(serializedForward, Does.Contain("destinationRoom: Chapel"));
            Assert.That(serializedForward, Does.Contain("canonicalPassage: {fileID: 4100000030}"));
            Assert.That(serializedReverse, Does.Contain("sourceRoom: Chapel"));
            Assert.That(serializedReverse, Does.Contain("doorName: Chapel_ServiceCorridor"));
            Assert.That(serializedReverse, Does.Contain("destinationRoom: Service Corridor"));
            Assert.That(serializedReverse, Does.Contain("canonicalPassage: {fileID: 4100000031}"));
            Assert.That(serializedForwardPassage, Does.Contain(
                "definition: {fileID: 11400000, guid: fc2a0af2de3f4ade831c53f64fe0271b, type: 2}"));
            Assert.That(serializedForwardPassage, Does.Contain("sourceRoomView: {fileID: 4100000009}"));
            Assert.That(serializedForwardPassage, Does.Contain("reversePassage: {fileID: 4100000031}"));
            Assert.That(serializedReversePassage, Does.Contain(
                "definition: {fileID: 11400000, guid: 47e06869bf2b47a2980b0d02a53ee1df, type: 2}"));
            Assert.That(serializedReversePassage, Does.Contain("sourceRoomView: {fileID: 4100000029}"));
            Assert.That(serializedReversePassage, Does.Contain("reversePassage: {fileID: 4100000030}"));
            foreach (string passageDocument in new[] { serializedForwardPassage, serializedReversePassage })
            {
                Assert.That(System.Text.RegularExpressions.Regex.Matches(
                    passageDocument,
                    "coordinateSpace: 1").Count, Is.EqualTo(2));
            ...
            Assert.That(serializedForwardPassage, Does.Contain(
                "roomViewLocalPosition: {x: -133.2642, y: -171.8258}"));
            Assert.That(serializedForwardPassage, Does.Contain(
                "roomViewLocalPosition: {x: 461.4019, y: -190.7613}"));
            Assert.That(serializedReversePassage, Does.Contain(
                "roomViewLocalPosition: {x: 461.4019, y: -190.7613}"));
            Assert.That(serializedReversePassage, Does.Contain(
                "roomViewLocalPosition: {x: -133.2642, y: -171.8258}"));
            ...
            Assert.That(AssetDatabase.AssetPathToGUID(
                ...
                Is.EqualTo("e3102dbfecc44551b6443ca88625a924"));
            Assert.That(AssetDatabase.LoadMainAssetAtPath(
                "Assets/_Chateau/Data/World/Passages/Passage_ServiceCorridor_Chapel.asset"), Is.Not.Null);
            Assert.That(AssetDatabase.LoadMainAssetAtPath(
                "Assets/_Chateau/Data/World/Passages/Passage_Chapel_ServiceCorridor.asset"), Is.Not.Null);
            ...
            UnityEngine.Events.UnityAction<string> recordRoomChanged = room =>
            {
                primaryEvents.Add(
                    ...
                    (GetPrivateStaticField<AudioSource>(typeof(DoorTriggerNavigation),
                        "activeNavigationAudioSource") == passageAudioSource
                        ? "audio-started"
                        : "audio-idle"));
                Assert.That(chapelHideTransform.gameObject.activeInHierarchy,
                    Is.EqualTo(room == Chapel),
                    $"Chapel hide anchor visibility after entering {room}");
            };
            ...
            for (int sizeIndex = 0; sizeIndex < renderedSizes.Length; sizeIndex++)
            {
                ...
                AssertButlersBilliardAuthoredObservationStructure(
                    observation,
                    includeNear: sizeIndex == 0,
                    $"Group09 legacy {renderedSize.x}x{renderedSize.y}");
                AssertVector2Within(observation.ForwardDispatch, observation.ForwardProductionApproach, 0.0001f,
                    $"{renderedSize.x}x{renderedSize.y} Service canonical dispatch");
                AssertVector2Within(observation.ReverseDispatch, observation.ReverseProductionApproach, 0.0001f,
                    $"{renderedSize.x}x{renderedSize.y} Chapel canonical dispatch");
                Assert.That(observation.HasForwardNullRoomViewLocal, Is.True,
                    $"{renderedSize.x}x{renderedSize.y} Service compatibility sample must map to native room space.");
                Assert.That(observation.HasReverseNullRoomViewLocal, Is.True,
                    $"{renderedSize.x}x{renderedSize.y} Chapel compatibility sample must map to native room space.");
                AssertFinite(observation.ForwardNullRoomViewLocal,
                    $"{renderedSize.x}x{renderedSize.y} Service native compatibility sample");
                AssertFinite(observation.ReverseNullRoomViewLocal,
                    $"{renderedSize.x}x{renderedSize.y} Chapel native compatibility sample");
                Assert.That(navigation.CurrentRoom, Is.EqualTo(ServiceCorridor));
                Assert.That(RequireOnlyActiveRoom(ServiceCorridor), Is.SameAs(serviceContent));
                Assert.That(serviceContent.GetComponent<CanonicalRoomView>().IsVisible, Is.True);
                Assert.That(chapelContent.gameObject.activeInHierarchy, Is.False);
                string observationLine =
                    $"[ServiceChapelAuthoredAspect] viewport={renderedSize.x}x{renderedSize.y} " +
                    (sizeIndex == 0 ? $"events={string.Join("|", primaryEvents)} " : string.Empty) +
                    $"serviceNullLocal={FormatVector(observation.ForwardNullRoomViewLocal)} " +
                    $"chapelNullLocal={FormatVector(observation.ReverseNullRoomViewLocal)} " +
                    FormatDoorRoundTripObservation(observation, includeNear: sizeIndex == 0);
                observationProfileLines.Add(observationLine);
                Debug.Log(observationLine);
            }
            ...
            Assert.That(primaryEvents, Is.EqualTo(new[]
            {
                $"arrived:{ServiceCorridor}:audio-idle",
                $"movement-stopped:{ServiceCorridor}:audio-idle",
                $"room-changed:{Chapel}:audio-started",
                $"arrived:{Chapel}:audio-idle",
                $"movement-stopped:{Chapel}:audio-idle",
                $"room-changed:{ServiceCorridor}:audio-started",
                $"room-changed:{Chapel}:audio-started",
                $"room-changed:{ServiceCorridor}:audio-started"
            }));
            ...
            AssertButlersBilliardAuthoredObservationStructure(
                maximum,
                includeNear: false,
                "Group09 legacy maximum zoom");
            AssertVector2Within(maximum.ForwardDispatch, maximum.ForwardProductionApproach, 0.0001f,
                "maximum Service canonical dispatch");
            AssertVector2Within(maximum.ReverseDispatch, maximum.ReverseProductionApproach, 0.0001f,
                "maximum Chapel canonical dispatch");
            Assert.That(maximum.HasForwardNullRoomViewLocal, Is.True);
            Assert.That(maximum.HasReverseNullRoomViewLocal, Is.True);
            AssertFinite(maximum.ForwardNullRoomViewLocal, "maximum Service native compatibility sample");
            AssertFinite(maximum.ReverseNullRoomViewLocal, "maximum Chapel native compatibility sample");
            string maximumObservationLine =
                $"[ServiceChapelAuthoredMaximumZoom] viewport={Screen.width}x{Screen.height} " +
                $"zoom={cameraManager.maxRoomZoom:0.###} " +
                $"serviceNullLocal={FormatVector(maximum.ForwardNullRoomViewLocal)} " +
                $"chapelNullLocal={FormatVector(maximum.ReverseNullRoomViewLocal)} " +
                FormatDoorRoundTripObservation(maximum, includeNear: false);
            observationProfileLines.Add(maximumObservationLine);
            Debug.Log(maximumObservationLine);

            string profileObservationLine =
                $"[ServiceChapelAuthoredProfile] forwardGeometry={FormatVector(forwardRect.anchoredPosition)}/" +
                $"{FormatVector(forwardRect.sizeDelta)} reverseGeometry={FormatVector(reverseRect.anchoredPosition)}/" +
                $"{FormatVector(reverseRect.sizeDelta)} callers=bound serializedDependencies=bound " +
                $"blockers={serviceBlockers.Length}/{chapelBlockers.Length} boundaries=14/18 " +
                $"chapelHide={FormatVector(chapelHideTransform.anchoredPosition)}";
            observationProfileLines.Add(profileObservationLine);
            Debug.Log(profileObservationLine);
            Assert.That(observationProfileLines, Has.Count.EqualTo(6));
            string observationProfile = string.Join("\n", observationProfileLines);
            Assert.That(observationProfile.EndsWith("\n", System.StringComparison.Ordinal), Is.False,
                "The six-line Group 09 authored fingerprint must not include a trailing newline.");
            string actualSha256 = ComputeSha256(observationProfile);
            Debug.Log($"[ServiceChapelAuthoredSha256] {actualSha256}");
            Assert.That(LegacyObservationSha256,
                Is.EqualTo("9574b82e5099f96bdaef11dd20bdbab386f5b54baf9ad06278b40af7a58449b2"),
                "The reviewed tests-first legacy evidence fingerprint must remain explicit.");
            Assert.That(actualSha256, Is.EqualTo(AuthoredObservationSha256),
                "The reviewed six-line Group 09 authored profile must remain exact.");

            Vector2 serviceAnchorCandidate = serviceAnchor;
            Vector2 chapelAnchorCandidate = chapelAnchor;
            PassageAnchorData serviceAnchorData = new PassageAnchorData();
            PassageAnchorData chapelAnchorData = new PassageAnchorData();
            SetPrivateField(
                serviceAnchorData,
                "coordinateSpace",
                PassageAnchorCoordinateSpace.RoomViewLocal);
            SetPrivateField(serviceAnchorData, "roomViewLocalPosition", serviceAnchorCandidate);
            SetPrivateField(
                chapelAnchorData,
                "coordinateSpace",
                PassageAnchorCoordinateSpace.RoomViewLocal);
            SetPrivateField(chapelAnchorData, "roomViewLocalPosition", chapelAnchorCandidate);
            Vector2Int[] candidateRenderedSizes =
            {
                new Vector2Int(1366, 768),
                new Vector2Int(1440, 1080),
                new Vector2Int(1920, 1080),
            ...
            List<string> candidateProfileLines = new List<string>(candidateRenderedSizes.Length);
            for (int profileIndex = 0; profileIndex < candidateRenderedSizes.Length; profileIndex++)
            {
                Vector2Int renderedSize = candidateRenderedSizes[profileIndex];
                bool maximumZoom = profileIndex == candidateRenderedSizes.Length - 1;
                yield return SetAndWaitForRenderedGameViewResolution(
                    (uint)renderedSize.x,
                    (uint)renderedSize.y);
                cameraManager.ResetRoomLookForPreview();
                ...
                Assert.That(navigation.CurrentRoom, Is.EqualTo(ServiceCorridor));
                Vector2 serviceLogical = AssertUnboundRoomViewLocalCandidate(
                    player,
                    cameraManager,
                    forward,
                    serviceAnchorData,
                    serviceAnchorCandidate,
                    $"{renderedSize.x}x{renderedSize.y}" + (maximumZoom ? " maximum Service" : " Service"));
                Assert.That(player.TryWarpToExact(serviceLogical), Is.True);
                SetPrivateField(forward, "lastPointerActivationFrame", -1);
                forward.ActivateDoor();
                for (int frame = 0; frame < 4 && navigation.CurrentRoom == ServiceCorridor; frame++)
                {
                    yield return null;
                }
                yield return WaitForSettledLayout();
                Assert.That(navigation.CurrentRoom, Is.EqualTo(Chapel));
                if (maximumZoom)
                {
                    ApplyMaximumRoomZoom(cameraManager);
                    yield return WaitForSettledLayout();
                ...
                Vector2 chapelLogical = AssertUnboundRoomViewLocalCandidate(
                    player,
                    cameraManager,
                    reverse,
                    chapelAnchorData,
                    chapelAnchorCandidate,
                    $"{renderedSize.x}x{renderedSize.y}" + (maximumZoom ? " maximum Chapel" : " Chapel"));
                Assert.That(player.TryWarpToExact(chapelLogical), Is.True);
                SetPrivateField(reverse, "lastPointerActivationFrame", -1);
                reverse.ActivateDoor();
                for (int frame = 0; frame < 4 && navigation.CurrentRoom == Chapel; frame++)
                {
                    yield return null;
                }
                yield return WaitForSettledLayout();
                ...
                string candidateLine =
                    $"[ServiceChapelRoomLocalAuthored] profile={renderedSize.x}x{renderedSize.y}" +
                    (maximumZoom ? "-maximum" : "-default") +
                    $" serviceLocal={FormatVector(serviceAnchorCandidate)} " +
                    $"serviceLogical={FormatVector(serviceLogical)} " +
                    $"chapelLocal={FormatVector(chapelAnchorCandidate)} " +
                    $"chapelLogical={FormatVector(chapelLogical)} threshold=145 exact=true";
                candidateProfileLines.Add(candidateLine);
                Debug.Log(candidateLine);
            }
            Assert.That(candidateProfileLines, Has.Count.EqualTo(5));
        }
        finally
        {
            if (forward != null)
    ...
    private static Vector2 AssertUnboundRoomViewLocalCandidate(
        ...
        string label)
    {
        ...
        AssertVector2Within(
            anchor.RoomViewLocalPosition,
            expectedRoomViewLocalPosition,
            0.0001f,
            $"{label} authored native coordinate");
        Assert.That(anchor.TryResolveLogicalPosition(player, out Vector2 logicalPosition), Is.True);
        AssertFinite(logicalPosition, $"{label} resolved logical coordinate");
        ...
        Assert.That(cameraManager.TryGetActiveRoomStageLocalPoint(
            new Vector3(worldPosition.x, worldPosition.y, player.transform.position.z),
            out Vector2 roundTripLocalPosition), Is.True);
        AssertVector2Within(
        ...
        Assert.That(TryEvaluateExactReachableMovementTarget(
            player,
            logicalPosition,
            out PointClickPlayerMovement.MovementTargetQuery movementQuery), Is.True);
        Assert.That(movementQuery.ExactPointWalkable, Is.True, $"{label} must be exactly walkable.");
        Assert.That(movementQuery.HasReachableDestination, Is.True, $"{label} must be path reachable.");
        Assert.That(movementQuery.UsesProjectedDestination, Is.False, $"{label} must not use projection.");
        AssertVector2Within(
            movementQuery.Destination,
            logicalPosition,
            0.0001f,
            $"{label} exact movement destination");
        AssertApproachWithinActivationDistance(trigger, player, logicalPosition, label);
        return logicalPosition;
    }

    private static IEnumerator AssertServiceKitchenRoomViewLocalRoundTrip(
        ...
        Vector2 serviceAnchor,
        Vector2 kitchenAnchor,
        bool maximumZoom,
        string profile)
    {
        const string ServiceCorridor = "Service Corridor";
        ...
        Assert.That(TryWarpToCharacterizedFarStart(
            ...
            forward,
            new Vector2(0f, -2.5f),
            out float serviceStartDistance), Is.True);
        Assert.That(serviceStartDistance, Is.GreaterThan(145f));
        Vector2 serviceLogical = AssertActiveRoomViewLocalAnchor(
            player,
            cameraManager,
            forward,
            forwardPassage,
            ...
            label: $"{profile} Service approach");
        SetPrivateField(forward, "lastPointerActivationFrame", -1);
        forward.ActivateDoor();
        Assert.That(player.HasDestination, Is.True);
        AssertVector2Within(GetPrivateValue<Vector2>(player, "finalDestination"),
            serviceLogical, 0.0001f, $"{profile} Service production approach");
        for (int frame = 0; frame < 180 && navigation.CurrentRoom == ServiceCorridor && player.HasDestination; frame++)
        {
            InvokePrivateMethod(player, "MoveTowardDestination");
            yield return null;
        }
        yield return WaitForSettledLayout();
        Physics2D.SyncTransforms();
        Assert.That(navigation.CurrentRoom, Is.EqualTo(Kitchen));
        Vector2 kitchenLogical = AssertActiveRoomViewLocalAnchor(
            player,
            cameraManager,
            reverse,
            reversePassage,
            ...
            label: $"{profile} Kitchen approach");
        AssertVector2Within(player.LogicalPosition, kitchenLogical, 0.001f,
            $"{profile} Kitchen room-local arrival");
        ...
        Assert.That(TryWarpToCharacterizedFarStart(
            ...
            reverse,
            new Vector2(0f, -2f),
            out float kitchenStartDistance), Is.True);
        Assert.That(kitchenStartDistance, Is.GreaterThan(145f));
        kitchenLogical = AssertActiveRoomViewLocalAnchor(
            player,
            cameraManager,
            reverse,
            reversePassage,
            ...
            kitchenAnchor,
            requireMovement: true,
            label: $"{profile} Kitchen far approach");
        SetPrivateField(reverse, "lastPointerActivationFrame", -1);
        reverse.ActivateDoor();
        Assert.That(player.HasDestination, Is.True);
        AssertVector2Within(GetPrivateValue<Vector2>(player, "finalDestination"),
            kitchenLogical, 0.0001f, $"{profile} Kitchen production approach");
        for (int frame = 0; frame < 180 && navigation.CurrentRoom == Kitchen && player.HasDestination; frame++)
        {
            InvokePrivateMethod(player, "MoveTowardDestination");
            yield return null;
        }
        ...
        Physics2D.SyncTransforms();
        Assert.That(navigation.CurrentRoom, Is.EqualTo(ServiceCorridor));
        serviceLogical = AssertActiveRoomViewLocalAnchor(
            player,
            cameraManager,
            forward,
            forwardPassage,
            ...
            serviceAnchor,
            requireMovement: false,
            label: $"{profile} Service return approach");
        AssertVector2Within(player.LogicalPosition, serviceLogical, 0.001f,
            $"{profile} Service room-local arrival");
        InvokePrivateStaticMethod(typeof(DoorTriggerNavigation), "StopCurrentNavigationSound");
        Debug.Log(
            $"[ServiceKitchenRoomLocal] profile={profile} " +
            $"serviceLocal={FormatVector(serviceAnchor)} serviceLogical={FormatVector(serviceLogical)} " +
            $"kitchenLocal={FormatVector(kitchenAnchor)} kitchenLogical={FormatVector(kitchenLogical)}");
    }

    private static Vector2 AssertActiveRoomViewLocalAnchor(
        PointClickPlayerMovement player,
        CameraManager cameraManager,
        DoorTriggerNavigation trigger,
        ...
        string label)
    {
        Assert.That(passage.SourceRoomView, Is.Not.Null);
        Assert.That(passage.SourceRoomView.IsVisible, Is.True);
        Assert.That(anchor.CoordinateSpace, Is.EqualTo(PassageAnchorCoordinateSpace.RoomViewLocal));
        AssertVector2Within(anchor.RoomViewLocalPosition,
            expectedRoomViewLocalPosition, 0.0001f, $"{label} authored native coordinate");
        Assert.That(anchor.TryResolveLogicalPosition(player, out Vector2 logicalPosition), Is.True);
        AssertFinite(logicalPosition, $"{label} resolved logical coordinate");
        Assert.That(player.TryGetWorldPointFromLogicalPosition(logicalPosition, out Vector2 worldPosition), Is.True);
        Assert.That(cameraManager.TryGetActiveRoomStageLocalPoint(
            new Vector3(worldPosition.x, worldPosition.y, player.transform.position.z),
            out Vector2 roundTripLocalPosition), Is.True);
        AssertVector2Within(roundTripLocalPosition,
            expectedRoomViewLocalPosition, 0.05f, $"{label} native round trip");
        Assert.That(TryEvaluateExactReachableMovementTarget(
            player,
            logicalPosition,
            out PointClickPlayerMovement.MovementTargetQuery movementQuery), Is.True);
        ...
        Assert.That(movementQuery.HasReachableDestination, Is.True, $"{label} must be path reachable.");
        Assert.That(movementQuery.UsesProjectedDestination, Is.False, $"{label} must not use projection.");
        Assert.That(movementQuery.WouldMove, Is.EqualTo(requireMovement),
            $"{label} movement state must match whether the player starts at the anchor.");
        AssertVector2Within(movementQuery.Destination,
            logicalPosition, 0.0001f, $"{label} exact movement destination");
        Assert.That(TryInvokeCanonicalApproachDestination(trigger, player, out Vector2 canonical), Is.True);
        Assert.That(TryInvokeTraversalApproachDestination(trigger, player, out Vector2 traversal, null), Is.True);
        AssertVector2Within(canonical, logicalPosition, 0.0001f, $"{label} canonical resolution");
        AssertVector2Within(traversal, logicalPosition, 0.0001f, $"{label} traversal resolution");
        AssertApproachWithinActivationDistance(trigger, player, logicalPosition, label);
        Assert.That(GetAuthoredAnchorScreenDistance(player, trigger, logicalPosition), Is.LessThanOrEqualTo(1f));
        return logicalPosition;
    }
    ...
    private static IEnumerator AssertGroup07AnchorCandidatesAndReturnToPantry(
        ...
        DoorTriggerNavigation forward,
        DoorTriggerNavigation reverse,
        Vector2 pantryCandidate,
        Vector2 serviceCandidate,
        bool maximumZoom,
        string profile)
    {
        const string ButlersPantry = "Butlers Pantry";
        ...
        Canvas.ForceUpdateCanvases();
        Physics2D.SyncTransforms();
        AssertButlersBilliardAnchorCandidateFromStarts(
            player,
            pantryCandidate,
            new[] { new Vector2(0f, -2f), new Vector2(2f, -2f) },
            $"{profile} Pantry candidate");
        AssertApproachWithinActivationDistance(
            forward,
            player,
            pantryCandidate,
            $"{profile} Pantry candidate");
        float pantryDistance = GetAuthoredAnchorScreenDistance(player, forward, pantryCandidate);
        Assert.That(pantryDistance, Is.LessThanOrEqualTo(125f),
            $"{profile} Pantry candidate must retain twenty pixels of threshold margin.");
        Assert.That(player.TryWarpToExact(pantryCandidate), Is.True);
        SetPrivateField(forward, "lastPointerActivationFrame", -1);
        forward.ActivateDoor();
        ...
        Canvas.ForceUpdateCanvases();
        Physics2D.SyncTransforms();
        ObjectMovementBlocker2D leftTableBlocker = FindInActiveScene<ObjectMovementBlocker2D>()
            .Single(item => item.name == "PlayerBlocker_service_corridor_left_table_0");
        ObjectMovementBlocker2D rightDeskBlocker = FindInActiveScene<ObjectMovementBlocker2D>()
            .Single(item => item.name == "PlayerBlocker_service_corridor_right_desk_0");
        leftTableBlocker.ApplySourceSortingNow();
        rightDeskBlocker.ApplySourceSortingNow();
        SpriteRenderer leftTableRenderer = FindInActiveScene<SpriteRenderer>()
            .Single(item => item.name == "service_corridor_left_table_0");
        SpriteRenderer rightDeskRenderer = FindInActiveScene<SpriteRenderer>()
            .Single(item => item.name == "service_corridor_right_desk_0");
        Assert.That(leftTableBlocker.CurrentSortingOrder, Is.EqualTo(
            1000 - Mathf.RoundToInt(leftTableBlocker.BlockingCollider.bounds.min.y * 100f)));
        Assert.That(rightDeskBlocker.CurrentSortingOrder, Is.EqualTo(
            1000 - Mathf.RoundToInt(rightDeskBlocker.BlockingCollider.bounds.min.y * 100f)));
        Assert.That(leftTableBlocker.CurrentSortingOrder, Is.GreaterThan(1000));
        Assert.That(rightDeskBlocker.CurrentSortingOrder, Is.GreaterThan(1000));
        Assert.That(leftTableRenderer.sortingOrder, Is.EqualTo(leftTableBlocker.CurrentSortingOrder));
        Assert.That(rightDeskRenderer.sortingOrder, Is.EqualTo(rightDeskBlocker.CurrentSortingOrder));
        Assert.That(leftTableRenderer.sortingLayerName, Is.EqualTo("People"));
        Assert.That(rightDeskRenderer.sortingLayerName, Is.EqualTo("People"));
        Assert.That(leftTableRenderer.spriteSortPoint, Is.EqualTo(SpriteSortPoint.Pivot));
        Assert.That(rightDeskRenderer.spriteSortPoint, Is.EqualTo(SpriteSortPoint.Pivot));
        AssertButlersBilliardAnchorCandidateFromStarts(
            player,
            serviceCandidate,
            new[] { new Vector2(2f, -2.5f), new Vector2(3f, -2.5f) },
            $"{profile} Service candidate");
        AssertApproachWithinActivationDistance(
            reverse,
            player,
            serviceCandidate,
            $"{profile} Service candidate");
        float serviceDistance = GetAuthoredAnchorScreenDistance(player, reverse, serviceCandidate);
        Assert.That(serviceDistance, Is.LessThanOrEqualTo(125f),
            $"{profile} Service candidate must retain twenty pixels of threshold margin.");
        Assert.That(player.TryWarpToExact(serviceCandidate), Is.True);
        SetPrivateField(reverse, "lastPointerActivationFrame", -1);
        reverse.ActivateDoor();
        for (int frame = 0; frame < 4; frame++)
        {
        ...
        InvokePrivateStaticMethod(typeof(DoorTriggerNavigation), "StopCurrentNavigationSound");
        Debug.Log(
            $"[ButlersServiceAnchorCalibration] profile={profile} " +
            $"pantry={FormatVector(pantryCandidate)} pantryScreenDistance={pantryDistance:0.###} " +
            $"service={FormatVector(serviceCandidate)} serviceScreenDistance={serviceDistance:0.###}");
    }

    [UnityTest]
    public IEnumerator ButlersPantryBilliardRoomAuthoredAnchorPassagesUseInvariantApproachesAndArrivals()
    {
        const string ButlersPantry = "Butlers Pantry";
        const string BilliardRoom = "Billiard Room";
        const string AuthoredObservationSha256 =
            "cd248f01301448b5cd807cc9331e58d99bd59a139bb772dab352869527b9a6eb";
        Vector2 pantryAnchorCandidate = new Vector2(3.244461f, -3.108338f);
        Vector2 billiardAnchorCandidate = new Vector2(6.9f, -1.6f);
        Vector2 requestedForwardStart = new Vector2(0f, -2f);
        Vector2 requestedReverseStart = new Vector2(0f, -2f);
        List<string> observationProfileLines = new List<string>(7);
        ...
        try
        {
            ...
            Assert.That(reversePassage.UsesAuthoredArrival, Is.True);
            AssertVector2Within(forwardPassage.ApproachAnchor.LogicalPosition,
                pantryAnchorCandidate, 0.0001f,
                "authored Pantry-to-Billiard approach anchor");
            AssertVector2Within(forwardPassage.ArrivalAnchor.LogicalPosition,
                billiardAnchorCandidate, 0.0001f,
                "authored Pantry-to-Billiard arrival anchor");
            AssertVector2Within(reversePassage.ApproachAnchor.LogicalPosition,
                billiardAnchorCandidate, 0.0001f,
                "authored Billiard-to-Pantry approach anchor");
            AssertVector2Within(reversePassage.ArrivalAnchor.LogicalPosition,
                pantryAnchorCandidate, 0.0001f,
                "authored Billiard-to-Pantry arrival anchor");
            foreach (DoorTriggerNavigation trigger in new[] { forward, reverse })
            {
                Assert.That(GetPrivateValue<float>(trigger, "maxPlayerScreenDistance"), Is.EqualTo(145f));
                Assert.That(GetPrivateValue<bool>(trigger, "requirePlayerInSourceRoom"), Is.True);
            ...
            Assert.That(reverseRect.parent.name, Is.EqualTo("Doors"));
            CanonicalRoomView pantryView = pantryContent.GetComponent<CanonicalRoomView>();
            CanonicalRoomView billiardView = billiardContent.GetComponent<CanonicalRoomView>();
            Assert.That(pantryView, Is.Not.Null);
            Assert.That(billiardView, Is.Not.Null);
            Assert.That(forwardPassage.SourceRoomView, Is.SameAs(pantryView));
            Assert.That(reversePassage.SourceRoomView, Is.SameAs(billiardView));
            Assert.That(forwardPassage.Definition.SourceRoom, Is.SameAs(pantryView.Definition));
            Assert.That(forwardPassage.Definition.DestinationRoom, Is.SameAs(billiardView.Definition));
            Assert.That(reversePassage.Definition.SourceRoom, Is.SameAs(billiardView.Definition));
            Assert.That(reversePassage.Definition.DestinationRoom, Is.SameAs(pantryView.Definition));
            Assert.That(billiardView.Definition.StableId, Is.EqualTo("room.billiard-room"));
            Assert.That(billiardView.Definition.DisplayName, Is.EqualTo(BilliardRoom));
            Assert.That(billiardView.Definition.LegacyNames, Is.EqualTo(new[] { BilliardRoom }));
            Assert.That(billiardView.LegacyContentGroup, Is.SameAs(billiardContent));
            Assert.That(billiardView.Root, Is.SameAs(billiardContent.transform));
            Assert.That(pantryView.IsVisible, Is.True);
            Assert.That(billiardView.IsVisible, Is.False);
            Assert.That(billiardView.HasGameContext, Is.True);
            Assert.That(forwardPassage.HasGameContext, Is.True);
            Assert.That(reversePassage.HasGameContext, Is.True);
            Assert.That(forwardPassage.isActiveAndEnabled, Is.True);
            Assert.That(reversePassage.isActiveAndEnabled, Is.False);
            Assert.That(billiardView.Definition.BackgroundTexture,
                Is.SameAs(GetPrivateField<Texture>(billiardContent, "roomBackgroundTexture")));
            Assert.That(billiardView.Definition.PerspectiveProfile, Is.Null);
            Assert.That(GetPrivateField<Texture>(pantryContent, "roomBackgroundTexture"), Is.Not.Null);
            ...
            Assert.That(FindInActiveScene<ObjectMovementBlocker2D>()
                ...
                Is.EqualTo(3));
            string[] expectedCutoutNames =
            {
                "billiard_table",
                "billiard_left_armchair",
                "billiard_left_lamp_table"
            };
            int[] expectedCutoutComponentCounts = { 3, 2, 2 };
            int[] expectedCutoutSortingOrders = { 1000, 1000, 1000 };
            string[] expectedBlockerNames =
            {
                "PlayerBlocker_billiard_table",
                "PlayerBlocker_billiard_left_armchair",
                "PlayerBlocker_billiard_left_lamp_table"
            };
            for (int cutoutIndex = 0; cutoutIndex < expectedCutoutNames.Length; cutoutIndex++)
            {
                string cutoutName = expectedCutoutNames[cutoutIndex];
                Transform cutout = FindInActiveScene<Transform>().Single(item => item.name == cutoutName);
                Assert.That(cutout.GetComponentInParent<RoomContentGroup>(true), Is.SameAs(billiardContent));
                Assert.That(cutout.GetComponents<Component>(),
                    Has.Length.EqualTo(expectedCutoutComponentCounts[cutoutIndex]));
                Assert.That(cutout.GetComponent<SpriteRenderer>(), Is.Not.Null);
                Assert.That(cutout.GetComponent<SpriteRenderer>().sortingOrder,
                    Is.EqualTo(expectedCutoutSortingOrders[cutoutIndex]));
            }
            foreach (string blockerName in expectedBlockerNames)
            {
                ObjectMovementBlocker2D blocker = FindInActiveScene<ObjectMovementBlocker2D>()
                    .Single(item => item.name == blockerName);
                Assert.That(blocker.GetComponentInParent<RoomContentGroup>(true), Is.SameAs(billiardContent));
                Assert.That(blocker.GetComponents<Component>(), Has.Length.EqualTo(3));
            }

            Chateau.Architecture.ValidationReport billiardViewReport =
                new Chateau.Architecture.ValidationReport();
            Chateau.Architecture.ValidationReport forwardPassageReport =
                new Chateau.Architecture.ValidationReport();
            Chateau.Architecture.ValidationReport reversePassageReport =
                new Chateau.Architecture.ValidationReport();
            billiardView.ValidateConfiguration(billiardViewReport);
            forwardPassage.ValidateConfiguration(forwardPassageReport);
            reversePassage.ValidateConfiguration(reversePassageReport);
            Assert.That(billiardViewReport.Messages, Is.Empty);
            Assert.That(forwardPassageReport.Messages, Is.Empty);
            Assert.That(reversePassageReport.Messages, Is.Empty);
            ...
            Assert.That(gameRoot.Context.Database.Definitions, Does.Contain(forwardPassage.Definition));
            Assert.That(gameRoot.Context.Database.Definitions, Does.Contain(reversePassage.Definition));
            Assert.That(AssetDatabase.GetAssetPath(billiardView.Definition),
                Is.EqualTo("Assets/_Chateau/Data/World/Rooms/Room_BilliardRoom.asset"));
            Assert.That(AssetDatabase.GetAssetPath(forwardPassage.Definition),
                Is.EqualTo("Assets/_Chateau/Data/World/Passages/Passage_ButlersPantry_BilliardRoom.asset"));
            Assert.That(AssetDatabase.GetAssetPath(reversePassage.Definition),
                Is.EqualTo("Assets/_Chateau/Data/World/Passages/Passage_BilliardRoom_ButlersPantry.asset"));
            List<Chateau.Architecture.ChateauBehaviour> sceneBehaviours =
                GetPrivateField<List<Chateau.Architecture.ChateauBehaviour>>(gameRoot, "sceneBehaviours");
            Assert.That(sceneBehaviours.Count(item => item == billiardView), Is.EqualTo(1));
            Assert.That(sceneBehaviours.Count(item => item == forwardPassage), Is.EqualTo(1));
            Assert.That(sceneBehaviours.Count(item => item == reversePassage), Is.EqualTo(1));
            ...
            foreach (string definitionGuid in new[]
            {
                ...
                "71ea8ce4d4eb8fa7f107abe24d7c903e",
                "be2f1b94b724dcfa061876e33bce02ca"
            })
            {
                Assert.That(databaseText, Does.Contain(
                    $"  - {{fileID: 11400000, guid: {definitionGuid}, type: 2}}"));
            ...
            string serializedBilliardView = RequireSerializedUnityDocument(sceneText, "4100000008");
            string serializedForwardPassage = RequireSerializedUnityDocument(sceneText, "4100000023");
            string serializedReversePassage = RequireSerializedUnityDocument(sceneText, "4100000024");
            string serializedGameRoot = RequireSerializedUnityDocument(sceneText, "1878886998");
            Assert.That(System.Text.RegularExpressions.Regex.Matches(
                sceneText,
                "^--- !u!",
            ...
            Assert.That(serializedBilliardView, Does.Contain(
                "definition: {fileID: 11400000, guid: bed158a9affd015fcc961340d9be5dd8, type: 2}"));
            Assert.That(serializedBilliardView, Does.Contain("legacyContentGroup: {fileID: 2300000012}"));
            Assert.That(serializedForwardPassage, Does.Contain(
                "definition: {fileID: 11400000, guid: 71ea8ce4d4eb8fa7f107abe24d7c903e, type: 2}"));
            Assert.That(serializedForwardPassage, Does.Contain("sourceRoomView: {fileID: 4100000007}"));
            Assert.That(serializedForwardPassage, Does.Contain("reversePassage: {fileID: 4100000024}"));
            Assert.That(serializedForwardPassage, Does.Contain(
            ...
            Assert.That(serializedReversePassage, Does.Contain("sourceRoomView: {fileID: 4100000008}"));
            Assert.That(serializedReversePassage, Does.Contain("reversePassage: {fileID: 4100000023}"));
            Assert.That(serializedReversePassage, Does.Contain(
                "  approachAnchor:\n    logicalPosition: {x: 6.9, y: -1.6}"));
            Assert.That(serializedReversePassage, Does.Contain(
                "  arrivalAnchor:\n    logicalPosition: {x: 3.244461, y: -3.108338}"));
            Assert.That(serializedReversePassage, Does.Contain("anchorMigrationStage: 2"));
            foreach (string fileId in new[] { "4100000008", "4100000023", "4100000024" })
            {
                Assert.That(serializedGameRoot, Does.Contain($"  - {{fileID: {fileId}}}"));
            }

            System.Text.RegularExpressions.MatchCollection triggerDocuments =
                System.Text.RegularExpressions.Regex.Matches(
                    sceneText.Replace("\r\n", "\n"),
                    "(?ms)^--- !u!114 &\\d+\\n.*?(?=^--- !u!|\\z)");
            string triggerScriptGuid = "7e419b0f8f26d4f2d8d03e567fef4c52";
            string[] serializedTriggerDocuments = triggerDocuments.Cast<System.Text.RegularExpressions.Match>()
                .Select(item => item.Value)
                .Where(document => document.Contains($"guid: {triggerScriptGuid}"))
                .ToArray();
            Assert.That(serializedTriggerDocuments, Has.Length.EqualTo(45));
            ...
            AssertButlersBilliardAuthoredObservationStructure(primary, includeNear: true, "primary");
            ...
            AssertVector2Within(primary.ForwardDispatch, pantryAnchorCandidate, 0.0001f,
                "primary Pantry-to-Billiard authored dispatch");
            AssertVector2Within(primary.ForwardProductionApproach, pantryAnchorCandidate, 0.0001f,
                "primary Pantry-to-Billiard authored production approach");
            AssertVector2Within(primary.ForwardArrival, billiardAnchorCandidate, 0.0001f,
                "primary authored Billiard arrival");
            ...
            AssertVector2Within(primary.ReverseDispatch, billiardAnchorCandidate, 0.0001f,
                "primary Billiard-to-Pantry authored dispatch");
            AssertVector2Within(primary.ReverseProductionApproach, billiardAnchorCandidate, 0.0001f,
                "primary Billiard-to-Pantry authored production approach");
            AssertVector2Within(primary.ReverseArrival, pantryAnchorCandidate, 0.0001f,
                "primary authored Pantry arrival");
            AssertVector2Within(primary.NearForwardArrival, billiardAnchorCandidate, 0.0001f,
                "primary near authored Billiard arrival");
            AssertVector2Within(primary.NearReverseArrival, pantryAnchorCandidate, 0.0001f,
                "primary near authored Pantry arrival");
            string primaryObservationLine =
                $"[ButlersBilliardAuthoredPrimary] events={string.Join("|", primaryEvents)} " +
                FormatDoorRoundTripObservation(primary, includeNear: true);
            ...
            for (int sizeIndex = 0; sizeIndex < renderedSizes.Length; sizeIndex++)
            {
                ...
                AssertVector2Within(aspect.ForwardDispatch, pantryAnchorCandidate, 0.0001f,
                    "aspect Pantry-to-Billiard authored dispatch");
                AssertVector2Within(aspect.ForwardProductionApproach, pantryAnchorCandidate, 0.0001f,
                    "aspect Pantry-to-Billiard authored production approach");
                AssertVector2Within(aspect.ForwardArrival, billiardAnchorCandidate,
                    0.0001f, "aspect authored Billiard arrival");
                ...
                AssertVector2Within(aspect.ReverseDispatch, billiardAnchorCandidate, 0.0001f,
                    "aspect Billiard-to-Pantry authored dispatch");
                AssertVector2Within(aspect.ReverseProductionApproach, billiardAnchorCandidate, 0.0001f,
                    "aspect Billiard-to-Pantry authored production approach");
                AssertVector2Within(aspect.ReverseArrival, pantryAnchorCandidate,
                    0.0001f, "aspect authored Pantry arrival");
                string aspectObservationLine =
                    $"[ButlersBilliardAuthoredAspect] viewport={renderedSize.x}x{renderedSize.y} " +
                    FormatDoorRoundTripObservation(aspect, includeNear: false);
            ...
            AssertButlersBilliardAuthoredObservationStructure(maximum, includeNear: false, "maximum zoom");
            ...
            AssertVector2Within(maximum.ForwardDispatch, pantryAnchorCandidate, 0.0001f,
                "maximum-zoom Pantry-to-Billiard authored dispatch");
            AssertVector2Within(maximum.ForwardProductionApproach, pantryAnchorCandidate, 0.0001f,
                "maximum-zoom Pantry-to-Billiard authored production approach");
            AssertVector2Within(maximum.ForwardArrival, billiardAnchorCandidate, 0.0001f,
                "maximum-zoom authored Billiard arrival");
            ...
            AssertVector2Within(maximum.ReverseDispatch, billiardAnchorCandidate, 0.0001f,
                "maximum-zoom Billiard-to-Pantry authored dispatch");
            AssertVector2Within(maximum.ReverseProductionApproach, billiardAnchorCandidate, 0.0001f,
                "maximum-zoom Billiard-to-Pantry authored production approach");
            AssertVector2Within(maximum.ReverseArrival, pantryAnchorCandidate, 0.0001f,
                "maximum-zoom authored Pantry arrival");
            string maximumObservationLine =
                $"[ButlersBilliardAuthoredMaximumZoom] viewport={Screen.width}x{Screen.height} " +
                $"zoom={cameraManager.maxRoomZoom:0.###} " +
                FormatDoorRoundTripObservation(maximum, includeNear: false);
            observationProfileLines.Add(maximumObservationLine);
            Debug.Log(maximumObservationLine);
            ...
            yield return AssertButlersBilliardCallerBoundStageZeroPoisonProof(
                navigation,
                player,
                cameraManager,
                forward,
                ...
                forwardPassage,
                reversePassage);
            yield return AssertButlersBilliardStageOneArrivalFallbackProof(
                navigation,
                player,
                cameraManager,
                forward,
                ...
                forwardPassage,
                reversePassage);
            yield return AssertButlersBilliardPairLocalNullCallerFallback(
                navigation,
                player,
                cameraManager,
                forward,
            ...
            string profileObservationLine =
                $"[ButlersBilliardAuthoredProfile] forwardGeometry={FormatVector(forwardRect.anchoredPosition)}/" +
                $"{FormatVector(forwardRect.sizeDelta)} reverseGeometry={FormatVector(reverseRect.anchoredPosition)}/" +
                $"{FormatVector(reverseRect.sizeDelta)} profiles=invariant callers=bound serializedDependencies=bound " +
    ...
    private static IEnumerator AssertButlersBilliardCallerBoundStageZeroPoisonProof(
        RoomNavigationManager navigation,
        PointClickPlayerMovement player,
        CameraManager cameraManager,
        ...
        CanonicalPassage reversePassage)
    {
        ...
        Assert.That(originalForwardStage, Is.EqualTo(PassageAnchorMigrationStage.AuthoredAnchors));
        Assert.That(originalReverseStage, Is.EqualTo(PassageAnchorMigrationStage.AuthoredAnchors));
        AssertVector2Within(originalForwardApproach, new Vector2(3.244461f, -3.108338f), 0.0001f,
            "serialized Pantry-to-Billiard authored approach before poison proof");
        AssertVector2Within(originalForwardArrival, new Vector2(6.9f, -1.6f), 0.0001f,
            "serialized Pantry-to-Billiard authored arrival before poison proof");
        AssertVector2Within(originalReverseApproach, new Vector2(6.9f, -1.6f), 0.0001f,
            "serialized Billiard-to-Pantry authored approach before poison proof");
        AssertVector2Within(originalReverseArrival, new Vector2(3.244461f, -3.108338f), 0.0001f,
            "serialized Billiard-to-Pantry authored arrival before poison proof");
        ...
        try
        {
            ...
            Assert.That(navigationFacade.CanTraverse(forwardPassage), Is.True,
                "Stage 0 must not validate or consume poisoned Group06 authored coordinates.");

            yield return AssertButlersBilliardLockedStageZeroRoundTrip(
                ...
                forwardTrigger,
                reverseTrigger,
                new Vector2(6.545288f, -1.559969f));
        ...
        finally
        {
            ...
            if (navigation.CurrentRoom == BilliardRoom)
            {
                cleanupRoomRestored = navigationFacade.TryTraverse(reversePassage);
            }
            cameraManager.ResetRoomLookForPreview();
            if (navigation.CurrentRoom == ButlersPantry)
            {
        ...
        CanonicalPassage reversePassage)
    {
        const string ButlersPantry = "Butlers Pantry";
        const string BilliardRoom = "Billiard Room";
        Vector2 pantryCandidate = new Vector2(3.244461f, -3.108338f);
        Vector2 billiardCandidate = new Vector2(6.9f, -1.6f);
        Vector2Int[] sizes =
        {
            new Vector2Int(1366, 768),
            new Vector2Int(1440, 1080),
        ...
        PassageAnchorMigrationStage originalReverseStage = reversePassage.AnchorMigrationStage;
        Vector2 originalPlayerPosition = player.LogicalPosition;
        float originalCurrentRoomPan = GetPrivateValue<float>(cameraManager, "currentRoomPan");
        float originalTargetRoomPan = GetPrivateValue<float>(cameraManager, "targetRoomPan");
        float originalCurrentVerticalPan = GetPrivateValue<float>(cameraManager, "currentRoomVerticalPan");
        float originalTargetVerticalPan = GetPrivateValue<float>(cameraManager, "targetRoomVerticalPan");
        float originalCurrentRoomFov = GetPrivateValue<float>(cameraManager, "currentRoomFov");
        float originalCurrentRoomZoom = GetPrivateValue<float>(cameraManager, "currentRoomZoom");
        float originalTargetRoomZoom = GetPrivateValue<float>(cameraManager, "targetRoomZoom");
        bool cleanupRoomRestored = true;
        bool cleanupPlayerRestored = true;
        ...
        Assert.That(originalForwardStage, Is.EqualTo(PassageAnchorMigrationStage.AuthoredAnchors));
        Assert.That(originalReverseStage, Is.EqualTo(PassageAnchorMigrationStage.AuthoredAnchors));
        AssertVector2Within(originalForwardApproach, pantryCandidate, 0.0001f,
            "serialized Pantry-to-Billiard authored approach");
        AssertVector2Within(originalForwardArrival, billiardCandidate, 0.0001f,
            "serialized Pantry-to-Billiard authored arrival");
        AssertVector2Within(originalReverseApproach, billiardCandidate, 0.0001f,
            "serialized Billiard-to-Pantry authored approach");
        AssertVector2Within(originalReverseArrival, pantryCandidate, 0.0001f,
            "serialized Billiard-to-Pantry authored arrival");
        Assert.That(GetPrivateField<CanonicalPassage>(forwardTrigger, "canonicalPassage"),
            Is.SameAs(forwardPassage));
        Assert.That(GetPrivateField<CanonicalPassage>(reverseTrigger, "canonicalPassage"),
            Is.SameAs(reversePassage));
        ...
        try
        {
            ...
            for (int profileIndex = 0; profileIndex < sizes.Length; profileIndex++)
            {
                ...
                AssertButlersBilliardAnchorCandidateFromStarts(
                    ...
                    $"{profile} Pantry candidate");
                AssertApproachWithinActivationDistance(
                    forwardTrigger, player, pantryCandidate, $"{profile} Pantry candidate");
                float pantryScreenDistance = GetAuthoredAnchorScreenDistance(
                    player, forwardTrigger, pantryCandidate);
                float pantryThreshold = GetPrivateValue<float>(forwardTrigger, "maxPlayerScreenDistance");
                Assert.That(pantryScreenDistance, Is.LessThanOrEqualTo(pantryThreshold + 0.5f),
                    $"{profile} Pantry candidate must be inside the serialized threshold.");
                Assert.That(pantryScreenDistance, Is.LessThanOrEqualTo(125f),
                    $"{profile} Pantry candidate must retain at least 20 pixels of threshold margin.");
                Assert.That(player.TryWarpToExact(new Vector2(0f, -2f)), Is.True);
                Assert.That(TryInvokeApproachDestination(
                ...
                AssertVector2Within(forwardDispatch, forwardLegacy, 0.0001f,
                    $"{profile} stage-1 Pantry dispatch remains legacy sampled");
                Assert.That(Vector2.Distance(forwardDispatch, pantryCandidate), Is.GreaterThan(0.05f),
                    $"{profile} stage-1 Pantry dispatch must remain distinct from the authored candidate.");
                ...
                AssertVector2Within(player.LogicalPosition, billiardCandidate, 0.0001f,
                    $"{profile} far forward authored Billiard arrival");

                AssertButlersBilliardAnchorCandidateFromStarts(
                    player,
                    billiardCandidate,
                    new[] { new Vector2(0f, -2f), new Vector2(2f, -2f) },
                    $"{profile} Billiard candidate");
                AssertApproachWithinActivationDistance(
                    reverseTrigger, player, billiardCandidate, $"{profile} Billiard candidate");
                float billiardScreenDistance = GetAuthoredAnchorScreenDistance(
                    player, reverseTrigger, billiardCandidate);
                float billiardThreshold = GetPrivateValue<float>(reverseTrigger, "maxPlayerScreenDistance");
                Assert.That(billiardScreenDistance, Is.LessThanOrEqualTo(billiardThreshold + 0.5f),
                    $"{profile} Billiard candidate must be inside the serialized threshold.");
                Assert.That(billiardScreenDistance, Is.LessThanOrEqualTo(125f),
                    $"{profile} Billiard candidate must retain at least 20 pixels of threshold margin.");
                Assert.That(player.TryWarpToExact(new Vector2(0f, -2f)), Is.True);
                Assert.That(TryInvokeApproachDestination(
                ...
                AssertVector2Within(reverseDispatch, reverseLegacy, 0.0001f,
                    $"{profile} stage-1 Billiard dispatch remains legacy sampled");
                Assert.That(Vector2.Distance(reverseDispatch, billiardCandidate), Is.GreaterThan(0.05f),
                    $"{profile} stage-1 Billiard dispatch must remain distinct from the authored candidate.");
                ...
                Physics2D.SyncTransforms();
                Assert.That(navigation.CurrentRoom, Is.EqualTo(ButlersPantry));
                AssertVector2Within(player.LogicalPosition, pantryCandidate, 0.0001f,
                    $"{profile} far reverse authored Pantry arrival");

                Assert.That(navigationFacade.TryTraverse(forwardPassage), Is.True);
                yield return WaitForSettledLayout();
                AssertVector2Within(player.LogicalPosition, billiardCandidate, 0.0001f,
                    $"{profile} near forward authored Billiard arrival");
                Assert.That(navigationFacade.TryTraverse(reversePassage), Is.True);
                yield return WaitForSettledLayout();
                AssertVector2Within(player.LogicalPosition, pantryCandidate, 0.0001f,
                    $"{profile} near reverse authored Pantry arrival");

                Debug.Log(
                    $"[ButlersBilliardStageOneArrivalProof] profile={profile} " +
                    $"pantry={FormatVector(pantryCandidate)} pantryScreenDistance={pantryScreenDistance:0.###} " +
                    $"pantryThreshold={pantryThreshold:0.###} billiard={FormatVector(billiardCandidate)} " +
                    $"billiardScreenDistance={billiardScreenDistance:0.###} " +
                    $"billiardThreshold={billiardThreshold:0.###} " +
                    $"forwardLegacy={FormatVector(forwardLegacy)} reverseLegacy={FormatVector(reverseLegacy)}");
            }
        }
        finally
        {
            ...
            SetPrivateField(cameraManager, "currentRoomPan", originalCurrentRoomPan);
            SetPrivateField(cameraManager, "targetRoomPan", originalTargetRoomPan);
            SetPrivateField(cameraManager, "currentRoomVerticalPan", originalCurrentVerticalPan);
            SetPrivateField(cameraManager, "targetRoomVerticalPan", originalTargetVerticalPan);
            SetPrivateField(cameraManager, "currentRoomFov", originalCurrentRoomFov);
            SetPrivateField(cameraManager, "currentRoomZoom", originalCurrentRoomZoom);
            SetPrivateField(cameraManager, "targetRoomZoom", originalTargetRoomZoom);
            InvokePrivateMethod(cameraManager, "ApplyBackgroundLayout");
            if (navigation.CurrentRoom == ButlersPantry)
            {
                cleanupPlayerRestored = player.TryWarpToExact(originalPlayerPosition);
        ...
        yield return WaitForSettledLayout();
        Assert.That(cleanupRoomRestored, Is.True,
            "Stage-one proof cleanup must restore Butlers Pantry through the reciprocal Passage.");
        Assert.That(cleanupPlayerRestored, Is.True,
        ...
        Assert.That(reversePassage.AnchorMigrationStage, Is.EqualTo(originalReverseStage));
        AssertVector2Within(forwardPassage.ApproachAnchor.LogicalPosition, originalForwardApproach, 0.0001f,
            "restored Pantry-to-Billiard approach after stage-one proof");
        AssertVector2Within(forwardPassage.ArrivalAnchor.LogicalPosition, originalForwardArrival, 0.0001f,
            "restored Pantry-to-Billiard arrival after stage-one proof");
        AssertVector2Within(reversePassage.ApproachAnchor.LogicalPosition, originalReverseApproach, 0.0001f,
            "restored Billiard-to-Pantry approach after stage-one proof");
        AssertVector2Within(reversePassage.ArrivalAnchor.LogicalPosition, originalReverseArrival, 0.0001f,
            "restored Billiard-to-Pantry arrival after stage-one proof");
        AssertVector2Within(player.LogicalPosition, originalPlayerPosition, 0.0001f,
            "stage-one Group06 player restoration");
        Assert.That(GetPrivateValue<float>(cameraManager, "currentRoomPan"),
            Is.EqualTo(originalCurrentRoomPan).Within(0.0001f));
        Assert.That(GetPrivateValue<float>(cameraManager, "targetRoomPan"),
            Is.EqualTo(originalTargetRoomPan).Within(0.0001f));
        Assert.That(GetPrivateValue<float>(cameraManager, "currentRoomVerticalPan"),
            Is.EqualTo(originalCurrentVerticalPan).Within(0.0001f));
        Assert.That(GetPrivateValue<float>(cameraManager, "targetRoomVerticalPan"),
            Is.EqualTo(originalTargetVerticalPan).Within(0.0001f));
        Assert.That(GetPrivateValue<float>(cameraManager, "currentRoomFov"),
            Is.EqualTo(originalCurrentRoomFov).Within(0.0001f));
        Assert.That(GetPrivateValue<float>(cameraManager, "currentRoomZoom"),
            Is.EqualTo(originalCurrentRoomZoom).Within(0.0001f));
        Assert.That(GetPrivateValue<float>(cameraManager, "targetRoomZoom"),
            Is.EqualTo(originalTargetRoomZoom).Within(0.0001f));
    }

    private static void AssertButlersBilliardAnchorCandidateFromStarts(
        PointClickPlayerMovement player,
        Vector2 candidate,
        Vector2[] starts,
        string label)
    {
        ...
        for (int startIndex = 0; startIndex < starts.Length; startIndex++)
        {
            Assert.That(player.TryWarpToExact(starts[startIndex]), Is.True,
                $"{label} start {startIndex} must be exact and walkable.");
            Assert.That(Vector2.Distance(player.LogicalPosition, candidate), Is.GreaterThan(1f),
                $"{label} start {startIndex} must exercise a real path.");
            Assert.That(TryEvaluateExactReachableMovementTarget(
                player,
                candidate,
    ...
    private static IEnumerator AssertButlersBilliardPairLocalNullCallerFallback(
        RoomNavigationManager navigation,
        PointClickPlayerMovement player,
        CameraManager cameraManager,
        DoorTriggerNavigation forwardTrigger,
        ...
        CanonicalPassage reversePassage)
    {
        ...
        Assert.That(originalCallers.Count(pair => pair.Value != null), Is.EqualTo(20));
        Assert.That(originalCallers.Count(pair => pair.Value == null), Is.EqualTo(25));
        ...
        try
        {
            ...
            Assert.That(allTriggers.Count(trigger =>
                GetPrivateField<CanonicalPassage>(trigger, "canonicalPassage") != null), Is.EqualTo(18));
            Assert.That(allTriggers.Count(trigger =>
                GetPrivateField<CanonicalPassage>(trigger, "canonicalPassage") == null), Is.EqualTo(27));
        ...
        Assert.That(reversePassage.AnchorMigrationStage, Is.EqualTo(originalReverseStage));
        AssertVector2Within(player.LogicalPosition, originalPlayerPosition, 0.0001f,
            "restored Pantry/Billiard pair-local fallback player position");
    }

    private static IEnumerator AssertButlersBilliardLockedStageZeroRoundTrip(
        string label,
        RoomNavigationManager navigation,
        PointClickPlayerMovement player,
        CameraManager cameraManager,
        ...
        Vector2 expectedFarForwardArrival)
    {
        ...
        Debug.Log($"[ButlersBilliardCallerProof] {label} " +
            FormatDoorRoundTripObservation(observation, includeNear: true));
        Assert.That(observation.ForwardScreenDistance, Is.EqualTo(189.196f).Within(0.75f));
        AssertVector2Within(observation.ForwardNull, new Vector2(2.744461f, -2.748338f), 0.05f,
            $"{label} forward legacy approach");
        AssertVector2Within(observation.ForwardLeft, new Vector2(3.111516f, -2.748338f), 0.05f,
            $"{label} forward legacy left approach");
        AssertVector2Within(observation.ForwardCenter, new Vector2(3.967978f, -2.748338f), 0.05f,
            $"{label} forward legacy center approach");
        AssertVector2Within(observation.ForwardRight, new Vector2(4.824442f, -2.748338f), 0.05f,
            $"{label} forward legacy right approach");
        ...
        AssertVector2Within(observation.ForwardArrival, expectedFarForwardArrival, 0.05f,
            $"{label} forward legacy Billiard arrival");
        Assert.That(observation.ReverseScreenDistance, Is.EqualTo(438.675f).Within(0.75f));
        AssertVector2Within(observation.ReverseNull, new Vector2(6.575521f, -1.484375f), 0.05f,
            $"{label} reverse legacy approach");
        AssertVector2Within(observation.ReverseLeft, new Vector2(6.809896f, -1.484375f), 0.05f,
            $"{label} reverse legacy left approach");
        AssertVector2Within(observation.ReverseCenter, new Vector2(6.575521f, -1.484375f), 0.05f,
            $"{label} reverse legacy center approach");
        AssertVector2Within(observation.ReverseRight, new Vector2(6.575521f, -1.484375f), 0.05f,
            $"{label} reverse legacy right approach");
        ...
        AssertVector2Within(observation.ReverseArrival, new Vector2(5.191498f, -2.748338f), 0.05f,
            $"{label} asymmetric reverse legacy Pantry arrival");
        AssertVector2Within(observation.NearForwardArrival, new Vector2(6.575521f, -1.484375f), 0.05f,
            $"{label} near forward legacy Billiard arrival");
        AssertVector2Within(observation.NearReverseArrival, new Vector2(5.191498f, -2.748338f), 0.05f,
            $"{label} near reverse legacy Pantry arrival");
        Assert.That(Vector2.Distance(observation.ReverseArrival, observation.ForwardNull), Is.GreaterThan(1f),
            $"{label} must preserve the asymmetric Pantry fallback result.");
        Assert.That(navigation.CurrentRoom, Is.EqualTo(ButlersPantry));
    }

    private static void AssertButlersBilliardAuthoredObservationStructure(
        ...
        CanonicalPassage reversePassage)
    {
        ...
        Assert.That(originalForwardStage, Is.EqualTo(PassageAnchorMigrationStage.AuthoredAnchors));
        Assert.That(originalReverseStage, Is.EqualTo(PassageAnchorMigrationStage.AuthoredAnchors));
        AssertVector2Within(originalForwardApproach, new Vector2(3.391918f, -0.36f), 0.0001f,
            "serialized Dining-to-Pantry authored approach before poison proof");
        AssertVector2Within(originalForwardArrival, new Vector2(-5.163103f, -3.463186f), 0.0001f,
            "serialized Dining-to-Pantry authored arrival before poison proof");
        AssertVector2Within(originalReverseApproach, new Vector2(-5.163103f, -3.463186f), 0.0001f,
            "serialized Pantry-to-Dining authored approach before poison proof");
        AssertVector2Within(originalReverseArrival, new Vector2(3.391918f, -0.36f), 0.0001f,
            "serialized Pantry-to-Dining authored arrival before poison proof");
        ...
        try
        {
            ...
            Assert.That(reversePassage.UsesAuthoredArrival, Is.False);
            Assert.That(navigationFacade.CanTraverse(forwardPassage), Is.True,
                "Stage 0 must not validate or consume poisoned authored coordinates.");

            yield return AssertDiningButlersLockedStageZeroRoundTrip(
                "caller-bound poisoned anchors",
                navigation,
                player,
                cameraManager,
        ...
        Assert.That(reversePassage.AnchorMigrationStage, Is.EqualTo(originalReverseStage));
        AssertVector2Within(forwardPassage.ApproachAnchor.LogicalPosition, originalForwardApproach, 0.0001f,
            "restored Dining-to-Pantry approach after poison proof");
        AssertVector2Within(forwardPassage.ArrivalAnchor.LogicalPosition, originalForwardArrival, 0.0001f,
            "restored Dining-to-Pantry arrival after poison proof");
        AssertVector2Within(reversePassage.ApproachAnchor.LogicalPosition, originalReverseApproach, 0.0001f,
            "restored Pantry-to-Dining approach after poison proof");
        AssertVector2Within(reversePassage.ArrivalAnchor.LogicalPosition, originalReverseArrival, 0.0001f,
            "restored Pantry-to-Dining arrival after poison proof");
        AssertVector2Within(player.LogicalPosition, originalPlayerPosition, 0.0001f,
            "restored Dining/Pantry poison-proof player position");
    }

    private static IEnumerator AssertDiningButlersStageOneArrivalFallbackProof(
        RoomNavigationManager navigation,
        PointClickPlayerMovement player,
        CameraManager cameraManager,
        DoorTriggerNavigation forwardTrigger,
        ...
        CanonicalPassage reversePassage)
    {
        const string DiningRoom = "Dining Room";
        const string ButlersPantry = "Butlers Pantry";
        Vector2 diningCandidate = new Vector2(3.391918f, -0.36f);
        Vector2 pantryCandidate = new Vector2(-5.163103f, -3.463186f);
        Vector2Int[] sizes =
        {
            new Vector2Int(1366, 768),
            new Vector2Int(1440, 1080),
        ...
        Assert.That(originalForwardStage, Is.EqualTo(PassageAnchorMigrationStage.AuthoredAnchors));
        Assert.That(originalReverseStage, Is.EqualTo(PassageAnchorMigrationStage.AuthoredAnchors));
        AssertVector2Within(originalForwardApproach, diningCandidate, 0.0001f,
            "serialized Dining-to-Pantry authored approach");
        AssertVector2Within(originalForwardArrival, pantryCandidate, 0.0001f,
            "serialized Dining-to-Pantry authored arrival");
        AssertVector2Within(originalReverseApproach, pantryCandidate, 0.0001f,
            "serialized Pantry-to-Dining authored approach");
        AssertVector2Within(originalReverseArrival, diningCandidate, 0.0001f,
            "serialized Pantry-to-Dining authored arrival");
        Assert.That(GetPrivateField<CanonicalPassage>(forwardTrigger, "canonicalPassage"),
            Is.SameAs(forwardPassage));
        Assert.That(GetPrivateField<CanonicalPassage>(reverseTrigger, "canonicalPassage"),
            Is.SameAs(reversePassage));
        ...
        try
        {
            ...
            for (int profileIndex = 0; profileIndex < sizes.Length; profileIndex++)
            {
                ...
                Assert.That(navigation.CurrentRoom, Is.EqualTo(DiningRoom));
                AssertDiningButlersAnchorCandidateFromStarts(
                    player,
                    diningCandidate,
                    new[] { new Vector2(0f, -1f), new Vector2(2f, -1f) },
                    $"{profile} Dining candidate");
                AssertApproachWithinActivationDistance(
                    forwardTrigger, player, diningCandidate, $"{profile} Dining candidate");
                float diningScreenDistance = GetAuthoredAnchorScreenDistance(
                    player, forwardTrigger, diningCandidate);
                float diningThreshold = GetPrivateValue<float>(forwardTrigger, "maxPlayerScreenDistance");
                Assert.That(diningScreenDistance,
                    Is.EqualTo(expectedDiningDistances[profileIndex]).Within(0.75f));
                Assert.That(diningScreenDistance, Is.LessThanOrEqualTo(diningThreshold + 0.5f),
                    $"{profile} Dining candidate must be inside the real serialized threshold.");
                Assert.That(diningScreenDistance, Is.LessThanOrEqualTo(125f),
                ...
                Assert.That(TryInvokeTraversalApproachDestination(
                    forwardTrigger, player, out Vector2 forwardDispatch, null), Is.True);
                AssertVector2Within(forwardDispatch, forwardLegacy, 0.0001f,
                    $"{profile} stage-1 Dining dispatch remains legacy sampled");
                Assert.That(Vector2.Distance(forwardDispatch, diningCandidate), Is.GreaterThan(0.05f),
                    $"{profile} stage-1 Dining dispatch must remain distinct from the authored candidate.");
                ...
                Assert.That(navigation.CurrentRoom, Is.EqualTo(ButlersPantry));
                AssertVector2Within(player.LogicalPosition, pantryCandidate, 0.0001f,
                    $"{profile} far forward authored Pantry arrival");
                ...
                AssertDiningButlersAnchorCandidateFromStarts(
                    ...
                    $"{profile} Pantry candidate");
                AssertApproachWithinActivationDistance(
                    reverseTrigger, player, pantryCandidate, $"{profile} Pantry candidate");
                float pantryScreenDistance = GetAuthoredAnchorScreenDistance(
                    player, reverseTrigger, pantryCandidate);
                float pantryThreshold = GetPrivateValue<float>(reverseTrigger, "maxPlayerScreenDistance");
                Assert.That(pantryScreenDistance,
                    Is.EqualTo(expectedPantryDistances[profileIndex]).Within(0.75f));
                Assert.That(pantryScreenDistance, Is.LessThanOrEqualTo(pantryThreshold + 0.5f),
                    $"{profile} Pantry candidate must be inside the real serialized threshold.");
                Assert.That(pantryScreenDistance, Is.LessThanOrEqualTo(125f),
                    $"{profile} Pantry candidate must retain at least 20 pixels of threshold margin.");
                Assert.That(player.TryWarpToExact(new Vector2(0f, -2f)), Is.True);
                Assert.That(TryInvokeApproachDestination(
                ...
                AssertVector2Within(reverseDispatch, reverseLegacy, 0.0001f,
                    $"{profile} stage-1 Pantry dispatch remains legacy sampled");
                Assert.That(Vector2.Distance(reverseDispatch, pantryCandidate), Is.GreaterThan(0.05f),
                    $"{profile} stage-1 Pantry dispatch must remain distinct from the authored candidate.");
                ...
                AssertVector2Within(player.LogicalPosition, diningCandidate, 0.0001f,
                    $"{profile} far reverse authored Dining arrival");

                Assert.That(navigationFacade.TryTraverse(forwardPassage), Is.True);
                yield return WaitForSettledLayout();
                AssertVector2Within(player.LogicalPosition, pantryCandidate, 0.0001f,
                    $"{profile} near forward authored Pantry arrival");
                Assert.That(navigationFacade.TryTraverse(reversePassage), Is.True);
                yield return WaitForSettledLayout();
                AssertVector2Within(player.LogicalPosition, diningCandidate, 0.0001f,
                    $"{profile} near reverse authored Dining arrival");

                Debug.Log(
                    $"[DiningButlersStageOneArrivalProof] profile={profile} " +
                    $"dining={FormatVector(diningCandidate)} diningScreenDistance={diningScreenDistance:0.###} " +
                    $"diningThreshold={diningThreshold:0.###} pantry={FormatVector(pantryCandidate)} " +
                    $"pantryScreenDistance={pantryScreenDistance:0.###} pantryThreshold={pantryThreshold:0.###} " +
                    $"forwardLegacy={FormatVector(forwardLegacy)} reverseLegacy={FormatVector(reverseLegacy)}");
            }
        }
        finally
        ...
        yield return WaitForSettledLayout();
        Assert.That(cleanupRoomRestored, Is.True,
            "Stage-one proof cleanup must restore Dining through the reciprocal Passage.");
        Assert.That(cleanupPlayerRestored, Is.True,
            "Stage-one proof cleanup must restore the exact player position.");
        Assert.That(navigation.CurrentRoom, Is.EqualTo(DiningRoom));
        Assert.That(forwardPassage.AnchorMigrationStage, Is.EqualTo(originalForwardStage));
        Assert.That(reversePassage.AnchorMigrationStage, Is.EqualTo(originalReverseStage));
        AssertVector2Within(forwardPassage.ApproachAnchor.LogicalPosition, originalForwardApproach, 0.0001f,
            "restored Dining-to-Pantry approach after stage-one proof");
        AssertVector2Within(forwardPassage.ArrivalAnchor.LogicalPosition, originalForwardArrival, 0.0001f,
            "restored Dining-to-Pantry arrival after stage-one proof");
        AssertVector2Within(reversePassage.ApproachAnchor.LogicalPosition, originalReverseApproach, 0.0001f,
            "restored Pantry-to-Dining approach after stage-one proof");
        AssertVector2Within(reversePassage.ArrivalAnchor.LogicalPosition, originalReverseArrival, 0.0001f,
            "restored Pantry-to-Dining arrival after stage-one proof");
        AssertVector2Within(player.LogicalPosition, originalPlayerPosition, 0.0001f,
    ...
    private static void AssertDiningButlersAnchorCandidateFromStarts(
        PointClickPlayerMovement player,
        Vector2 candidate,
        Vector2[] starts,
        string label)
    ...
    private static IEnumerator AssertDiningButlersPairLocalNullCallerFallback(
        RoomNavigationManager navigation,
        PointClickPlayerMovement player,
        CameraManager cameraManager,
        DoorTriggerNavigation forwardTrigger,
        ...
        CanonicalPassage reversePassage)
    {
        ...
        Assert.That(reversePassage.AnchorMigrationStage, Is.EqualTo(originalReverseStage));
        AssertVector2Within(player.LogicalPosition, originalPlayerPosition, 0.0001f,
            "restored Dining/Pantry pair-local fallback player position");
    }

    private static IEnumerator AssertDiningButlersLockedStageZeroRoundTrip(
        string label,
        RoomNavigationManager navigation,
        PointClickPlayerMovement player,
        CameraManager cameraManager,
        ...
        DoorTriggerNavigation reverseTrigger)
    {
        ...
        Assert.That(observation, Is.Not.Null);
        AssertVector2Within(observation.ForwardNull, new Vector2(2.891918f, 0f), 0.05f,
            $"{label} forward legacy approach");
        AssertVector2Within(observation.ForwardDispatch, observation.ForwardNull, 0.0001f,
            $"{label} forward legacy dispatch");
        AssertVector2Within(observation.ForwardProductionApproach, observation.ForwardNull, 0.0001f,
            $"{label} forward production approach");
        AssertVector2Within(observation.ForwardArrival, new Vector2(-4.663103f, -3.103186f), 0.05f,
            $"{label} forward legacy arrival");
        AssertVector2Within(observation.ReverseNull, new Vector2(-4.663103f, -3.103186f), 0.05f,
            $"{label} reverse legacy approach");
        AssertVector2Within(observation.ReverseDispatch, observation.ReverseNull, 0.0001f,
            $"{label} reverse legacy dispatch");
        AssertVector2Within(observation.ReverseProductionApproach, observation.ReverseNull, 0.0001f,
            $"{label} reverse production approach");
        AssertVector2Within(observation.ReverseArrival, new Vector2(2.891918f, 0f), 0.05f,
            $"{label} reverse legacy arrival");
        AssertVector2Within(observation.NearForwardArrival, new Vector2(-4.663103f, -3.103186f), 0.05f,
            $"{label} near forward legacy arrival");
        AssertVector2Within(observation.NearReverseArrival, new Vector2(2.891918f, 0f), 0.05f,
            $"{label} near reverse legacy arrival");
        Assert.That(navigation.CurrentRoom, Is.EqualTo(DiningRoom));
        Debug.Log($"[DiningButlersCallerProof] {label} " +
            FormatDoorRoundTripObservation(observation, includeNear: true));
    }
    ...
    private sealed class DoorRoundTripObservation
    {
        ...
        public bool HasForwardNullRoomViewLocal;
        public Vector2 ForwardNullRoomViewLocal;
        public Vector2 ForwardArrival;
        public bool HasForwardArrivalRoomViewLocal;
        public Vector2 ForwardArrivalRoomViewLocal;
        ...
        public bool HasReverseNullRoomViewLocal;
        public Vector2 ReverseNullRoomViewLocal;
        public Vector2 ReverseArrival;
        public bool HasReverseArrivalRoomViewLocal;
        public Vector2 ReverseArrivalRoomViewLocal;
    ...
    private static IEnumerator ObserveDoorRoundTrip(
        ...
        string sourceContentRoom = null,
        string destinationContentRoom = null)
    {
        ...
        observation.HasForwardNullRoomViewLocal = TryMapLogicalPointToActiveRoomStageLocal(
            player,
            cameraManager,
            observation.ForwardNull,
            out observation.ForwardNullRoomViewLocal);
        Assert.That(TryInvokeApproachDestination(
        ...
        RoomContentGroup activeDestination = RequireOnlyActiveRoom(destinationContentRoom ?? destinationRoom);
        Assert.That(cameraManager.cameraBackground.texture,
        ...
        observation.HasForwardArrivalRoomViewLocal = TryMapLogicalPointToActiveRoomStageLocal(
            player,
            cameraManager,
            observation.ForwardArrival,
            out observation.ForwardArrivalRoomViewLocal);
        ...
        observation.HasReverseNullRoomViewLocal = TryMapLogicalPointToActiveRoomStageLocal(
            player,
            cameraManager,
            observation.ReverseNull,
            out observation.ReverseNullRoomViewLocal);
        Assert.That(TryInvokeApproachDestination(
        ...
        Physics2D.SyncTransforms();
        Assert.That(navigation.CurrentRoom, Is.EqualTo(sourceRoom));
        RoomContentGroup activeSource = RequireOnlyActiveRoom(sourceContentRoom ?? sourceRoom);
        Assert.That(cameraManager.cameraBackground.texture,
            Is.SameAs(GetPrivateField<Texture>(activeSource, "roomBackgroundTexture")));
        observation.ReverseArrival = player.LogicalPosition;
        observation.HasReverseArrivalRoomViewLocal = TryMapLogicalPointToActiveRoomStageLocal(
            player,
            cameraManager,
            observation.ReverseArrival,
            out observation.ReverseArrivalRoomViewLocal);
    ...
    private static bool TryMapLogicalPointToActiveRoomStageLocal(
        PointClickPlayerMovement player,
        CameraManager cameraManager,
        Vector2 logicalPosition,
        out Vector2 roomViewLocalPosition)
    {
        roomViewLocalPosition = Vector2.zero;
        if (!player.TryGetWorldPointFromLogicalPosition(logicalPosition, out Vector2 worldPosition))
        {
            return false;
        }

        return cameraManager.TryGetActiveRoomStageLocalPoint(
            new Vector3(worldPosition.x, worldPosition.y, player.transform.position.z),
            out roomViewLocalPosition);
    }
    ...
    private static void ApplyMaximumRoomZoom(CameraManager cameraManager)
    {
        SetPrivateField(cameraManager, "currentRoomZoom", cameraManager.maxRoomZoom);
        SetPrivateField(cameraManager, "targetRoomZoom", cameraManager.maxRoomZoom);
        SetPrivateField(cameraManager, "roomZoomVelocity", 0f);
        InvokePrivateMethod(cameraManager, "ApplyBackgroundLayout");
    }
    ...
    [UnityTest]
    public IEnumerator GrandEntranceRearBilliardLegacyPassagesAndCatalogConflictAreCharacterizedAcrossRenderedAspects()
    {
        const string RearDisplayName = "Grand Entrance Hall Rear View";
        const string RearLegacyName = "Grand Entrance Hall Rear view";
        const string BilliardRoom = "Billiard Room";
        const string ExpectedObservationSha256 =
            "56e7cc4b59801f8dfb9da547ff584f7b184bea0d15398e7a0b31f85b83b1b39b";
        ...
        try
        {
            ...
            CanonicalPassage setupPassage = setupTrigger.GetComponent<CanonicalPassage>();
            Assert.That(setupPassage, Is.Not.Null,
                "Group11 setup must reuse the accepted Entrance-to-Rear canonical Passage.");
            Assert.That(setupPassage.ArrivalPlacementMode,
            ...
            Assert.That(forward.gameObject.name, Is.EqualTo("DoorTrigger_GEH_Rear_BilliardRoom"));
            Assert.That(reverse.gameObject.name, Is.EqualTo("DoorTrigger_BilliardRoom_GEH"));
            ...
            Assert.That(forward.DoorName, Is.EqualTo("GEH_BilliardRoom"));
            Assert.That(forward.DestinationRoom, Is.EqualTo(BilliardRoom));
            Assert.That(reverse.SourceRoom, Is.EqualTo(BilliardRoom));
            Assert.That(reverse.DoorName, Is.EqualTo("BilliardRoom_GEH"));
            Assert.That(reverse.DestinationRoom, Is.EqualTo(RearDisplayName));
            foreach (DoorTriggerNavigation trigger in new[] { forward, reverse })
            {
                Assert.That(GetPrivateValue<bool>(trigger, "requirePlayerInSourceRoom"), Is.True);
                Assert.That(trigger.UsesCameraSequence, Is.False);
                ...
                Assert.That(GetPrivateValue<float>(trigger, "maxPlayerScreenDistance"), Is.EqualTo(145f));
                Assert.That(GetPrivateValue<bool>(trigger, "playDoorOpenSound"), Is.True);
                Assert.That(trigger.GetComponent<Image>(), Is.Not.Null);
            ...
            Assert.That(reverseRect.parent.name, Is.EqualTo("Doors"));
            AssertVector2Within(forwardRect.anchoredPosition,
                new Vector2(640.84204f, -109.46669f), 0.0001f,
                "Rear-to-Billiard trigger position");
            AssertVector2Within(forwardRect.sizeDelta,
                new Vector2(122.4507f, 282.7566f), 0.0001f,
                "Rear-to-Billiard trigger size");
            AssertVector2Within(reverseRect.anchoredPosition,
                new Vector2(-623.16205f, 61.70283f), 0.0001f,
                "Billiard-to-Rear trigger position");
            AssertVector2Within(reverseRect.sizeDelta,
                new Vector2(243.676f, 352.8653f), 0.0001f,
                "Billiard-to-Rear trigger size");
            AssertVector2Within(new Vector2(forwardRect.localScale.x, forwardRect.localScale.y),
                Vector2.one, 0.0001f, "Rear-to-Billiard trigger scale");
            AssertVector2Within(new Vector2(reverseRect.localScale.x, reverseRect.localScale.y),
            ...
            PolygonCollider2D rearBoundary = rearContent.GetComponentsInChildren<PolygonCollider2D>(true)
                .Single(collider => collider.name == "PlayerBoundary");
            PolygonCollider2D billiardBoundary = billiardContent.GetComponentsInChildren<PolygonCollider2D>(true)
                .Single(collider => collider.name == "PlayerBoundary");
            Assert.That(rearBoundary.pathCount, Is.EqualTo(1));
            Assert.That(rearBoundary.GetPath(0), Has.Length.EqualTo(5));
            Assert.That(billiardBoundary.pathCount, Is.EqualTo(1));
            Assert.That(billiardBoundary.GetPath(0), Has.Length.EqualTo(11));

            string[] rearRailNames =
            {
                "back_entrance_railing_left_0",
                "back_entrance_railing_right_0"
            };
            int[] rearRailOrders = { 1698, 1690 };
            for (int railIndex = 0; railIndex < rearRailNames.Length; railIndex++)
            {
                SpriteRenderer rail = FindInActiveScene<SpriteRenderer>()
                    .Single(item => item.name == rearRailNames[railIndex]);
                Assert.That(rail.GetComponentInParent<RoomContentGroup>(true), Is.SameAs(rearContent));
                Assert.That(rail.sortingOrder, Is.EqualTo(rearRailOrders[railIndex]));
            }
            ...
            int[] cutoutComponentCounts = { 3, 2, 2 };
            for (int pairIndex = 0; pairIndex < cutoutNames.Length; pairIndex++)
            {
                Transform cutout = FindInActiveScene<Transform>()
                    .Single(item => item.name == cutoutNames[pairIndex]);
                SpriteRenderer cutoutRenderer = cutout.GetComponent<SpriteRenderer>();
                Assert.That(cutout.GetComponentInParent<RoomContentGroup>(true), Is.SameAs(billiardContent));
                Assert.That(cutout.GetComponents<Component>(),
                    Has.Length.EqualTo(cutoutComponentCounts[pairIndex]));
                Assert.That(cutoutRenderer, Is.Not.Null);
                Assert.That(cutoutRenderer.sortingOrder, Is.EqualTo(1000));

                ObjectMovementBlocker2D blocker = FindInActiveScene<ObjectMovementBlocker2D>()
                    .Single(item => item.name == blockerNames[pairIndex]);
                Assert.That(blocker.GetComponentInParent<RoomContentGroup>(true),
                    Is.SameAs(billiardContent));
                Assert.That(blocker.GetComponents<Component>(), Has.Length.EqualTo(3));
                Assert.That(blocker.SourceObjectName, Is.EqualTo(cutoutNames[pairIndex]));
                Assert.That(blocker.SourceRoomName, Is.EqualTo(BilliardRoom));
                Assert.That(blocker.BlockingCollider, Is.Not.Null);
            }
            ...
            string serializedReverseRect = RequireSerializedUnityDocument(sceneText, "2300000121", "224");
            Assert.That(System.Text.RegularExpressions.Regex.Matches(
                sceneText,
                "^--- !u!",
                System.Text.RegularExpressions.RegexOptions.Multiline).Count,
            ...
            Assert.That(serializedReverse, Does.Contain("m_GameObject: {fileID: 2300000120}"));
            Assert.That(serializedForwardRect, Does.Contain("m_AnchoredPosition: {x: 640.84204, y: -109.46669}"));
            Assert.That(serializedForwardRect, Does.Contain("m_SizeDelta: {x: 122.4507, y: 282.7566}"));
            Assert.That(serializedReverseRect, Does.Contain("m_AnchoredPosition: {x: -623.16205, y: 61.70283}"));
            Assert.That(serializedReverseRect, Does.Contain("m_SizeDelta: {x: 243.676, y: 352.8653}"));
            foreach (string triggerDocument in new[] { serializedForward, serializedReverse })
            {
            ...
            System.Text.RegularExpressions.MatchCollection triggerDocuments =
                System.Text.RegularExpressions.Regex.Matches(
                    sceneText.Replace("\r\n", "\n"),
                    "(?ms)^--- !u!114 &\\d+\\n.*?(?=^--- !u!|\\z)");
            string[] serializedTriggerDocuments = triggerDocuments
            ...
            Assert.That(serializedTriggerDocuments.Count(document =>
                document.Contains("canonicalPassage: {fileID:") &&
                !document.Contains("canonicalPassage: {fileID: 0}")), Is.EqualTo(22));
            Assert.That(serializedTriggerDocuments.Count(document =>
                !document.Contains("canonicalPassage:")), Is.EqualTo(23));

            string legacyDoorData = System.IO.File.ReadAllText(System.IO.Path.Combine(
                projectRoot,
                "Assets/Resources/Navigation/doors.txt"));
            DoorDataParseResult legacyCatalog = DoorDataParser.Parse(legacyDoorData);
            Assert.That(legacyCatalog.IsValid, Is.True);
            DoorRoute catalogForward = legacyCatalog.RoutesByDoorId["GEH_BilliardRoom"];
            DoorRoute catalogReverse = legacyCatalog.RoutesByDoorId["BilliardRoom_GEH"];
            Assert.That(catalogForward.SourceRoom, Is.EqualTo(EntranceRoom));
            Assert.That(catalogForward.DestinationRoom, Is.EqualTo(BilliardRoom));
            Assert.That(catalogReverse.SourceRoom, Is.EqualTo(BilliardRoom));
            Assert.That(catalogReverse.DestinationRoom, Is.EqualTo(EntranceRoom));
            Assert.That(catalogForward.SourceRoom, Is.Not.EqualTo(forward.SourceRoom));
            Assert.That(catalogReverse.DestinationRoom, Is.Not.EqualTo(reverse.DestinationRoom));
            ...
            for (int profileIndex = 0; profileIndex < renderedSizes.Length; profileIndex++)
            {
                Vector2Int renderedSize = renderedSizes[profileIndex];
                bool maximumZoom = profileIndex == renderedSizes.Length - 1;
                bool includeNear = profileIndex == 0;
                yield return SetAndWaitForRenderedGameViewResolution(
                    (uint)renderedSize.x,
                    (uint)renderedSize.y);
                cameraManager.ResetRoomLookForPreview();
                ...
                List<string> orderedEvents = new List<string>();
                List<Vector2> arrivalCallbacks = new List<Vector2>();
                List<Vector2> arrivalCallbackLocals = new List<Vector2>();
                List<Vector2> movementStopCallbacks = new List<Vector2>();
                System.Action recordArrival = () =>
                {
                    orderedEvents.Add(
                        $"arrived:{navigation.CurrentRoom}:" +
                        (GetPrivateStaticField<AudioSource>(typeof(DoorTriggerNavigation),
                            ...
                            ? "audio-idle"
                            : "audio-started"));
                    arrivalCallbacks.Add(player.LogicalPosition);
                    Assert.That(TryMapLogicalPointToActiveRoomStageLocal(
                        player,
                        cameraManager,
                        player.LogicalPosition,
                        out Vector2 local), Is.True);
                    arrivalCallbackLocals.Add(local);
                };
                System.Action recordMovementStop = () =>
                {
                    orderedEvents.Add(
                        $"movement-stopped:{navigation.CurrentRoom}:" +
                        (GetPrivateStaticField<AudioSource>(typeof(DoorTriggerNavigation),
                            ...
                            ? "audio-idle"
                            : "audio-started"));
                    movementStopCallbacks.Add(player.LogicalPosition);
                };
                UnityEngine.Events.UnityAction<string> recordRoomChanged = room =>
                    orderedEvents.Add(
                        $"room-changed:{room}:" +
                ...
                try
                {
                    IEnumerator routine = ObserveDoorRoundTrip(
                        ...
                        sourceContentRoom: RearLegacyName);
                    while (routine.MoveNext())
                    {
                        yield return routine.Current;
                    }
                ...
                Assert.That(observation.HasForwardNullRoomViewLocal, Is.True);
                Assert.That(observation.HasReverseNullRoomViewLocal, Is.True);
                Assert.That(observation.HasForwardArrivalRoomViewLocal, Is.True);
                Assert.That(observation.HasReverseArrivalRoomViewLocal, Is.True);
                Assert.That(arrivalCallbacks, Has.Count.EqualTo(2),
                    "Only the two far auto-walk legs may emit arrival callbacks.");
                Assert.That(movementStopCallbacks, Has.Count.EqualTo(2),
                    "Only the two far auto-walk legs may emit movement-stop callbacks.");
                Assert.That(arrivalCallbackLocals, Has.Count.EqualTo(2));
                AssertVector2Within(arrivalCallbacks[0], observation.ForwardProductionApproach,
                    0.0001f, "actual Rear approach stop");
                AssertVector2Within(arrivalCallbacks[1], observation.ReverseProductionApproach,
                    0.0001f, "actual Billiard approach stop");
                AssertVector2Within(movementStopCallbacks[0], arrivalCallbacks[0],
                    0.0001f, "Rear movement-stop position");
                AssertVector2Within(movementStopCallbacks[1], arrivalCallbacks[1],
                    0.0001f, "Billiard movement-stop position");
                AssertFinite(arrivalCallbackLocals[0], "Rear approach stop RoomView-local");
                AssertFinite(arrivalCallbackLocals[1], "Billiard approach stop RoomView-local");
                AssertFinite(observation.ForwardArrival, "Billiard arrival logical");
                AssertFinite(observation.ForwardArrivalRoomViewLocal, "Billiard arrival RoomView-local");
                AssertFinite(observation.ReverseArrival, "Rear arrival logical");
                AssertFinite(observation.ReverseArrivalRoomViewLocal, "Rear arrival RoomView-local");

                List<string> expectedEvents = new List<string>
                {
                    $"arrived:{RearDisplayName}:audio-idle",
                    $"movement-stopped:{RearDisplayName}:audio-idle",
                    $"room-changed:{BilliardRoom}:audio-started",
                    $"arrived:{BilliardRoom}:audio-idle",
                    $"movement-stopped:{BilliardRoom}:audio-idle",
                    $"room-changed:{RearDisplayName}:audio-started"
                ...
                if (includeNear)
                {
                    expectedEvents.Add($"room-changed:{BilliardRoom}:audio-started");
                    expectedEvents.Add($"room-changed:{RearDisplayName}:audio-started");
                    AssertFinite(observation.NearForwardArrival, "near Billiard arrival");
                    AssertFinite(observation.NearReverseArrival, "near Rear arrival");
                }
                Assert.That(orderedEvents, Is.EqualTo(expectedEvents));
                Assert.That(navigation.CurrentRoom, Is.EqualTo(RearDisplayName));
                Assert.That(RequireOnlyActiveRoom(RearLegacyName), Is.SameAs(rearContent));
                Assert.That(cameraManager.cameraBackground.texture,
                    Is.SameAs(GetPrivateField<Texture>(rearContent, "roomBackgroundTexture")));
                DoorTriggerNavigation incidentalBottomEdgeHover = DoorTriggerNavigation.HoveredTrigger;
                Assert.That(incidentalBottomEdgeHover, Is.Not.SameAs(forward),
                    "Rear-to-Billiard hover must clear after traversal.");
                Assert.That(incidentalBottomEdgeHover, Is.Not.SameAs(reverse),
                    "Billiard-to-Rear hover must clear after traversal.");
                if (incidentalBottomEdgeHover != null)
                {
                    Assert.That(incidentalBottomEdgeHover.gameObject.name,
                        Is.EqualTo("DoorTrigger_GEH_Rear_GEH_Front"),
                        "The rendered test pointer may independently remain over the accepted Group10 bottom edge.");
                    incidentalBottomEdgeHover.OnPointerExit(null);
                }
                Assert.That(DoorTriggerNavigation.HoveredTrigger, Is.Null);
                Assert.That(GetPrivateStaticField<DoorTriggerNavigation>(typeof(DoorTriggerNavigation),
                    "pendingApproachTrigger"), Is.Null);
                Assert.That(GetPrivateStaticField<AudioSource>(typeof(DoorTriggerNavigation),
                    "activeNavigationAudioSource"), Is.Null);
                Assert.That(passagePromptText.gameObject.activeSelf, Is.False);
                Assert.That(passageAudioSource.GetComponents<GameAudioSourceVolume>(),
                    Has.Length.EqualTo(1));

                string profileLine =
                    $"[GrandEntranceRearBilliardLegacyProfile] viewport={renderedSize.x}x{renderedSize.y} " +
                    $"zoom={(maximumZoom ? "maximum" : "default")} events={string.Join("|", orderedEvents)} " +
                    $"forwardStop={FormatVector(arrivalCallbacks[0])} " +
                    $"forwardStopLocal={FormatVector(arrivalCallbackLocals[0])} " +
                    $"forwardNull={FormatVector(observation.ForwardNull)} " +
                    $"forwardNullLocal={FormatVector(observation.ForwardNullRoomViewLocal)} " +
                    $"forwardArrival={FormatVector(observation.ForwardArrival)} " +
                    $"forwardArrivalLocal={FormatVector(observation.ForwardArrivalRoomViewLocal)} " +
                    $"reverseStop={FormatVector(arrivalCallbacks[1])} " +
                    $"reverseStopLocal={FormatVector(arrivalCallbackLocals[1])} " +
                    $"reverseNull={FormatVector(observation.ReverseNull)} " +
                    $"reverseNullLocal={FormatVector(observation.ReverseNullRoomViewLocal)} " +
                    $"reverseArrival={FormatVector(observation.ReverseArrival)} " +
                    $"reverseArrivalLocal={FormatVector(observation.ReverseArrivalRoomViewLocal)} " +
                    FormatDoorRoundTripObservation(observation, includeNear);
                profileLines.Add(profileLine);
                Debug.Log(profileLine);
            }

            string structuralLine =
                "[GrandEntranceRearBilliardLegacyStructure] " +
                "triggerIds=357269799/2300000124 " +
                $"names={forward.gameObject.name}/{reverse.gameObject.name} " +
                $"geometry={FormatVector(forwardRect.anchoredPosition)}/{FormatVector(forwardRect.sizeDelta)}/" +
                $"{FormatVector(reverseRect.anchoredPosition)}/{FormatVector(reverseRect.sizeDelta)} " +
                "profile=door/proximity145/autowalk/autoactivate " +
                "serializedDependencies=null/null callers=null/null counts=45/12/22/22/23 " +
                "boundaries=rear5/billiard11 rails=2 cutoutBlockerPairs=3 doorButtons=0 " +
                "catalog=Grand Entrance Hall->Billiard Room/Billiard Room->Grand Entrance Hall " +
                "inspector=Grand Entrance Hall Rear view->Billiard Room/" +
                "Billiard Room->Grand Entrance Hall Rear View";
            profileLines.Add(structuralLine);
            Debug.Log(structuralLine);
            Assert.That(profileLines, Has.Count.EqualTo(6));
            string observationProfile = string.Join("\n", profileLines);
            Assert.That(observationProfile.EndsWith("\n", System.StringComparison.Ordinal), Is.False);
            string actualSha256 = ComputeSha256(observationProfile);
            Debug.Log($"[GrandEntranceRearBilliardLegacySha256] {actualSha256}");
            Assert.That(actualSha256, Is.EqualTo(ExpectedObservationSha256),
        ...
        bool drawingVisible)
    {
        CanonicalRoomView[] roomViews = FindInActiveScene<CanonicalRoomView>();
        Assert.That(roomViews, Has.Length.EqualTo(11),
            "The first ten completed reciprocal groups must retain their eleven passive RoomView scene owners.");
        CanonicalPassage[] passages = FindInActiveScene<CanonicalPassage>();
        Assert.That(passages, Has.Length.EqualTo(20),
            "The ten complete route grafts must remain exactly twenty reciprocal canonical scene bindings.");
        ...
        Assert.That(
            FindInActiveScene<DoorTriggerNavigation>()
                .Count(trigger => GetPrivateField<CanonicalPassage>(trigger, "canonicalPassage") != null),
            Is.EqualTo(20),
            "All eighteen complete stage-2 Passages must retain their canonical callers.");
        Assert.That(
            FindInActiveScene<DoorTriggerNavigation>()
                .Count(trigger => GetPrivateField<CanonicalPassage>(trigger, "canonicalPassage") == null),
            Is.EqualTo(25));
        ...
        bool libraryVisible)
    {
        ...
        Assert.That(
            FindInActiveScene<CanonicalPassage>()
                .Count(passage => passage.AnchorMigrationStage == PassageAnchorMigrationStage.AuthoredAnchors),
            Is.EqualTo(20),
            "All eighteen registered scene passages must be complete stage-2 routes.");
        Assert.That(forwardPassage.HasGameContext, Is.True);
        Assert.That(reversePassage.HasGameContext, Is.True);
        Assert.That(forwardPassage.enabled, Is.True);
        Assert.That(reversePassage.enabled, Is.True);

...
public sealed class ButlerRoomScaleCalibrationWindow : EditorWindow
{
    ...
    private void DrawAdvancedTools(PointClickPlayerMovement movement, string selectedRoom, string[] rooms)
    {
        ...
        using (new EditorGUI.DisabledScope(Application.isPlaying))
        {
                    ...
                    "Cancel"))
            {
                ...
                previewSize = ReadCurrentFinalLocalScaleY(movement);
                MarkDirty(movement);
    ...
    private void SetSelectedButler(PointClickPlayerMovement movement, string status)
    {
        ...
        previewSize = ReadCurrentFinalLocalScaleY(selectedButler);
        lastStatus = status;
    }
    ...
    internal static float ReadCurrentFinalLocalScaleY(PointClickPlayerMovement movement)
    {
        return movement != null
            ? Mathf.Max(0.001f, Mathf.Abs(movement.transform.localScale.y))
            : 1f;
    }

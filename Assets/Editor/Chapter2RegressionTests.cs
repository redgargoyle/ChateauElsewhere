...
public class Chapter2RegressionTests
{
    ...
    public void Chapter2HidesGuestsAtSevenThenSeatsGuestsOnDiningRoomReveal()
    {
        ...
        Assert.That(guestSearchText, Does.Match(@"(?s)\bSeatGuestsInDiningRoom\s*\(\s*List<GuestSearchEntry>\s+guestsToSeat\s*\)\s*\{.*SetGuestProjectionSortingEnabled\(guest, false\).*SetSeated\(true\).*ApplyDiningRoomGuestSorting\(guest, i\).*ApplyState\(\)"), "Dining seating should prevent projected guests from overriding the authored dinner foreground composition.");
        Assert.That(guestSearchText, Does.Contain("diningGuestSortingOrderBase = 1620"), "Dining guests should sort below the authored foreground chair/table overlays.");
        ...
        Assert.That(guestSearchText, Does.Match(@"(?s)\bSetGuestProjectionSortingEnabled\s*\([^)]*\)\s*\{.*GetComponentsInChildren<RoomProjectedEntity>\(true\).*SetApplySorting\(enabled, enabled\)"), "RoomProjectedEntity should keep dining position/scale projection without fighting dining sorting.");
    ...
    public void Chapter3DebugSkipStagesFoundGuestsInDiningRoom()
    {
        ...
        Assert.That(guestSearchText, Does.Match(@"(?s)\bSeatGuestsInDiningRoom\s*\(\s*List<GuestSearchEntry>\s+guestsToSeat\s*\)\s*\{.*SetCurrentRoom\(diningSeat\.RoomId\).*SetVisibleByChapterState\(true\).*SetSeated\(true\).*ApplyDiningRoomGuestSorting\(guest, i\).*ApplyState\(\)"), "Chapter 3 debug skip seating should place guests in the authored Dining Room sorting band.");
    }

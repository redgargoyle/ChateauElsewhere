...
public class Chapter2RegressionTests
{
    ...
    public void ChapterSubtitlesDecorateExistingFlowWithoutOwningGameplay()
    {
        ...
        string queueEntranceDialogueBody = ExtractMethodBody(chapter1Text, "private void QueueGuestEntranceDialogue");
        string completeEntranceDialogueBody = ExtractMethodBody(chapter1Text, "private void CompleteGuestEntranceDialogueAndMoveToCoatSpot");
        string moveToCoatSpotBody = ExtractMethodBody(chapter1Text, "private IEnumerator MoveGuestFromFrontAnchorToCoatSpot");
        string queueGuestLineBody = ExtractMethodBody(chapter1Text, "private void QueueGuestLine");
        ...
        string beginEntranceGroupMoveBody = ExtractMethodBody(chapter1Text, "private void BeginEntranceGroupMoveToDrawingRoom");
        ...
        Assert.That(chapter1AdmissionBody, Does.Not.Contain("QueueGuestLine"), "Guest dialogue must not begin during the door-to-front-anchor movement.");
        Assert.That(chapter1AdmissionBody, Does.Contain("GetFrontEntranceGuestAnchor(guest)"), "The first leg should use the guest's assigned front entrance anchor.");
        Assert.That(chapter1AdmissionBody, Does.Contain("FrontEntranceAnchorReached = moveResolved && reachedFrontAnchor"), "The first-leg result must record physical arrival rather than merely a stopped mover.");
        ...
        Assert.That(chapter1AdmitGroupsBody, Does.Contain("orderedGroups.Sort(CompareEntranceGroupsByRosterOrder)"), "Accumulated groups should retain roster order through two-stage admission.");
        Assert.That(chapter1AdmitGroupsBody, Does.Contain("while (!AreFrontEntranceMovesResolved(group))"), "Dialogue should wait for every first-leg movement result in the pair.");
        Assert.That(chapter1AdmitGroupsBody, Does.Contain("if (!HasEntranceGroupReachedFrontAnchors(group))"), "A failed first-leg route must block dialogue and coat staging for the pair.");
        Assert.That(chapter1AdmitGroupsBody, Does.Contain("QueueGuestEntranceDialogue(orderedGuests[guestIndex])"), "Only the post-arrival ordered pass should queue guest dialogue.");
        int firstEntranceMoveIndex = chapter1AdmitGroupsBody.IndexOf("StartCoroutine(AdmitGuestToEntranceHall", System.StringComparison.Ordinal);
        int frontMoveBarrierIndex = chapter1AdmitGroupsBody.IndexOf("while (!AreFrontEntranceMovesResolved(group))", System.StringComparison.Ordinal);
        int frontArrivalGateIndex = chapter1AdmitGroupsBody.IndexOf("if (!HasEntranceGroupReachedFrontAnchors(group))", System.StringComparison.Ordinal);
        int queueEntranceDialogueIndex = chapter1AdmitGroupsBody.IndexOf("QueueGuestEntranceDialogue(orderedGuests[guestIndex])", System.StringComparison.Ordinal);
        Assert.That(firstEntranceMoveIndex, Is.GreaterThanOrEqualTo(0));
        Assert.That(frontMoveBarrierIndex, Is.GreaterThan(firstEntranceMoveIndex), "All first-leg movement must start before the pair arrival barrier.");
        Assert.That(frontArrivalGateIndex, Is.GreaterThan(frontMoveBarrierIndex), "Physical arrival must be checked after all movement results resolve.");
        Assert.That(queueEntranceDialogueIndex, Is.GreaterThan(frontArrivalGateIndex), "No guest line may be queued before the complete pair reaches its anchors.");
        Assert.That(queueEntranceDialogueBody, Does.Contain("!guest.FrontEntranceAnchorReached"), "A guest who did not reach the assigned anchor must never speak.");
        Assert.That(queueEntranceDialogueBody, Does.Contain("guest.EntranceDialogueQueued"), "Entry dialogue should have a one-shot queue guard.");
        Assert.That(queueEntranceDialogueBody, Does.Contain("CompleteGuestEntranceDialogueAndMoveToCoatSpot(guest)"), "The last entry line should release that guest's second movement leg.");
        Assert.That(completeEntranceDialogueBody, Does.Contain("guest.EntranceDialogueAdvanceStarted"), "Completion, skipping, and cancellation must share one idempotent continuation guard.");
        Assert.That(completeEntranceDialogueBody, Does.Match(@"guest\.EntranceDialogueAdvanceStarted = true;[\s\S]*StartCoroutine\(MoveGuestFromFrontAnchorToCoatSpot\(guest\)\)"), "The one-shot guard must be set before starting the second leg.");
        Assert.That(moveToCoatSpotBody, Does.Contain("Transform waitSpot = GetEntranceHallGuestSpot(guest)"), "The existing EntranceGuestSpot remains the second-stage coat position.");
        Assert.That(moveToCoatSpotBody, Does.Contain("GetEntranceApproachAnimationDirection(guest, waitSpot)"), "The second leg should select its own signed display direction.");
        Assert.That(moveToCoatSpotBody, Does.Contain("if (!moveResolved || !reachedWaitSpot)"), "A stopped or failed second-leg mover must not unlock coat interaction.");
        int secondMoveIndex = moveToCoatSpotBody.IndexOf("yield return MoveGuestTo", System.StringComparison.Ordinal);
        int secondArrivalGateIndex = moveToCoatSpotBody.IndexOf("if (!moveResolved || !reachedWaitSpot)", System.StringComparison.Ordinal);
        int offerCoatIndex = moveToCoatSpotBody.IndexOf("OfferGuestCoat(guest)", System.StringComparison.Ordinal);
        Assert.That(secondMoveIndex, Is.GreaterThanOrEqualTo(0));
        Assert.That(secondArrivalGateIndex, Is.GreaterThan(secondMoveIndex));
        Assert.That(offerCoatIndex, Is.GreaterThan(secondArrivalGateIndex), "Coat interaction must remain disabled until the assigned coat spot is physically reached.");
        Assert.That(queueGuestLineBody, Does.Contain("onComplete?.Invoke()"), "Missing speech services should release the same guarded second-leg continuation.");
        Assert.That(queueGuestLineBody, Does.Match(@"service\.BeginSpeakLine\([\s\S]*onComplete\)"), "Normal completion, skip, and queue cancellation should all use the shared speech completion callback.");
        ...
        int beginPairMovementIndex = moveEntranceGroupBody.IndexOf("BeginEntranceGroupMoveToDrawingRoom(group)", System.StringComparison.Ordinal);
    ...
    public void Chapter2GuestPanicExitsHalfTheGuestsThroughEachDrawingRoomDoor()
    {
        ...
        Assert.That(panicText, Does.Contain("FindActiveRoutePlanner"), "Guest panic should resolve the named Player movement component through the shared route-planner locator.");
    ...
    public void Chapter2GuestPreferenceExitWalksToDoorBeforeDiningTransfer()
    {
        ...
        Assert.That(waypointText, Does.Contain("PointClickPlayerMovement"), "Guest movement should reuse the Butler's existing floor-boundary authority.");
        Assert.That(waypointText, Does.Contain("TryBuildReachableLogicalPath"), "Guest movement should consume the Butler's reachable route rather than crossing floor voids.");
        Assert.That(waypointText, Does.Contain("MoveLogicalPointToward"), "Guest route steps should use the Butler's shared y-axis movement function.");
        Assert.That(waypointText, Does.Contain("TryGetLogicalPositionFromWorldPoint"), "Guest routes should be stored in stable room-stage coordinates.");
        Assert.That(waypointText, Does.Contain("TryGetWorldPointFromLogicalPosition"), "Guest routes should follow room pan and zoom without leaving the floor.");
        Assert.That(waypointText, Does.Contain("TryFindArrivalDestination"), "Door exits should use the door's reachable floor approach instead of its tall trigger center.");
        Assert.That(waypointText, Does.Contain("GetCurrentFloorWorldPoint"), "The route should bind the guest's visible feet to the floor, not its transform pivot.");
        Assert.That(waypointText, Does.Contain("Vector3.MoveTowards"), "A direct transform fallback should remain available only when no gameplay floor planner exists.");
        Assert.That(waypointText, Does.Contain("Only explicit opt-outs keep the legacy transform path"), "The direct fallback must require an explicit non-gameplay opt-out.");
        Assert.That(waypointText, Does.Not.Contain("AddComponent<PointClickPlayerMovement>"), "Guests must not receive a duplicate navigation controller.");
        Assert.That(guestSearchText, Does.Contain("mover.AlignVisibleFeetToWaypoints = true"), "Chapter 2 exits should consistently use guest feet as the route contact point.");

...
public class Chapter1GuestRoomVisibilityRegressionTests
{
    ...
    public void LiveDoorAnswerUsesStableEntranceWorldPositions()
    {
        ...
        string projectedDoorBody = ExtractDeclaredMethodBody(controllerText, "TryPlaceProjectedGuestFeetAtTarget");
        ...
        Assert.That(admitBody, Does.Contain("PrepareGuestAtDoorArrival(guest)"), "The initial door spawn should use the shared feet-and-scale preparation path.");
        ...
        Assert.That(projectedDoorBody, Does.Contain("SetRoomLocalFootPoint(footPoint)"), "Projected door spawning should align every guest directly to the same door-foot anchor.");
        ...
        Assert.That(placeFeetBody, Does.Contain("BindGuestToRoomStagePoint(guestState, profileTarget)"), "World guests should remain locked to the door anchor until movement begins.");
    ...
    public void EntranceDoorSpawnAlignsEveryGuestToOneFootAnchorAfterScaling()
    {
        ...
        string projectedMethodBody = ExtractDeclaredMethodBody(controllerText, "TryPlaceProjectedGuestFeetAtTarget");
        ...
        Assert.That(projectedMethodBody, Does.Contain("projection.SetRoomLocalFootPoint(footPoint)"), "Projected guests should bind their feet directly to GuestArrival_Door.");
        Assert.That(projectedMethodBody, Does.Not.Contain("roomLocalPairOffset"), "Projected door placement should not add a per-guest spawn offset.");
    ...
    [Test]
    public void EntranceHallGuestDepthSortingRefreshesAfterGuestsMove()
    {
        string controllerText = File.ReadAllText(Chapter1ArrivalControllerPath);
        string lateUpdateMethodBody = ExtractDeclaredMethodBody(controllerText, "LateUpdate");
        string refreshMethodBody = ExtractDeclaredMethodBody(controllerText, "RefreshEntranceHallGuestDepthSorting");
        string applyMethodBody = ExtractDeclaredMethodBody(controllerText, "ApplyEntranceHallGuestSorting");

        Assert.That(lateUpdateMethodBody, Does.Contain("RefreshEntranceHallGuestDepthSorting()"), "Entrance guest depth must refresh after movement and animation updates, like the butler's LateUpdate sorting.");
        Assert.That(refreshMethodBody, Does.Contain("guestStates.Count"), "The refresh must cover every configured Chapter 1 guest, not only the most recently admitted guest.");
        Assert.That(refreshMethodBody, Does.Contain("guestState.EnteredEntranceHall"), "Only guests actively present in the Entrance Hall should receive its temporary depth override.");
        Assert.That(refreshMethodBody, Does.Contain("ApplyEntranceHallGuestSorting(guestState)"), "The per-frame refresh must reuse the existing shared guest foot-depth sorter instead of creating a competing formula.");
        Assert.That(refreshMethodBody, Does.Not.Contain("GetSortingOrderForFootY"), "The refresh should delegate order calculation to the shared guest sorter.");
        Assert.That(applyMethodBody, Does.Contain("GetEntranceHallGuestSortingOrder"), "Each refreshed guest order must still come from the shared butler-compatible foot-Y calculation.");
    }

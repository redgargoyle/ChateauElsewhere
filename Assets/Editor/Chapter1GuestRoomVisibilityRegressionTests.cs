...
public class Chapter1GuestRoomVisibilityRegressionTests
{
    ...
    public void LiveDoorAnswerUsesStableEntranceWorldPositions()
    {
        ...
        Assert.That(doorArrivalBaseBody, Does.Match(@"GetWorldDoorArrivalTarget[\s\S]*TryGetWorldPositionForGuestTarget[\s\S]*TryGetWorldFrontDoorAnswerSpot[\s\S]*GetWorldEntranceCenterPosition\(guestState\)"), "Front-door spawning should prefer the door-base anchor, then the Butler's cached answer spot, then the stable entrance fallback.");
        Assert.That(doorArrivalTargetBody, Does.Contain("GetFrontDoorArrivalPoint(frontDoorArrivalPoint)"), "Front-door spawning should use the configured GuestArrival_Door front entrance anchor.");
        ...
        Assert.That(guestArrivalDoorBlock, Does.Contain("m_LocalPosition: {x: -8, y: -94"), "GuestArrival_Door should be the centered front-door foot/base anchor.");
        Assert.That(doorAnswerTriggerBlock, Does.Contain("m_LocalPosition: {x: -7.216162"), "The clickable door-answer trigger should also be centered on the front entrance.");
    ...
    public void Chapter1GuestsUseAuthoredScaleAsRoomZoomBaseline()
    {
        ...
        string prepareMethodBody = ExtractMethodBody(controllerText, "private void PrepareSceneGuestObject");
        string disablePlayerMethodBody = ExtractMethodBody(controllerText, "private void DisablePlayerOnlyComponents");

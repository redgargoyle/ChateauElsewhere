...
public class Chapter1GuestRoomVisibilityRegressionTests
{
    ...
    public void GuestArrivalDoorIsTheOnlyEditableFootSpawnAndUsesButlerScale()
    {
        ...
        Assert.That(
            sceneText,
            Does.Match(@"roomId: Grand Entrance Hall\s+enabled: 1\s+frontRoomLocalFootY: -381\.67844\s+backRoomLocalFootY: -98\.47123\s+frontFinalLocalScaleY: 2\.1461537\s+backFinalLocalScaleY: 0\.661823\s"),
            "Entrance guests should use the one Butler-calibrated CharacterRoomScaleCatalog endpoint pair without the old 2.2 enlargement.");
